...
public class TextDigitalDocumentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TextDigitalDocument.jsonld");
        assertNotNull(is, "TextDigitalDocument.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TextDigitalDocument acc = mapper.readValue(jsonld, TextDigitalDocument.class);
        assertNotNull(acc);

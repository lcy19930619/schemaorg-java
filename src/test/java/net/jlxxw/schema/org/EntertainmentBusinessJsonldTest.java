...
public class EntertainmentBusinessJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EntertainmentBusiness.jsonld");
        assertNotNull(is, "EntertainmentBusiness.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EntertainmentBusiness acc = mapper.readValue(jsonld, EntertainmentBusiness.class);
        assertNotNull(acc);

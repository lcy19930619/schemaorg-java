...
public class AudioObjectSnapshotJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AudioObjectSnapshot.jsonld");
        assertNotNull(is, "AudioObjectSnapshot.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AudioObjectSnapshot acc = mapper.readValue(jsonld, AudioObjectSnapshot.class);
        assertNotNull(acc);

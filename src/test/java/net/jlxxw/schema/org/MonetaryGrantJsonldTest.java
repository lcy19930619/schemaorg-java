...
public class MonetaryGrantJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MonetaryGrant.jsonld");
        assertNotNull(is, "MonetaryGrant.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MonetaryGrant acc = mapper.readValue(jsonld, MonetaryGrant.class);
        assertNotNull(acc);

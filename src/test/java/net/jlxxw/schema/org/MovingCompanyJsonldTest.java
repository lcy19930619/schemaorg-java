...
public class MovingCompanyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MovingCompany.jsonld");
        assertNotNull(is, "MovingCompany.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MovingCompany acc = mapper.readValue(jsonld, MovingCompany.class);
        assertNotNull(acc);

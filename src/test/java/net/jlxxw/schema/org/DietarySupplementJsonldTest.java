...
public class DietarySupplementJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DietarySupplement.jsonld");
        assertNotNull(is, "DietarySupplement.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DietarySupplement acc = mapper.readValue(jsonld, DietarySupplement.class);
        assertNotNull(acc);

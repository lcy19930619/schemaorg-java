...
public class ImageObjectSnapshotJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ImageObjectSnapshot.jsonld");
        assertNotNull(is, "ImageObjectSnapshot.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ImageObjectSnapshot acc = mapper.readValue(jsonld, ImageObjectSnapshot.class);
        assertNotNull(acc);

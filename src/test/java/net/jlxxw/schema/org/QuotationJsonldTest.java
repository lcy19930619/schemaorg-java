...
public class QuotationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Quotation.jsonld");
        assertNotNull(is, "Quotation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Quotation acc = mapper.readValue(jsonld, Quotation.class);
        assertNotNull(acc);

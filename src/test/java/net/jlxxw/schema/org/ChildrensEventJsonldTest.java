...
public class ChildrensEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ChildrensEvent.jsonld");
        assertNotNull(is, "ChildrensEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ChildrensEvent acc = mapper.readValue(jsonld, ChildrensEvent.class);
        assertNotNull(acc);

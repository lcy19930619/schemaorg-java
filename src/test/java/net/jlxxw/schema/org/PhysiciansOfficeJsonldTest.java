...
public class PhysiciansOfficeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PhysiciansOffice.jsonld");
        assertNotNull(is, "PhysiciansOffice.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PhysiciansOffice acc = mapper.readValue(jsonld, PhysiciansOffice.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        PhysiciansOffice obj = new PhysiciansOffice();
        assertEquals("schema:PhysiciansOffice", obj.getId());

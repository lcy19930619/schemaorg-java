...
public class SportsEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SportsEvent.jsonld");
        assertNotNull(is, "SportsEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SportsEvent acc = mapper.readValue(jsonld, SportsEvent.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        SportsEvent obj = new SportsEvent();
        assertEquals("schema:SportsEvent", obj.getId());

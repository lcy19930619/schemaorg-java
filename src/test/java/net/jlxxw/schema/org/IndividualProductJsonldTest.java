...
public class IndividualProductJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("IndividualProduct.jsonld");
        assertNotNull(is, "IndividualProduct.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        IndividualProduct acc = mapper.readValue(jsonld, IndividualProduct.class);
        assertNotNull(acc);

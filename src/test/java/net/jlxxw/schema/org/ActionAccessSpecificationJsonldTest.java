...
public class ActionAccessSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ActionAccessSpecification.jsonld");
        assertNotNull(is, "ActionAccessSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ActionAccessSpecification acc = mapper.readValue(jsonld, ActionAccessSpecification.class);
        assertNotNull(acc);

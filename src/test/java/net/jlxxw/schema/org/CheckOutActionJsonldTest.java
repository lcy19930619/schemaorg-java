...
public class CheckOutActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CheckOutAction.jsonld");
        assertNotNull(is, "CheckOutAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CheckOutAction acc = mapper.readValue(jsonld, CheckOutAction.class);
        assertNotNull(acc);

...
public class SchoolDistrictJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SchoolDistrict.jsonld");
        assertNotNull(is, "SchoolDistrict.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SchoolDistrict acc = mapper.readValue(jsonld, SchoolDistrict.class);
        assertNotNull(acc);

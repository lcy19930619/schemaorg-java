...
public class MedicalTrialJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalTrial.jsonld");
        assertNotNull(is, "MedicalTrial.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalTrial acc = mapper.readValue(jsonld, MedicalTrial.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalTrial obj = new MedicalTrial();
        assertEquals("schema:MedicalTrial", obj.getId());

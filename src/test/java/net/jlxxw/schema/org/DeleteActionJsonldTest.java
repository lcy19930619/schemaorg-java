...
public class DeleteActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DeleteAction.jsonld");
        assertNotNull(is, "DeleteAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DeleteAction acc = mapper.readValue(jsonld, DeleteAction.class);
        assertNotNull(acc);

...
public class IntangibleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Intangible.jsonld");
        assertNotNull(is, "Intangible.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Intangible acc = mapper.readValue(jsonld, Intangible.class);
        assertNotNull(acc);

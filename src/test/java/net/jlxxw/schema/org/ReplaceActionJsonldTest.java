...
public class ReplaceActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReplaceAction.jsonld");
        assertNotNull(is, "ReplaceAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReplaceAction acc = mapper.readValue(jsonld, ReplaceAction.class);
        assertNotNull(acc);

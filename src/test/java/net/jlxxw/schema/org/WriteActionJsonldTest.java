...
public class WriteActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WriteAction.jsonld");
        assertNotNull(is, "WriteAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WriteAction acc = mapper.readValue(jsonld, WriteAction.class);
        assertNotNull(acc);

...
public class EndorsementRatingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EndorsementRating.jsonld");
        assertNotNull(is, "EndorsementRating.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EndorsementRating acc = mapper.readValue(jsonld, EndorsementRating.class);
        assertNotNull(acc);

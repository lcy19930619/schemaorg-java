...
public class SellActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SellAction.jsonld");
        assertNotNull(is, "SellAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SellAction acc = mapper.readValue(jsonld, SellAction.class);
        assertNotNull(acc);

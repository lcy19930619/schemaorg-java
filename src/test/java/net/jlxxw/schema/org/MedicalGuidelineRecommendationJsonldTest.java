...
public class MedicalGuidelineRecommendationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalGuidelineRecommendation.jsonld");
        assertNotNull(is, "MedicalGuidelineRecommendation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalGuidelineRecommendation acc = mapper.readValue(jsonld, MedicalGuidelineRecommendation.class);
        assertNotNull(acc);

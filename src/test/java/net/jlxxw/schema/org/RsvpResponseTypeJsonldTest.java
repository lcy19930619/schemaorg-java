...
public class RsvpResponseTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RsvpResponseType.jsonld");
        assertNotNull(is, "RsvpResponseType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RsvpResponseType acc = mapper.readValue(jsonld, RsvpResponseType.class);
        assertNotNull(acc);

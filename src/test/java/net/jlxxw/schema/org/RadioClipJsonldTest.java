...
public class RadioClipJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RadioClip.jsonld");
        assertNotNull(is, "RadioClip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RadioClip acc = mapper.readValue(jsonld, RadioClip.class);
        assertNotNull(acc);

...
public class CollectionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Collection.jsonld");
        assertNotNull(is, "Collection.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Collection acc = mapper.readValue(jsonld, Collection.class);
        assertNotNull(acc);

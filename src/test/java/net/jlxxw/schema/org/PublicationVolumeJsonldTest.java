...
public class PublicationVolumeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PublicationVolume.jsonld");
        assertNotNull(is, "PublicationVolume.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PublicationVolume acc = mapper.readValue(jsonld, PublicationVolume.class);
        assertNotNull(acc);

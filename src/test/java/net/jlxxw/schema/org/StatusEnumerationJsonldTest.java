...
public class StatusEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("StatusEnumeration.jsonld");
        assertNotNull(is, "StatusEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        StatusEnumeration acc = mapper.readValue(jsonld, StatusEnumeration.class);
        assertNotNull(acc);

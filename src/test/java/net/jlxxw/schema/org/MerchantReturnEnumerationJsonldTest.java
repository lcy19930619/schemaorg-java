...
public class MerchantReturnEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MerchantReturnEnumeration.jsonld");
        assertNotNull(is, "MerchantReturnEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MerchantReturnEnumeration acc = mapper.readValue(jsonld, MerchantReturnEnumeration.class);
        assertNotNull(acc);

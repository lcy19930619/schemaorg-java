...
public class MuscleJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Muscle.jsonld");
        assertNotNull(is, "Muscle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Muscle acc = mapper.readValue(jsonld, Muscle.class);
        assertNotNull(acc);

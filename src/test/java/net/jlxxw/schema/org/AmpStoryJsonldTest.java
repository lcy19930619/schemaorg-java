...
public class AmpStoryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AmpStory.jsonld");
        assertNotNull(is, "AmpStory.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AmpStory acc = mapper.readValue(jsonld, AmpStory.class);
        assertNotNull(acc);

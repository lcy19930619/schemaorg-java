...
public class HowToStepJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HowToStep.jsonld");
        assertNotNull(is, "HowToStep.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HowToStep acc = mapper.readValue(jsonld, HowToStep.class);
        assertNotNull(acc);

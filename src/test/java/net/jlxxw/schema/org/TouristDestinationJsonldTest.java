...
public class TouristDestinationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TouristDestination.jsonld");
        assertNotNull(is, "TouristDestination.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TouristDestination acc = mapper.readValue(jsonld, TouristDestination.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        TouristDestination obj = new TouristDestination();
        assertEquals("schema:TouristDestination", obj.getId());

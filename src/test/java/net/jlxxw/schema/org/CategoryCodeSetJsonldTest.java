...
public class CategoryCodeSetJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CategoryCodeSet.jsonld");
        assertNotNull(is, "CategoryCodeSet.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CategoryCodeSet acc = mapper.readValue(jsonld, CategoryCodeSet.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        CategoryCodeSet obj = new CategoryCodeSet();
        assertEquals("schema:CategoryCodeSet", obj.getId());

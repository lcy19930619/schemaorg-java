...
public class RoleJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Role.jsonld");
        assertNotNull(is, "Role.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Role acc = mapper.readValue(jsonld, Role.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Role obj = new Role();
        assertEquals("schema:Role", obj.getId());

...
public class WPHeaderJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WPHeader.jsonld");
        assertNotNull(is, "WPHeader.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WPHeader acc = mapper.readValue(jsonld, WPHeader.class);
        assertNotNull(acc);

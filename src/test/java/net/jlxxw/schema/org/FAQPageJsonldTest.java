...
public class FAQPageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FAQPage.jsonld");
        assertNotNull(is, "FAQPage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FAQPage acc = mapper.readValue(jsonld, FAQPage.class);
        assertNotNull(acc);

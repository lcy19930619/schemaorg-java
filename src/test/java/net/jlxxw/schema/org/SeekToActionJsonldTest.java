...
public class SeekToActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SeekToAction.jsonld");
        assertNotNull(is, "SeekToAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SeekToAction acc = mapper.readValue(jsonld, SeekToAction.class);
        assertNotNull(acc);

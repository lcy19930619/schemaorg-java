...
public class PropertyValueSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PropertyValueSpecification.jsonld");
        assertNotNull(is, "PropertyValueSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PropertyValueSpecification acc = mapper.readValue(jsonld, PropertyValueSpecification.class);
        assertNotNull(acc);

...
public class EnergyConsumptionDetailsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EnergyConsumptionDetails.jsonld");
        assertNotNull(is, "EnergyConsumptionDetails.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EnergyConsumptionDetails acc = mapper.readValue(jsonld, EnergyConsumptionDetails.class);
        assertNotNull(acc);

...
public class SyllabusJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Syllabus.jsonld");
        assertNotNull(is, "Syllabus.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Syllabus acc = mapper.readValue(jsonld, Syllabus.class);
        assertNotNull(acc);

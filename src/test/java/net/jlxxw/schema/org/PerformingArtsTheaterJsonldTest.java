...
public class PerformingArtsTheaterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PerformingArtsTheater.jsonld");
        assertNotNull(is, "PerformingArtsTheater.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PerformingArtsTheater acc = mapper.readValue(jsonld, PerformingArtsTheater.class);
        assertNotNull(acc);

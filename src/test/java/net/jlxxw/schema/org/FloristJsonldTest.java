...
public class FloristJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Florist.jsonld");
        assertNotNull(is, "Florist.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Florist acc = mapper.readValue(jsonld, Florist.class);
        assertNotNull(acc);

...
public class AggregateRatingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AggregateRating.jsonld");
        assertNotNull(is, "AggregateRating.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AggregateRating acc = mapper.readValue(jsonld, AggregateRating.class);
        assertNotNull(acc);

...
public class HomeGoodsStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HomeGoodsStore.jsonld");
        assertNotNull(is, "HomeGoodsStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HomeGoodsStore acc = mapper.readValue(jsonld, HomeGoodsStore.class);
        assertNotNull(acc);

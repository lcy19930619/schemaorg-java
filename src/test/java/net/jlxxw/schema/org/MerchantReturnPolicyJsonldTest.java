...
public class MerchantReturnPolicyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MerchantReturnPolicy.jsonld");
        assertNotNull(is, "MerchantReturnPolicy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MerchantReturnPolicy acc = mapper.readValue(jsonld, MerchantReturnPolicy.class);
        assertNotNull(acc);

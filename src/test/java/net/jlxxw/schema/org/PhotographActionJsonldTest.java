...
public class PhotographActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PhotographAction.jsonld");
        assertNotNull(is, "PhotographAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PhotographAction acc = mapper.readValue(jsonld, PhotographAction.class);
        assertNotNull(acc);

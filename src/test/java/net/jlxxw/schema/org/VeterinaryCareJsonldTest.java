...
public class VeterinaryCareJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VeterinaryCare.jsonld");
        assertNotNull(is, "VeterinaryCare.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VeterinaryCare acc = mapper.readValue(jsonld, VeterinaryCare.class);
        assertNotNull(acc);

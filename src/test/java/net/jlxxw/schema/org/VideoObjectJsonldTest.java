...
public class VideoObjectJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VideoObject.jsonld");
        assertNotNull(is, "VideoObject.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VideoObject acc = mapper.readValue(jsonld, VideoObject.class);
        assertNotNull(acc);

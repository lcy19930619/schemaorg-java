...
public class RadiationTherapyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RadiationTherapy.jsonld");
        assertNotNull(is, "RadiationTherapy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RadiationTherapy acc = mapper.readValue(jsonld, RadiationTherapy.class);
        assertNotNull(acc);

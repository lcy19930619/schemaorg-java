...
public class TaxiStandJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TaxiStand.jsonld");
        assertNotNull(is, "TaxiStand.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TaxiStand acc = mapper.readValue(jsonld, TaxiStand.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        TaxiStand obj = new TaxiStand();
        assertEquals("schema:TaxiStand", obj.getId());

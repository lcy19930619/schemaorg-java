...
public class BedAndBreakfastJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BedAndBreakfast.jsonld");
        assertNotNull(is, "BedAndBreakfast.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BedAndBreakfast acc = mapper.readValue(jsonld, BedAndBreakfast.class);
        assertNotNull(acc);

...
public class SynagogueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Synagogue.jsonld");
        assertNotNull(is, "Synagogue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Synagogue acc = mapper.readValue(jsonld, Synagogue.class);
        assertNotNull(acc);

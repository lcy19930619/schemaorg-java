...
public class RentActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RentAction.jsonld");
        assertNotNull(is, "RentAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RentAction acc = mapper.readValue(jsonld, RentAction.class);
        assertNotNull(acc);

...
public class WarrantyPromiseJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WarrantyPromise.jsonld");
        assertNotNull(is, "WarrantyPromise.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WarrantyPromise acc = mapper.readValue(jsonld, WarrantyPromise.class);
        assertNotNull(acc);

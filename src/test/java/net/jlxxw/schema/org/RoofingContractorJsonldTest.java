...
public class RoofingContractorJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RoofingContractor.jsonld");
        assertNotNull(is, "RoofingContractor.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RoofingContractor acc = mapper.readValue(jsonld, RoofingContractor.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        RoofingContractor obj = new RoofingContractor();
        assertEquals("schema:RoofingContractor", obj.getId());

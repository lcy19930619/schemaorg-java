...
public class SchoolJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("School.jsonld");
        assertNotNull(is, "School.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        School acc = mapper.readValue(jsonld, School.class);
        assertNotNull(acc);

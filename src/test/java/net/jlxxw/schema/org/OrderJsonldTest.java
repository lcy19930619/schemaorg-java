...
public class OrderJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Order.jsonld");
        assertNotNull(is, "Order.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Order acc = mapper.readValue(jsonld, Order.class);
        assertNotNull(acc);

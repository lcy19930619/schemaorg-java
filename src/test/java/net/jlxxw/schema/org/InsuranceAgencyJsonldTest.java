...
public class InsuranceAgencyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("InsuranceAgency.jsonld");
        assertNotNull(is, "InsuranceAgency.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        InsuranceAgency acc = mapper.readValue(jsonld, InsuranceAgency.class);
        assertNotNull(acc);

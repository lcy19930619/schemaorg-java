...
public class SocialMediaPostingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SocialMediaPosting.jsonld");
        assertNotNull(is, "SocialMediaPosting.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SocialMediaPosting acc = mapper.readValue(jsonld, SocialMediaPosting.class);
        assertNotNull(acc);

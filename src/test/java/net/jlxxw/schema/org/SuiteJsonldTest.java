...
public class SuiteJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Suite.jsonld");
        assertNotNull(is, "Suite.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Suite acc = mapper.readValue(jsonld, Suite.class);
        assertNotNull(acc);

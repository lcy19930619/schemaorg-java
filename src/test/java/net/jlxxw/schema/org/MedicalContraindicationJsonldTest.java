...
public class MedicalContraindicationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalContraindication.jsonld");
        assertNotNull(is, "MedicalContraindication.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalContraindication acc = mapper.readValue(jsonld, MedicalContraindication.class);
        assertNotNull(acc);

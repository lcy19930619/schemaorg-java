...
public class CDCPMDRecordJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CDCPMDRecord.jsonld");
        assertNotNull(is, "CDCPMDRecord.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CDCPMDRecord acc = mapper.readValue(jsonld, CDCPMDRecord.class);
        assertNotNull(acc);

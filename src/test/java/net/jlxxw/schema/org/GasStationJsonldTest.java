...
public class GasStationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GasStation.jsonld");
        assertNotNull(is, "GasStation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GasStation acc = mapper.readValue(jsonld, GasStation.class);
        assertNotNull(acc);

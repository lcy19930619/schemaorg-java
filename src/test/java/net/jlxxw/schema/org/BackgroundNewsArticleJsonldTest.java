...
public class BackgroundNewsArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BackgroundNewsArticle.jsonld");
        assertNotNull(is, "BackgroundNewsArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BackgroundNewsArticle acc = mapper.readValue(jsonld, BackgroundNewsArticle.class);
        assertNotNull(acc);

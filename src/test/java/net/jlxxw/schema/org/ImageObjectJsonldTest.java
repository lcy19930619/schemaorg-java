...
public class ImageObjectJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ImageObject.jsonld");
        assertNotNull(is, "ImageObject.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ImageObject acc = mapper.readValue(jsonld, ImageObject.class);
        assertNotNull(acc);

...
public class ServiceChannelJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ServiceChannel.jsonld");
        assertNotNull(is, "ServiceChannel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ServiceChannel acc = mapper.readValue(jsonld, ServiceChannel.class);
        assertNotNull(acc);

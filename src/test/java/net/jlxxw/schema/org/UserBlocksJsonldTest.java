...
public class UserBlocksJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserBlocks.jsonld");
        assertNotNull(is, "UserBlocks.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserBlocks acc = mapper.readValue(jsonld, UserBlocks.class);
        assertNotNull(acc);

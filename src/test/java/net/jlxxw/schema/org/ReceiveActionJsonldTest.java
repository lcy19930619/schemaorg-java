...
public class ReceiveActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReceiveAction.jsonld");
        assertNotNull(is, "ReceiveAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReceiveAction acc = mapper.readValue(jsonld, ReceiveAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ReceiveAction obj = new ReceiveAction();
        assertEquals("schema:ReceiveAction", obj.getId());

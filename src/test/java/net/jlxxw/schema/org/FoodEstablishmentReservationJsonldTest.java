...
public class FoodEstablishmentReservationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FoodEstablishmentReservation.jsonld");
        assertNotNull(is, "FoodEstablishmentReservation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FoodEstablishmentReservation acc = mapper.readValue(jsonld, FoodEstablishmentReservation.class);
        assertNotNull(acc);

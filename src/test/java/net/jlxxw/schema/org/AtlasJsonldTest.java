...
public class AtlasJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Atlas.jsonld");
        assertNotNull(is, "Atlas.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Atlas acc = mapper.readValue(jsonld, Atlas.class);
        assertNotNull(acc);

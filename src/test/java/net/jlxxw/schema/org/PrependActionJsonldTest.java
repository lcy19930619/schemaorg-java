...
public class PrependActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PrependAction.jsonld");
        assertNotNull(is, "PrependAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PrependAction acc = mapper.readValue(jsonld, PrependAction.class);
        assertNotNull(acc);

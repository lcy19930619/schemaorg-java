...
public class FinancialProductJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FinancialProduct.jsonld");
        assertNotNull(is, "FinancialProduct.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FinancialProduct acc = mapper.readValue(jsonld, FinancialProduct.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        FinancialProduct obj = new FinancialProduct();
        assertEquals("schema:FinancialProduct", obj.getId());

...
public class MedicalConditionStageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalConditionStage.jsonld");
        assertNotNull(is, "MedicalConditionStage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalConditionStage acc = mapper.readValue(jsonld, MedicalConditionStage.class);
        assertNotNull(acc);

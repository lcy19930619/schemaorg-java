...
public class MerchantReturnPolicySeasonalOverrideJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MerchantReturnPolicySeasonalOverride.jsonld");
        assertNotNull(is, "MerchantReturnPolicySeasonalOverride.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MerchantReturnPolicySeasonalOverride acc = mapper.readValue(jsonld, MerchantReturnPolicySeasonalOverride.class);
        assertNotNull(acc);

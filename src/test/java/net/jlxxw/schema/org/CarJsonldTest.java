...
public class CarJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Car.jsonld");
        assertNotNull(is, "Car.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Car acc = mapper.readValue(jsonld, Car.class);
        assertNotNull(acc);

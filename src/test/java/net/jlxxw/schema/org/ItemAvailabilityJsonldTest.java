...
public class ItemAvailabilityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ItemAvailability.jsonld");
        assertNotNull(is, "ItemAvailability.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ItemAvailability acc = mapper.readValue(jsonld, ItemAvailability.class);
        assertNotNull(acc);

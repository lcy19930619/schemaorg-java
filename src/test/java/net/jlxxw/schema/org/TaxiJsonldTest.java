...
public class TaxiJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Taxi.jsonld");
        assertNotNull(is, "Taxi.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Taxi acc = mapper.readValue(jsonld, Taxi.class);
        assertNotNull(acc);

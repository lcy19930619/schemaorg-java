...
public class PerformanceRoleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PerformanceRole.jsonld");
        assertNotNull(is, "PerformanceRole.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PerformanceRole acc = mapper.readValue(jsonld, PerformanceRole.class);
        assertNotNull(acc);

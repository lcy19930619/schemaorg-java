...
public class BusStationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BusStation.jsonld");
        assertNotNull(is, "BusStation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BusStation acc = mapper.readValue(jsonld, BusStation.class);
        assertNotNull(acc);

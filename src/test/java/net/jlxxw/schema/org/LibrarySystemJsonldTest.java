...
public class LibrarySystemJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LibrarySystem.jsonld");
        assertNotNull(is, "LibrarySystem.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LibrarySystem acc = mapper.readValue(jsonld, LibrarySystem.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ...
        assertEquals("schema:LibrarySystem", obj.getId());

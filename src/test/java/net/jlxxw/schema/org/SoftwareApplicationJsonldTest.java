...
public class SoftwareApplicationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SoftwareApplication.jsonld");
        assertNotNull(is, "SoftwareApplication.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SoftwareApplication acc = mapper.readValue(jsonld, SoftwareApplication.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ...
        assertEquals("schema:SoftwareApplication", obj.getId());

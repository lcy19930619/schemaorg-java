...
public class BefriendActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BefriendAction.jsonld");
        assertNotNull(is, "BefriendAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BefriendAction acc = mapper.readValue(jsonld, BefriendAction.class);
        assertNotNull(acc);

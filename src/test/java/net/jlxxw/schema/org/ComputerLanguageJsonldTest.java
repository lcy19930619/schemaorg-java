...
public class ComputerLanguageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ComputerLanguage.jsonld");
        assertNotNull(is, "ComputerLanguage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ComputerLanguage acc = mapper.readValue(jsonld, ComputerLanguage.class);
        assertNotNull(acc);

...
public class TransferActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TransferAction.jsonld");
        assertNotNull(is, "TransferAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TransferAction acc = mapper.readValue(jsonld, TransferAction.class);
        assertNotNull(acc);

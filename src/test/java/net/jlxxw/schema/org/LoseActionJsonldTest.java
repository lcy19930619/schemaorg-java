...
public class LoseActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LoseAction.jsonld");
        assertNotNull(is, "LoseAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LoseAction acc = mapper.readValue(jsonld, LoseAction.class);
        assertNotNull(acc);

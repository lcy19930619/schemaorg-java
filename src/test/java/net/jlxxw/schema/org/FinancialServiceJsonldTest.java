...
public class FinancialServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FinancialService.jsonld");
        assertNotNull(is, "FinancialService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FinancialService acc = mapper.readValue(jsonld, FinancialService.class);
        assertNotNull(acc);

...
public class RadioSeriesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RadioSeries.jsonld");
        assertNotNull(is, "RadioSeries.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RadioSeries acc = mapper.readValue(jsonld, RadioSeries.class);
        assertNotNull(acc);

...
public class ConsortiumJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Consortium.jsonld");
        assertNotNull(is, "Consortium.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Consortium acc = mapper.readValue(jsonld, Consortium.class);
        assertNotNull(acc);

...
public class AdultOrientedEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AdultOrientedEnumeration.jsonld");
        assertNotNull(is, "AdultOrientedEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AdultOrientedEnumeration acc = mapper.readValue(jsonld, AdultOrientedEnumeration.class);
        assertNotNull(acc);

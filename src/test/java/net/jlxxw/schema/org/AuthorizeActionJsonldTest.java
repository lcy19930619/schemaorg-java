...
public class AuthorizeActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AuthorizeAction.jsonld");
        assertNotNull(is, "AuthorizeAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AuthorizeAction acc = mapper.readValue(jsonld, AuthorizeAction.class);
        assertNotNull(acc);

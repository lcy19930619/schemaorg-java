...
public class ContactPointOptionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ContactPointOption.jsonld");
        assertNotNull(is, "ContactPointOption.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ContactPointOption acc = mapper.readValue(jsonld, ContactPointOption.class);
        assertNotNull(acc);

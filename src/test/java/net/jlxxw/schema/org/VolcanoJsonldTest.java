...
public class VolcanoJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Volcano.jsonld");
        assertNotNull(is, "Volcano.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Volcano acc = mapper.readValue(jsonld, Volcano.class);
        assertNotNull(acc);

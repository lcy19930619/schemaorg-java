...
public class MedicalRiskFactorJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalRiskFactor.jsonld");
        assertNotNull(is, "MedicalRiskFactor.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalRiskFactor acc = mapper.readValue(jsonld, MedicalRiskFactor.class);
        assertNotNull(acc);

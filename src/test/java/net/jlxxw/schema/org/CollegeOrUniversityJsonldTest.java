...
public class CollegeOrUniversityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CollegeOrUniversity.jsonld");
        assertNotNull(is, "CollegeOrUniversity.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CollegeOrUniversity acc = mapper.readValue(jsonld, CollegeOrUniversity.class);
        assertNotNull(acc);

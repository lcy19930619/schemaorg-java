...
public class CourthouseJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Courthouse.jsonld");
        assertNotNull(is, "Courthouse.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Courthouse acc = mapper.readValue(jsonld, Courthouse.class);
        assertNotNull(acc);

...
public class CourseInstanceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CourseInstance.jsonld");
        assertNotNull(is, "CourseInstance.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CourseInstance acc = mapper.readValue(jsonld, CourseInstance.class);
        assertNotNull(acc);

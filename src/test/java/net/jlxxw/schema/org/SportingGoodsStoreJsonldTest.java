...
public class SportingGoodsStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SportingGoodsStore.jsonld");
        assertNotNull(is, "SportingGoodsStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SportingGoodsStore acc = mapper.readValue(jsonld, SportingGoodsStore.class);
        assertNotNull(acc);

...
public class HardwareStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HardwareStore.jsonld");
        assertNotNull(is, "HardwareStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HardwareStore acc = mapper.readValue(jsonld, HardwareStore.class);
        assertNotNull(acc);

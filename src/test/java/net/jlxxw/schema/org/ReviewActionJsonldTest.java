...
public class ReviewActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReviewAction.jsonld");
        assertNotNull(is, "ReviewAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReviewAction acc = mapper.readValue(jsonld, ReviewAction.class);
        assertNotNull(acc);

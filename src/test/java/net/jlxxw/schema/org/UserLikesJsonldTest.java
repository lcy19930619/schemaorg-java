...
public class UserLikesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserLikes.jsonld");
        assertNotNull(is, "UserLikes.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserLikes acc = mapper.readValue(jsonld, UserLikes.class);
        assertNotNull(acc);

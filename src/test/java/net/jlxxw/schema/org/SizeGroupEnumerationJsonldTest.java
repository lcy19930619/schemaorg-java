...
public class SizeGroupEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SizeGroupEnumeration.jsonld");
        assertNotNull(is, "SizeGroupEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SizeGroupEnumeration acc = mapper.readValue(jsonld, SizeGroupEnumeration.class);
        assertNotNull(acc);

...
public class ParcelDeliveryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ParcelDelivery.jsonld");
        assertNotNull(is, "ParcelDelivery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ParcelDelivery acc = mapper.readValue(jsonld, ParcelDelivery.class);
        assertNotNull(acc);

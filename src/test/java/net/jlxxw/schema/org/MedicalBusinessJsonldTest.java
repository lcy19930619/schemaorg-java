...
public class MedicalBusinessJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalBusiness.jsonld");
        assertNotNull(is, "MedicalBusiness.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalBusiness acc = mapper.readValue(jsonld, MedicalBusiness.class);
        assertNotNull(acc);

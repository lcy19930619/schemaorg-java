...
public class AlignmentObjectJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AlignmentObject.jsonld");
        assertNotNull(is, "AlignmentObject.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AlignmentObject acc = mapper.readValue(jsonld, AlignmentObject.class);
        assertNotNull(acc);

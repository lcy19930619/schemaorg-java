...
public class PondJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Pond.jsonld");
        assertNotNull(is, "Pond.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Pond acc = mapper.readValue(jsonld, Pond.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Pond obj = new Pond();
        assertEquals("schema:Pond", obj.getId());

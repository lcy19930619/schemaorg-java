...
public class FinancialIncentiveJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FinancialIncentive.jsonld");
        assertNotNull(is, "FinancialIncentive.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FinancialIncentive acc = mapper.readValue(jsonld, FinancialIncentive.class);
        assertNotNull(acc);

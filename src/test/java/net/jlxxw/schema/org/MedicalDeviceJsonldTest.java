...
public class MedicalDeviceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalDevice.jsonld");
        assertNotNull(is, "MedicalDevice.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalDevice acc = mapper.readValue(jsonld, MedicalDevice.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalDevice obj = new MedicalDevice();
        assertEquals("schema:MedicalDevice", obj.getId());

...
public class BioChemEntityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BioChemEntity.jsonld");
        assertNotNull(is, "BioChemEntity.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BioChemEntity acc = mapper.readValue(jsonld, BioChemEntity.class);
        assertNotNull(acc);

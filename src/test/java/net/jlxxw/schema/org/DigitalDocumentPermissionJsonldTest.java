...
public class DigitalDocumentPermissionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DigitalDocumentPermission.jsonld");
        assertNotNull(is, "DigitalDocumentPermission.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DigitalDocumentPermission acc = mapper.readValue(jsonld, DigitalDocumentPermission.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        DigitalDocumentPermission obj = new DigitalDocumentPermission();
        assertEquals("schema:DigitalDocumentPermission", obj.getId());

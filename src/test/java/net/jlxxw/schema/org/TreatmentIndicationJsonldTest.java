...
public class TreatmentIndicationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TreatmentIndication.jsonld");
        assertNotNull(is, "TreatmentIndication.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TreatmentIndication acc = mapper.readValue(jsonld, TreatmentIndication.class);
        assertNotNull(acc);

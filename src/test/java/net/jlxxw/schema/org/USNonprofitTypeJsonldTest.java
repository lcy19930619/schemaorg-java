...
public class USNonprofitTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("USNonprofitType.jsonld");
        assertNotNull(is, "USNonprofitType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        USNonprofitType acc = mapper.readValue(jsonld, USNonprofitType.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        USNonprofitType obj = new USNonprofitType();
        assertEquals("schema:USNonprofitType", obj.getId());

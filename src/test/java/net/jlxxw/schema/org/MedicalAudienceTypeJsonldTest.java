...
public class MedicalAudienceTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalAudienceType.jsonld");
        assertNotNull(is, "MedicalAudienceType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalAudienceType acc = mapper.readValue(jsonld, MedicalAudienceType.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalAudienceType obj = new MedicalAudienceType();
        assertEquals("schema:MedicalAudienceType", obj.getId());

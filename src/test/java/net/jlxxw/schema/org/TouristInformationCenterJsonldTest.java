...
public class TouristInformationCenterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TouristInformationCenter.jsonld");
        assertNotNull(is, "TouristInformationCenter.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TouristInformationCenter acc = mapper.readValue(jsonld, TouristInformationCenter.class);
        assertNotNull(acc);

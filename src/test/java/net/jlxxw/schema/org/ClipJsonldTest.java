...
public class ClipJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Clip.jsonld");
        assertNotNull(is, "Clip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Clip acc = mapper.readValue(jsonld, Clip.class);
        assertNotNull(acc);

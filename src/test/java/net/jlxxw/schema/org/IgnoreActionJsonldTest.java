...
public class IgnoreActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("IgnoreAction.jsonld");
        assertNotNull(is, "IgnoreAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        IgnoreAction acc = mapper.readValue(jsonld, IgnoreAction.class);
        assertNotNull(acc);

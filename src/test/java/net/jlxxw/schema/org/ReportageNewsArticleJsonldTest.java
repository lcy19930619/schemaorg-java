...
public class ReportageNewsArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReportageNewsArticle.jsonld");
        assertNotNull(is, "ReportageNewsArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReportageNewsArticle acc = mapper.readValue(jsonld, ReportageNewsArticle.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ReportageNewsArticle obj = new ReportageNewsArticle();
        assertEquals("schema:ReportageNewsArticle", obj.getId());

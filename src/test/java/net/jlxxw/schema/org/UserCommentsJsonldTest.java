...
public class UserCommentsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserComments.jsonld");
        assertNotNull(is, "UserComments.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserComments acc = mapper.readValue(jsonld, UserComments.class);
        assertNotNull(acc);

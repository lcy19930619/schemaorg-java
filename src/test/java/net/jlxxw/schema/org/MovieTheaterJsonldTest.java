...
public class MovieTheaterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MovieTheater.jsonld");
        assertNotNull(is, "MovieTheater.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MovieTheater acc = mapper.readValue(jsonld, MovieTheater.class);
        assertNotNull(acc);

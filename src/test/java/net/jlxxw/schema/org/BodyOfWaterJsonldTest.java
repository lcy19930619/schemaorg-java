...
public class BodyOfWaterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BodyOfWater.jsonld");
        assertNotNull(is, "BodyOfWater.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BodyOfWater acc = mapper.readValue(jsonld, BodyOfWater.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        BodyOfWater obj = new BodyOfWater();
        assertEquals("schema:BodyOfWater", obj.getId());

...
public class TierBenefitEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TierBenefitEnumeration.jsonld");
        assertNotNull(is, "TierBenefitEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TierBenefitEnumeration acc = mapper.readValue(jsonld, TierBenefitEnumeration.class);
        assertNotNull(acc);

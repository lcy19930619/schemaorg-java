...
public class ClassJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Class.jsonld");
        assertNotNull(is, "Class.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Class acc = mapper.readValue(jsonld, Class.class);
        assertNotNull(acc);

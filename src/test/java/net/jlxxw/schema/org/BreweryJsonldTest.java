...
public class BreweryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Brewery.jsonld");
        assertNotNull(is, "Brewery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Brewery acc = mapper.readValue(jsonld, Brewery.class);
        assertNotNull(acc);

...
public class RadioStationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RadioStation.jsonld");
        assertNotNull(is, "RadioStation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RadioStation acc = mapper.readValue(jsonld, RadioStation.class);
        assertNotNull(acc);

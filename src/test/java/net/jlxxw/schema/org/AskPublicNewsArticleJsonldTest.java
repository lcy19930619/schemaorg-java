...
public class AskPublicNewsArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AskPublicNewsArticle.jsonld");
        assertNotNull(is, "AskPublicNewsArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AskPublicNewsArticle acc = mapper.readValue(jsonld, AskPublicNewsArticle.class);
        assertNotNull(acc);

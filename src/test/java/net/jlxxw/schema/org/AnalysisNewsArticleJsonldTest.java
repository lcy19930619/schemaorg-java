...
public class AnalysisNewsArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AnalysisNewsArticle.jsonld");
        assertNotNull(is, "AnalysisNewsArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AnalysisNewsArticle acc = mapper.readValue(jsonld, AnalysisNewsArticle.class);
        assertNotNull(acc);

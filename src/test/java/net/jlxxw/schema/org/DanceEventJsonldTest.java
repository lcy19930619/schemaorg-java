...
public class DanceEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DanceEvent.jsonld");
        assertNotNull(is, "DanceEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DanceEvent acc = mapper.readValue(jsonld, DanceEvent.class);
        assertNotNull(acc);

...
public class LodgingReservationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LodgingReservation.jsonld");
        assertNotNull(is, "LodgingReservation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LodgingReservation acc = mapper.readValue(jsonld, LodgingReservation.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        LodgingReservation obj = new LodgingReservation();
        assertEquals("schema:LodgingReservation", obj.getId());

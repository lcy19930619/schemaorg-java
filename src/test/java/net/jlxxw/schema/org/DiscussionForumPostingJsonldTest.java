...
public class DiscussionForumPostingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DiscussionForumPosting.jsonld");
        assertNotNull(is, "DiscussionForumPosting.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DiscussionForumPosting acc = mapper.readValue(jsonld, DiscussionForumPosting.class);
        assertNotNull(acc);

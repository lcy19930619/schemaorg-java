...
public class CancelActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CancelAction.jsonld");
        assertNotNull(is, "CancelAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CancelAction acc = mapper.readValue(jsonld, CancelAction.class);
        assertNotNull(acc);

...
public class MotorcycleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Motorcycle.jsonld");
        assertNotNull(is, "Motorcycle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Motorcycle acc = mapper.readValue(jsonld, Motorcycle.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ...
        assertEquals("schema:Motorcycle", obj.getId());

...
public class VideoObjectSnapshotJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VideoObjectSnapshot.jsonld");
        assertNotNull(is, "VideoObjectSnapshot.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VideoObjectSnapshot acc = mapper.readValue(jsonld, VideoObjectSnapshot.class);
        assertNotNull(acc);

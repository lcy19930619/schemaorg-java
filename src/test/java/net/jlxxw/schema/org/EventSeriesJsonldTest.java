...
public class EventSeriesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EventSeries.jsonld");
        assertNotNull(is, "EventSeries.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EventSeries acc = mapper.readValue(jsonld, EventSeries.class);
        assertNotNull(acc);

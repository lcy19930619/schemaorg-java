...
public class MolecularEntityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MolecularEntity.jsonld");
        assertNotNull(is, "MolecularEntity.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MolecularEntity acc = mapper.readValue(jsonld, MolecularEntity.class);
        assertNotNull(acc);

...
public class IndividualPhysicianJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("IndividualPhysician.jsonld");
        assertNotNull(is, "IndividualPhysician.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        IndividualPhysician acc = mapper.readValue(jsonld, IndividualPhysician.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        IndividualPhysician obj = new IndividualPhysician();
        assertEquals("schema:IndividualPhysician", obj.getId());

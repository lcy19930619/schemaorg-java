...
public class EmailMessageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EmailMessage.jsonld");
        assertNotNull(is, "EmailMessage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EmailMessage acc = mapper.readValue(jsonld, EmailMessage.class);
        assertNotNull(acc);

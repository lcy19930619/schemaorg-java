...
public class WantActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WantAction.jsonld");
        assertNotNull(is, "WantAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WantAction acc = mapper.readValue(jsonld, WantAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        WantAction obj = new WantAction();
        assertEquals("schema:WantAction", obj.getId());

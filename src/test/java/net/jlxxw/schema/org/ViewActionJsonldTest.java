...
public class ViewActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ViewAction.jsonld");
        assertNotNull(is, "ViewAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ViewAction acc = mapper.readValue(jsonld, ViewAction.class);
        assertNotNull(acc);

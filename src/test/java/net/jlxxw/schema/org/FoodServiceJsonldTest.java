...
public class FoodServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FoodService.jsonld");
        assertNotNull(is, "FoodService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FoodService acc = mapper.readValue(jsonld, FoodService.class);
        assertNotNull(acc);

...
public class IncentiveTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("IncentiveType.jsonld");
        assertNotNull(is, "IncentiveType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        IncentiveType acc = mapper.readValue(jsonld, IncentiveType.class);
        assertNotNull(acc);

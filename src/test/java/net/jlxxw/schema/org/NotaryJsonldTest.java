...
public class NotaryJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Notary.jsonld");
        assertNotNull(is, "Notary.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Notary acc = mapper.readValue(jsonld, Notary.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Notary obj = new Notary();
        assertEquals("schema:Notary", obj.getId());

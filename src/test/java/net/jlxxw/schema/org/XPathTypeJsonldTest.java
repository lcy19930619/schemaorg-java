...
public class XPathTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("XPathType.jsonld");
        assertNotNull(is, "XPathType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        XPathType acc = mapper.readValue(jsonld, XPathType.class);
        assertNotNull(acc);

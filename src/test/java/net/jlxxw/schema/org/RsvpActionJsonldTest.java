...
public class RsvpActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RsvpAction.jsonld");
        assertNotNull(is, "RsvpAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RsvpAction acc = mapper.readValue(jsonld, RsvpAction.class);
        assertNotNull(acc);

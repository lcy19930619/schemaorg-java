...
public class FoodEstablishmentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FoodEstablishment.jsonld");
        assertNotNull(is, "FoodEstablishment.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FoodEstablishment acc = mapper.readValue(jsonld, FoodEstablishment.class);
        assertNotNull(acc);

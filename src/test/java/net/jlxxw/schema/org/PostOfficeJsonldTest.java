...
public class PostOfficeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PostOffice.jsonld");
        assertNotNull(is, "PostOffice.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PostOffice acc = mapper.readValue(jsonld, PostOffice.class);
        assertNotNull(acc);

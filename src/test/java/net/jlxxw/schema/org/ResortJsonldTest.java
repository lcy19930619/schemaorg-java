...
public class ResortJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Resort.jsonld");
        assertNotNull(is, "Resort.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Resort acc = mapper.readValue(jsonld, Resort.class);
        assertNotNull(acc);

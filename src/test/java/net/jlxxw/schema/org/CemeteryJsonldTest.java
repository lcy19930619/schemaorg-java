...
public class CemeteryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Cemetery.jsonld");
        assertNotNull(is, "Cemetery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Cemetery acc = mapper.readValue(jsonld, Cemetery.class);
        assertNotNull(acc);

...
public class CampingPitchJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CampingPitch.jsonld");
        assertNotNull(is, "CampingPitch.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CampingPitch acc = mapper.readValue(jsonld, CampingPitch.class);
        assertNotNull(acc);

...
public class CodeJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Code.jsonld");
        assertNotNull(is, "Code.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Code acc = mapper.readValue(jsonld, Code.class);
        assertNotNull(acc);

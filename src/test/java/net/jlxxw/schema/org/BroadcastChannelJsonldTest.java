...
public class BroadcastChannelJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BroadcastChannel.jsonld");
        assertNotNull(is, "BroadcastChannel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BroadcastChannel acc = mapper.readValue(jsonld, BroadcastChannel.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        BroadcastChannel obj = new BroadcastChannel();
        assertEquals("schema:BroadcastChannel", obj.getId());

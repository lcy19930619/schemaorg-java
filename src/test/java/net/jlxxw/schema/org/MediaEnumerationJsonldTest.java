...
public class MediaEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MediaEnumeration.jsonld");
        assertNotNull(is, "MediaEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MediaEnumeration acc = mapper.readValue(jsonld, MediaEnumeration.class);
        assertNotNull(acc);

...
public class RegisterActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RegisterAction.jsonld");
        assertNotNull(is, "RegisterAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RegisterAction acc = mapper.readValue(jsonld, RegisterAction.class);
        assertNotNull(acc);

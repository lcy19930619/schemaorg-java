...
public class AttorneyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Attorney.jsonld");
        assertNotNull(is, "Attorney.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Attorney acc = mapper.readValue(jsonld, Attorney.class);
        assertNotNull(acc);

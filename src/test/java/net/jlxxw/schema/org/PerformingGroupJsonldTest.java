...
public class PerformingGroupJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PerformingGroup.jsonld");
        assertNotNull(is, "PerformingGroup.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PerformingGroup acc = mapper.readValue(jsonld, PerformingGroup.class);
        assertNotNull(acc);

...
public class MedicalStudyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalStudy.jsonld");
        assertNotNull(is, "MedicalStudy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalStudy acc = mapper.readValue(jsonld, MedicalStudy.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalStudy obj = new MedicalStudy();
        assertEquals("schema:MedicalStudy", obj.getId());

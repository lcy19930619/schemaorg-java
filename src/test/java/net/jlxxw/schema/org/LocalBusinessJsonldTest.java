...
public class LocalBusinessJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LocalBusiness.jsonld");
        assertNotNull(is, "LocalBusiness.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LocalBusiness acc = mapper.readValue(jsonld, LocalBusiness.class);
        assertNotNull(acc);

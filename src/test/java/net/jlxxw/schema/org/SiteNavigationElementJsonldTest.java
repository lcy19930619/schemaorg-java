...
public class SiteNavigationElementJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SiteNavigationElement.jsonld");
        assertNotNull(is, "SiteNavigationElement.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SiteNavigationElement acc = mapper.readValue(jsonld, SiteNavigationElement.class);
        assertNotNull(acc);

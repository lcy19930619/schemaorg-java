...
public class TrainTripJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TrainTrip.jsonld");
        assertNotNull(is, "TrainTrip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TrainTrip acc = mapper.readValue(jsonld, TrainTrip.class);
        assertNotNull(acc);

...
public class HyperTocEntryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HyperTocEntry.jsonld");
        assertNotNull(is, "HyperTocEntry.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HyperTocEntry acc = mapper.readValue(jsonld, HyperTocEntry.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        HyperTocEntry obj = new HyperTocEntry();
        assertEquals("schema:HyperTocEntry", obj.getId());

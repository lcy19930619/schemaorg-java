...
public class MovieRentalStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MovieRentalStore.jsonld");
        assertNotNull(is, "MovieRentalStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MovieRentalStore acc = mapper.readValue(jsonld, MovieRentalStore.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MovieRentalStore obj = new MovieRentalStore();
        assertEquals("schema:MovieRentalStore", obj.getId());

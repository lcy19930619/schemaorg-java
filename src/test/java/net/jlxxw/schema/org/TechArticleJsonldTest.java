...
public class TechArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TechArticle.jsonld");
        assertNotNull(is, "TechArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TechArticle acc = mapper.readValue(jsonld, TechArticle.class);
        assertNotNull(acc);

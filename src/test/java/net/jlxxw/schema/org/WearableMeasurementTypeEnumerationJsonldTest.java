...
public class WearableMeasurementTypeEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WearableMeasurementTypeEnumeration.jsonld");
        assertNotNull(is, "WearableMeasurementTypeEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WearableMeasurementTypeEnumeration acc = mapper.readValue(jsonld, WearableMeasurementTypeEnumeration.class);
        assertNotNull(acc);

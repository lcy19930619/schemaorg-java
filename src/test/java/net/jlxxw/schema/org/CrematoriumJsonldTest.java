...
public class CrematoriumJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Crematorium.jsonld");
        assertNotNull(is, "Crematorium.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Crematorium acc = mapper.readValue(jsonld, Crematorium.class);
        assertNotNull(acc);

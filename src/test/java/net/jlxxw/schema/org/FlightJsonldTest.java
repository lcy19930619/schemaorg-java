...
public class FlightJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Flight.jsonld");
        assertNotNull(is, "Flight.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Flight acc = mapper.readValue(jsonld, Flight.class);
        assertNotNull(acc);

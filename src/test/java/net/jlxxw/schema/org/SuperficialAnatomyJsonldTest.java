...
public class SuperficialAnatomyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SuperficialAnatomy.jsonld");
        assertNotNull(is, "SuperficialAnatomy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SuperficialAnatomy acc = mapper.readValue(jsonld, SuperficialAnatomy.class);
        assertNotNull(acc);

...
public class StatisticalPopulationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("StatisticalPopulation.jsonld");
        assertNotNull(is, "StatisticalPopulation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        StatisticalPopulation acc = mapper.readValue(jsonld, StatisticalPopulation.class);
        assertNotNull(acc);

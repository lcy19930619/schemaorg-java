...
public class VisualArtworkJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VisualArtwork.jsonld");
        assertNotNull(is, "VisualArtwork.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VisualArtwork acc = mapper.readValue(jsonld, VisualArtwork.class);
        assertNotNull(acc);

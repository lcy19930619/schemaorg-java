...
public class EducationalAudienceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EducationalAudience.jsonld");
        assertNotNull(is, "EducationalAudience.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EducationalAudience acc = mapper.readValue(jsonld, EducationalAudience.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        EducationalAudience obj = new EducationalAudience();
        assertEquals("schema:EducationalAudience", obj.getId());

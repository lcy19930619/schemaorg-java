...
public class AutomatedTellerJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AutomatedTeller.jsonld");
        assertNotNull(is, "AutomatedTeller.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AutomatedTeller acc = mapper.readValue(jsonld, AutomatedTeller.class);
        assertNotNull(acc);

...
public class DepositAccountJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DepositAccount.jsonld");
        assertNotNull(is, "DepositAccount.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DepositAccount acc = mapper.readValue(jsonld, DepositAccount.class);
        assertNotNull(acc);

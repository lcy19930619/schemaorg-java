...
public class InvoiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Invoice.jsonld");
        assertNotNull(is, "Invoice.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Invoice acc = mapper.readValue(jsonld, Invoice.class);
        assertNotNull(acc);

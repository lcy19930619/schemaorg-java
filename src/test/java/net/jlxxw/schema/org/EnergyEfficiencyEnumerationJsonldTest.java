...
public class EnergyEfficiencyEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EnergyEfficiencyEnumeration.jsonld");
        assertNotNull(is, "EnergyEfficiencyEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EnergyEfficiencyEnumeration acc = mapper.readValue(jsonld, EnergyEfficiencyEnumeration.class);
        assertNotNull(acc);

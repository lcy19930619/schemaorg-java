...
public class ServicePeriodJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ServicePeriod.jsonld");
        assertNotNull(is, "ServicePeriod.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ServicePeriod acc = mapper.readValue(jsonld, ServicePeriod.class);
        assertNotNull(acc);

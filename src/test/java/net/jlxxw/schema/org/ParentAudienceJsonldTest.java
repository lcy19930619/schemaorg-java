...
public class ParentAudienceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ParentAudience.jsonld");
        assertNotNull(is, "ParentAudience.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ParentAudience acc = mapper.readValue(jsonld, ParentAudience.class);
        assertNotNull(acc);

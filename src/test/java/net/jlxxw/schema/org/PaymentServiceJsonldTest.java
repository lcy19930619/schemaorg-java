...
public class PaymentServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PaymentService.jsonld");
        assertNotNull(is, "PaymentService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PaymentService acc = mapper.readValue(jsonld, PaymentService.class);
        assertNotNull(acc);

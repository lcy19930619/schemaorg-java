...
public class AccountingServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AccountingService.jsonld");
        assertNotNull(is, "AccountingService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AccountingService acc = mapper.readValue(jsonld, AccountingService.class);
        assertNotNull(acc);

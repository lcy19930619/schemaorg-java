...
public class SocialEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SocialEvent.jsonld");
        assertNotNull(is, "SocialEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SocialEvent acc = mapper.readValue(jsonld, SocialEvent.class);
        assertNotNull(acc);

...
public class DistilleryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Distillery.jsonld");
        assertNotNull(is, "Distillery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Distillery acc = mapper.readValue(jsonld, Distillery.class);
        assertNotNull(acc);

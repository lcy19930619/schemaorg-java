...
public class GeoCircleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GeoCircle.jsonld");
        assertNotNull(is, "GeoCircle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GeoCircle acc = mapper.readValue(jsonld, GeoCircle.class);
        assertNotNull(acc);

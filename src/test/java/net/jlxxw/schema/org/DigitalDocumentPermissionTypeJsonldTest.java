...
public class DigitalDocumentPermissionTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DigitalDocumentPermissionType.jsonld");
        assertNotNull(is, "DigitalDocumentPermissionType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DigitalDocumentPermissionType acc = mapper.readValue(jsonld, DigitalDocumentPermissionType.class);
        assertNotNull(acc);

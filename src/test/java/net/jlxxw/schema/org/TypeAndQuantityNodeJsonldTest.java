...
public class TypeAndQuantityNodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TypeAndQuantityNode.jsonld");
        assertNotNull(is, "TypeAndQuantityNode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TypeAndQuantityNode acc = mapper.readValue(jsonld, TypeAndQuantityNode.class);
        assertNotNull(acc);

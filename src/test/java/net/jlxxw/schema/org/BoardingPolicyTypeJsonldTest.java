...
public class BoardingPolicyTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BoardingPolicyType.jsonld");
        assertNotNull(is, "BoardingPolicyType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BoardingPolicyType acc = mapper.readValue(jsonld, BoardingPolicyType.class);
        assertNotNull(acc);

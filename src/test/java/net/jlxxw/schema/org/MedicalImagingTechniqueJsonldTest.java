...
public class MedicalImagingTechniqueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalImagingTechnique.jsonld");
        assertNotNull(is, "MedicalImagingTechnique.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalImagingTechnique acc = mapper.readValue(jsonld, MedicalImagingTechnique.class);
        assertNotNull(acc);

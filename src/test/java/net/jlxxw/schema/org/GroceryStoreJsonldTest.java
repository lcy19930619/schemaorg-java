...
public class GroceryStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GroceryStore.jsonld");
        assertNotNull(is, "GroceryStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GroceryStore acc = mapper.readValue(jsonld, GroceryStore.class);
        assertNotNull(acc);

...
public class MenuJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Menu.jsonld");
        assertNotNull(is, "Menu.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Menu acc = mapper.readValue(jsonld, Menu.class);
        assertNotNull(acc);

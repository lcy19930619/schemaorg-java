...
public class CategoryCodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CategoryCode.jsonld");
        assertNotNull(is, "CategoryCode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CategoryCode acc = mapper.readValue(jsonld, CategoryCode.class);
        assertNotNull(acc);

...
public class MedicalSignOrSymptomJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalSignOrSymptom.jsonld");
        assertNotNull(is, "MedicalSignOrSymptom.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalSignOrSymptom acc = mapper.readValue(jsonld, MedicalSignOrSymptom.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalSignOrSymptom obj = new MedicalSignOrSymptom();
        assertEquals("schema:MedicalSignOrSymptom", obj.getId());

...
public class StatisticalVariableJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("StatisticalVariable.jsonld");
        assertNotNull(is, "StatisticalVariable.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        StatisticalVariable acc = mapper.readValue(jsonld, StatisticalVariable.class);
        assertNotNull(acc);

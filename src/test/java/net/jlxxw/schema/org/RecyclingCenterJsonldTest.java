...
public class RecyclingCenterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RecyclingCenter.jsonld");
        assertNotNull(is, "RecyclingCenter.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RecyclingCenter acc = mapper.readValue(jsonld, RecyclingCenter.class);
        assertNotNull(acc);

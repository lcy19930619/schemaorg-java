...
public class SeasonJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Season.jsonld");
        assertNotNull(is, "Season.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Season acc = mapper.readValue(jsonld, Season.class);
        assertNotNull(acc);

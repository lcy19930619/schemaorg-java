...
public class DeliveryEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DeliveryEvent.jsonld");
        assertNotNull(is, "DeliveryEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DeliveryEvent acc = mapper.readValue(jsonld, DeliveryEvent.class);
        assertNotNull(acc);

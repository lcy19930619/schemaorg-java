...
public class RVParkJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RVPark.jsonld");
        assertNotNull(is, "RVPark.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RVPark acc = mapper.readValue(jsonld, RVPark.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        RVPark obj = new RVPark();
        assertEquals("schema:RVPark", obj.getId());

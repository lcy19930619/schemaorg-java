...
public class CorrectionCommentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CorrectionComment.jsonld");
        assertNotNull(is, "CorrectionComment.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CorrectionComment acc = mapper.readValue(jsonld, CorrectionComment.class);
        assertNotNull(acc);

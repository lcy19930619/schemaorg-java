...
public class DrugCostCategoryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DrugCostCategory.jsonld");
        assertNotNull(is, "DrugCostCategory.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DrugCostCategory acc = mapper.readValue(jsonld, DrugCostCategory.class);
        assertNotNull(acc);

...
public class MonetaryAmountDistributionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MonetaryAmountDistribution.jsonld");
        assertNotNull(is, "MonetaryAmountDistribution.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MonetaryAmountDistribution acc = mapper.readValue(jsonld, MonetaryAmountDistribution.class);
        assertNotNull(acc);

...
public class ThingJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Thing.jsonld");
        assertNotNull(is, "Thing.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Thing acc = mapper.readValue(jsonld, Thing.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Thing obj = new Thing();
        assertEquals("schema:Thing", obj.getId());

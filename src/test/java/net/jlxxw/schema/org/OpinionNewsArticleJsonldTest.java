...
public class OpinionNewsArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OpinionNewsArticle.jsonld");
        assertNotNull(is, "OpinionNewsArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OpinionNewsArticle acc = mapper.readValue(jsonld, OpinionNewsArticle.class);
        assertNotNull(acc);

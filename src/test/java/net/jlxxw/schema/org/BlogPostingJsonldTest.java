...
public class BlogPostingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BlogPosting.jsonld");
        assertNotNull(is, "BlogPosting.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BlogPosting acc = mapper.readValue(jsonld, BlogPosting.class);
        assertNotNull(acc);

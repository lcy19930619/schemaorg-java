...
public class HackathonJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Hackathon.jsonld");
        assertNotNull(is, "Hackathon.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Hackathon acc = mapper.readValue(jsonld, Hackathon.class);
        assertNotNull(acc);

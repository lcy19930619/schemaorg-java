...
public class FulfillmentTypeEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FulfillmentTypeEnumeration.jsonld");
        assertNotNull(is, "FulfillmentTypeEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FulfillmentTypeEnumeration acc = mapper.readValue(jsonld, FulfillmentTypeEnumeration.class);
        assertNotNull(acc);

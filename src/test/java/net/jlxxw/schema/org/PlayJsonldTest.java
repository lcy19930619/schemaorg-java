...
public class PlayJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Play.jsonld");
        assertNotNull(is, "Play.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Play acc = mapper.readValue(jsonld, Play.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Play obj = new Play();
        assertEquals("schema:Play", obj.getId());

...
public class PlumberJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Plumber.jsonld");
        assertNotNull(is, "Plumber.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Plumber acc = mapper.readValue(jsonld, Plumber.class);
        assertNotNull(acc);

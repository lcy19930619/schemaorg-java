...
public class ReserveActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReserveAction.jsonld");
        assertNotNull(is, "ReserveAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReserveAction acc = mapper.readValue(jsonld, ReserveAction.class);
        assertNotNull(acc);

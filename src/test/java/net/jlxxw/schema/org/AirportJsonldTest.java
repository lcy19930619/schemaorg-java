...
public class AirportJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Airport.jsonld");
        assertNotNull(is, "Airport.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Airport acc = mapper.readValue(jsonld, Airport.class);
        assertNotNull(acc);

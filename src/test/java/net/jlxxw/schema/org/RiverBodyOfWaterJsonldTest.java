...
public class RiverBodyOfWaterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RiverBodyOfWater.jsonld");
        assertNotNull(is, "RiverBodyOfWater.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RiverBodyOfWater acc = mapper.readValue(jsonld, RiverBodyOfWater.class);
        assertNotNull(acc);

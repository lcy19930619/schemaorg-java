...
public class DiagnosticLabJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DiagnosticLab.jsonld");
        assertNotNull(is, "DiagnosticLab.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DiagnosticLab acc = mapper.readValue(jsonld, DiagnosticLab.class);
        assertNotNull(acc);

...
public class BusinessFunctionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BusinessFunction.jsonld");
        assertNotNull(is, "BusinessFunction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BusinessFunction acc = mapper.readValue(jsonld, BusinessFunction.class);
        assertNotNull(acc);

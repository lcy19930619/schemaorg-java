...
public class LegalServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LegalService.jsonld");
        assertNotNull(is, "LegalService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LegalService acc = mapper.readValue(jsonld, LegalService.class);
        assertNotNull(acc);

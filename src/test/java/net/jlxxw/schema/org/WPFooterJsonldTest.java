...
public class WPFooterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WPFooter.jsonld");
        assertNotNull(is, "WPFooter.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WPFooter acc = mapper.readValue(jsonld, WPFooter.class);
        assertNotNull(acc);

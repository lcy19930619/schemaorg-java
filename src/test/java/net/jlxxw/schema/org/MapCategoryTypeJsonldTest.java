...
public class MapCategoryTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MapCategoryType.jsonld");
        assertNotNull(is, "MapCategoryType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MapCategoryType acc = mapper.readValue(jsonld, MapCategoryType.class);
        assertNotNull(acc);

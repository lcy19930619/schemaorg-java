...
public class PlaceOfWorshipJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PlaceOfWorship.jsonld");
        assertNotNull(is, "PlaceOfWorship.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PlaceOfWorship acc = mapper.readValue(jsonld, PlaceOfWorship.class);
        assertNotNull(acc);

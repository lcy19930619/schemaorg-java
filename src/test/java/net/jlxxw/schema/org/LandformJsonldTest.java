...
public class LandformJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Landform.jsonld");
        assertNotNull(is, "Landform.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Landform acc = mapper.readValue(jsonld, Landform.class);
        assertNotNull(acc);

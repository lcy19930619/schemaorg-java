...
public class AutoDealerJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AutoDealer.jsonld");
        assertNotNull(is, "AutoDealer.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AutoDealer acc = mapper.readValue(jsonld, AutoDealer.class);
        assertNotNull(acc);

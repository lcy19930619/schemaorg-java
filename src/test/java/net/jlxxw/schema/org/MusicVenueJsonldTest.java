...
public class MusicVenueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicVenue.jsonld");
        assertNotNull(is, "MusicVenue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicVenue acc = mapper.readValue(jsonld, MusicVenue.class);
        assertNotNull(acc);

...
public class PronounceableTextJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PronounceableText.jsonld");
        assertNotNull(is, "PronounceableText.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PronounceableText acc = mapper.readValue(jsonld, PronounceableText.class);
        assertNotNull(acc);

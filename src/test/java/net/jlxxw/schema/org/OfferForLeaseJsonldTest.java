...
public class OfferForLeaseJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OfferForLease.jsonld");
        assertNotNull(is, "OfferForLease.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OfferForLease acc = mapper.readValue(jsonld, OfferForLease.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        OfferForLease obj = new OfferForLease();
        assertEquals("schema:OfferForLease", obj.getId());

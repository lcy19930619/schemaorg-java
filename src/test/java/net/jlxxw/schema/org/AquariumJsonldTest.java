...
public class AquariumJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Aquarium.jsonld");
        assertNotNull(is, "Aquarium.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Aquarium acc = mapper.readValue(jsonld, Aquarium.class);
        assertNotNull(acc);

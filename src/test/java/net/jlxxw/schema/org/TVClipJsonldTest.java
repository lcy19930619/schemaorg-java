...
public class TVClipJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TVClip.jsonld");
        assertNotNull(is, "TVClip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TVClip acc = mapper.readValue(jsonld, TVClip.class);
        assertNotNull(acc);

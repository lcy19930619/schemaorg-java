...
public class ProductCollectionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ProductCollection.jsonld");
        assertNotNull(is, "ProductCollection.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ProductCollection acc = mapper.readValue(jsonld, ProductCollection.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ProductCollection obj = new ProductCollection();
        assertEquals("schema:ProductCollection", obj.getId());

...
public class WPSideBarJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WPSideBar.jsonld");
        assertNotNull(is, "WPSideBar.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WPSideBar acc = mapper.readValue(jsonld, WPSideBar.class);
        assertNotNull(acc);

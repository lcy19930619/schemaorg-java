...
public class MuseumJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Museum.jsonld");
        assertNotNull(is, "Museum.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Museum acc = mapper.readValue(jsonld, Museum.class);
        assertNotNull(acc);

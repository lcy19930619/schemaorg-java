...
public class MedicalTherapyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalTherapy.jsonld");
        assertNotNull(is, "MedicalTherapy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalTherapy acc = mapper.readValue(jsonld, MedicalTherapy.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalTherapy obj = new MedicalTherapy();
        assertEquals("schema:MedicalTherapy", obj.getId());

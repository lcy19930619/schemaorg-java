...
public class DayOfWeekJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DayOfWeek.jsonld");
        assertNotNull(is, "DayOfWeek.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DayOfWeek acc = mapper.readValue(jsonld, DayOfWeek.class);
        assertNotNull(acc);

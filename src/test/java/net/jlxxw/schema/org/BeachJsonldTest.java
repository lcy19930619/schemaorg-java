...
public class BeachJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Beach.jsonld");
        assertNotNull(is, "Beach.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Beach acc = mapper.readValue(jsonld, Beach.class);
        assertNotNull(acc);

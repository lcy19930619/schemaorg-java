...
public class GovernmentBuildingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GovernmentBuilding.jsonld");
        assertNotNull(is, "GovernmentBuilding.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GovernmentBuilding acc = mapper.readValue(jsonld, GovernmentBuilding.class);
        assertNotNull(acc);

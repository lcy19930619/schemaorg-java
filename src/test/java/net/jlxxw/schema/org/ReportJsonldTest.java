...
public class ReportJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Report.jsonld");
        assertNotNull(is, "Report.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Report acc = mapper.readValue(jsonld, Report.class);
        assertNotNull(acc);

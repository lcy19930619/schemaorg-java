...
public class MemberProgramTierJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MemberProgramTier.jsonld");
        assertNotNull(is, "MemberProgramTier.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MemberProgramTier acc = mapper.readValue(jsonld, MemberProgramTier.class);
        assertNotNull(acc);

...
public class QuantitativeValueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("QuantitativeValue.jsonld");
        assertNotNull(is, "QuantitativeValue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        QuantitativeValue acc = mapper.readValue(jsonld, QuantitativeValue.class);
        assertNotNull(acc);

...
public class MedicalTrialDesignJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalTrialDesign.jsonld");
        assertNotNull(is, "MedicalTrialDesign.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalTrialDesign acc = mapper.readValue(jsonld, MedicalTrialDesign.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalTrialDesign obj = new MedicalTrialDesign();
        assertEquals("schema:MedicalTrialDesign", obj.getId());

...
public class ChooseActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ChooseAction.jsonld");
        assertNotNull(is, "ChooseAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ChooseAction acc = mapper.readValue(jsonld, ChooseAction.class);
        assertNotNull(acc);

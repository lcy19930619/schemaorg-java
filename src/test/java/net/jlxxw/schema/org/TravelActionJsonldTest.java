...
public class TravelActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TravelAction.jsonld");
        assertNotNull(is, "TravelAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TravelAction acc = mapper.readValue(jsonld, TravelAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        TravelAction obj = new TravelAction();
        assertEquals("schema:TravelAction", obj.getId());

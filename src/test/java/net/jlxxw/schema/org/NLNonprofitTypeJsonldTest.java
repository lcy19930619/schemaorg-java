...
public class NLNonprofitTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("NLNonprofitType.jsonld");
        assertNotNull(is, "NLNonprofitType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        NLNonprofitType acc = mapper.readValue(jsonld, NLNonprofitType.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        NLNonprofitType obj = new NLNonprofitType();
        assertEquals("schema:NLNonprofitType", obj.getId());

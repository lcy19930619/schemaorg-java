...
public class NewspaperJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Newspaper.jsonld");
        assertNotNull(is, "Newspaper.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Newspaper acc = mapper.readValue(jsonld, Newspaper.class);
        assertNotNull(acc);

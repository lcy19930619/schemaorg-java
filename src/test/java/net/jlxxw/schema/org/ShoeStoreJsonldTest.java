...
public class ShoeStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ShoeStore.jsonld");
        assertNotNull(is, "ShoeStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ShoeStore acc = mapper.readValue(jsonld, ShoeStore.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ShoeStore obj = new ShoeStore();
        assertEquals("schema:ShoeStore", obj.getId());

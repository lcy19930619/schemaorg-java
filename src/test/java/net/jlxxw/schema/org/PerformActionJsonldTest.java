...
public class PerformActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PerformAction.jsonld");
        assertNotNull(is, "PerformAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PerformAction acc = mapper.readValue(jsonld, PerformAction.class);
        assertNotNull(acc);

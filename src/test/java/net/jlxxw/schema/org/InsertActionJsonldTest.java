...
public class InsertActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("InsertAction.jsonld");
        assertNotNull(is, "InsertAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        InsertAction acc = mapper.readValue(jsonld, InsertAction.class);
        assertNotNull(acc);

...
public class UserInteractionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserInteraction.jsonld");
        assertNotNull(is, "UserInteraction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserInteraction acc = mapper.readValue(jsonld, UserInteraction.class);
        assertNotNull(acc);

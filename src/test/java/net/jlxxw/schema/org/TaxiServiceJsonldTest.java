...
public class TaxiServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TaxiService.jsonld");
        assertNotNull(is, "TaxiService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TaxiService acc = mapper.readValue(jsonld, TaxiService.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        TaxiService obj = new TaxiService();
        assertEquals("schema:TaxiService", obj.getId());

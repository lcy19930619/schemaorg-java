...
public class MedicalIndicationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalIndication.jsonld");
        assertNotNull(is, "MedicalIndication.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalIndication acc = mapper.readValue(jsonld, MedicalIndication.class);
        assertNotNull(acc);

...
public class BroadcastFrequencySpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BroadcastFrequencySpecification.jsonld");
        assertNotNull(is, "BroadcastFrequencySpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BroadcastFrequencySpecification acc = mapper.readValue(jsonld, BroadcastFrequencySpecification.class);
        assertNotNull(acc);

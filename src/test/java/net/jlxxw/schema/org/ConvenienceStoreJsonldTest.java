...
public class ConvenienceStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ConvenienceStore.jsonld");
        assertNotNull(is, "ConvenienceStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ConvenienceStore acc = mapper.readValue(jsonld, ConvenienceStore.class);
        assertNotNull(acc);

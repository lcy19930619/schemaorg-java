...
public class TVEpisodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TVEpisode.jsonld");
        assertNotNull(is, "TVEpisode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TVEpisode acc = mapper.readValue(jsonld, TVEpisode.class);
        assertNotNull(acc);

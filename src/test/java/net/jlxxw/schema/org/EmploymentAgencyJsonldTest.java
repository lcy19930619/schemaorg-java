...
public class EmploymentAgencyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EmploymentAgency.jsonld");
        assertNotNull(is, "EmploymentAgency.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EmploymentAgency acc = mapper.readValue(jsonld, EmploymentAgency.class);
        assertNotNull(acc);

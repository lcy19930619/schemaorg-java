...
public class ElectricianJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Electrician.jsonld");
        assertNotNull(is, "Electrician.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Electrician acc = mapper.readValue(jsonld, Electrician.class);
        assertNotNull(acc);

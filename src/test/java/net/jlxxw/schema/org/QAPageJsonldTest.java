...
public class QAPageJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("QAPage.jsonld");
        assertNotNull(is, "QAPage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        QAPage acc = mapper.readValue(jsonld, QAPage.class);
        assertNotNull(acc);

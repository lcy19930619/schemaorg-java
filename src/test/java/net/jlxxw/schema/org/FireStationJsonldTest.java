...
public class FireStationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FireStation.jsonld");
        assertNotNull(is, "FireStation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FireStation acc = mapper.readValue(jsonld, FireStation.class);
        assertNotNull(acc);

...
public class BankAccountJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BankAccount.jsonld");
        assertNotNull(is, "BankAccount.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BankAccount acc = mapper.readValue(jsonld, BankAccount.class);
        assertNotNull(acc);

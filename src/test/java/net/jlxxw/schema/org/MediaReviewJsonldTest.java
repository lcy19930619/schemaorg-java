...
public class MediaReviewJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MediaReview.jsonld");
        assertNotNull(is, "MediaReview.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MediaReview acc = mapper.readValue(jsonld, MediaReview.class);
        assertNotNull(acc);

...
public class UserPlaysJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserPlays.jsonld");
        assertNotNull(is, "UserPlays.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserPlays acc = mapper.readValue(jsonld, UserPlays.class);
        assertNotNull(acc);

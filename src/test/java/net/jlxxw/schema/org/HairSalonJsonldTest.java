...
public class HairSalonJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HairSalon.jsonld");
        assertNotNull(is, "HairSalon.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HairSalon acc = mapper.readValue(jsonld, HairSalon.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        HairSalon obj = new HairSalon();
        assertEquals("schema:HairSalon", obj.getId());

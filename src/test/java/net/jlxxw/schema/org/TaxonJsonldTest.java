...
public class TaxonJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Taxon.jsonld");
        assertNotNull(is, "Taxon.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Taxon acc = mapper.readValue(jsonld, Taxon.class);
        assertNotNull(acc);

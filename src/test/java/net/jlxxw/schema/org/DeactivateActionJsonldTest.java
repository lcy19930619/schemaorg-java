...
public class DeactivateActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DeactivateAction.jsonld");
        assertNotNull(is, "DeactivateAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DeactivateAction acc = mapper.readValue(jsonld, DeactivateAction.class);
        assertNotNull(acc);

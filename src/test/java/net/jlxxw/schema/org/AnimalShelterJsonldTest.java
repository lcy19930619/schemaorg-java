...
public class AnimalShelterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AnimalShelter.jsonld");
        assertNotNull(is, "AnimalShelter.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AnimalShelter acc = mapper.readValue(jsonld, AnimalShelter.class);
        assertNotNull(acc);

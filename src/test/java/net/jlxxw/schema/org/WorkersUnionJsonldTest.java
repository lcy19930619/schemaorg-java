...
public class WorkersUnionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WorkersUnion.jsonld");
        assertNotNull(is, "WorkersUnion.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WorkersUnion acc = mapper.readValue(jsonld, WorkersUnion.class);
        assertNotNull(acc);

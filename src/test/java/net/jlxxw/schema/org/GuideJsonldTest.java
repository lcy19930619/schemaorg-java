...
public class GuideJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Guide.jsonld");
        assertNotNull(is, "Guide.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Guide acc = mapper.readValue(jsonld, Guide.class);
        assertNotNull(acc);

...
public class RentalCarReservationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RentalCarReservation.jsonld");
        assertNotNull(is, "RentalCarReservation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RentalCarReservation acc = mapper.readValue(jsonld, RentalCarReservation.class);
        assertNotNull(acc);

...
public class OrganizationRoleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OrganizationRole.jsonld");
        assertNotNull(is, "OrganizationRole.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OrganizationRole acc = mapper.readValue(jsonld, OrganizationRole.class);
        assertNotNull(acc);

...
public class ProfessionalServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ProfessionalService.jsonld");
        assertNotNull(is, "ProfessionalService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ProfessionalService acc = mapper.readValue(jsonld, ProfessionalService.class);
        assertNotNull(acc);

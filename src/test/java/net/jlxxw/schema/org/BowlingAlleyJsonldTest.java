...
public class BowlingAlleyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BowlingAlley.jsonld");
        assertNotNull(is, "BowlingAlley.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BowlingAlley acc = mapper.readValue(jsonld, BowlingAlley.class);
        assertNotNull(acc);

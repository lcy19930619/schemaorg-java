...
public class EventVenueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EventVenue.jsonld");
        assertNotNull(is, "EventVenue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EventVenue acc = mapper.readValue(jsonld, EventVenue.class);
        assertNotNull(acc);

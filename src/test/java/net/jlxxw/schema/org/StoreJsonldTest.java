...
public class StoreJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Store.jsonld");
        assertNotNull(is, "Store.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Store acc = mapper.readValue(jsonld, Store.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Store obj = new Store();
        assertEquals("schema:Store", obj.getId());

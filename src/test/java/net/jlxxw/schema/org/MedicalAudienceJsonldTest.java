...
public class MedicalAudienceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalAudience.jsonld");
        assertNotNull(is, "MedicalAudience.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalAudience acc = mapper.readValue(jsonld, MedicalAudience.class);
        assertNotNull(acc);

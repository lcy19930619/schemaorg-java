...
public class ChurchJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Church.jsonld");
        assertNotNull(is, "Church.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Church acc = mapper.readValue(jsonld, Church.class);
        assertNotNull(acc);

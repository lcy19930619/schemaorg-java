...
public class HowToToolJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HowToTool.jsonld");
        assertNotNull(is, "HowToTool.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HowToTool acc = mapper.readValue(jsonld, HowToTool.class);
        assertNotNull(acc);

...
public class CreativeWorkSeriesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CreativeWorkSeries.jsonld");
        assertNotNull(is, "CreativeWorkSeries.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CreativeWorkSeries acc = mapper.readValue(jsonld, CreativeWorkSeries.class);
        assertNotNull(acc);

...
public class GameAvailabilityEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GameAvailabilityEnumeration.jsonld");
        assertNotNull(is, "GameAvailabilityEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GameAvailabilityEnumeration acc = mapper.readValue(jsonld, GameAvailabilityEnumeration.class);
        assertNotNull(acc);

...
public class HowToSupplyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HowToSupply.jsonld");
        assertNotNull(is, "HowToSupply.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HowToSupply acc = mapper.readValue(jsonld, HowToSupply.class);
        assertNotNull(acc);

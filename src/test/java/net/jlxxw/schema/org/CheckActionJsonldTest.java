...
public class CheckActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CheckAction.jsonld");
        assertNotNull(is, "CheckAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CheckAction acc = mapper.readValue(jsonld, CheckAction.class);
        assertNotNull(acc);

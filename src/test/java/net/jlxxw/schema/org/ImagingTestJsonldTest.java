...
public class ImagingTestJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ImagingTest.jsonld");
        assertNotNull(is, "ImagingTest.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ImagingTest acc = mapper.readValue(jsonld, ImagingTest.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ImagingTest obj = new ImagingTest();
        assertEquals("schema:ImagingTest", obj.getId());

...
public class BuyActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BuyAction.jsonld");
        assertNotNull(is, "BuyAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BuyAction acc = mapper.readValue(jsonld, BuyAction.class);
        assertNotNull(acc);

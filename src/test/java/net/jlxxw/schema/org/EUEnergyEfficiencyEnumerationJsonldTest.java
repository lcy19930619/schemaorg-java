...
public class EUEnergyEfficiencyEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EUEnergyEfficiencyEnumeration.jsonld");
        assertNotNull(is, "EUEnergyEfficiencyEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EUEnergyEfficiencyEnumeration acc = mapper.readValue(jsonld, EUEnergyEfficiencyEnumeration.class);
        assertNotNull(acc);

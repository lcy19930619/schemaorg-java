...
public class BookJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Book.jsonld");
        assertNotNull(is, "Book.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Book acc = mapper.readValue(jsonld, Book.class);
        assertNotNull(acc);

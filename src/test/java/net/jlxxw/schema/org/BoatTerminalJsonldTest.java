...
public class BoatTerminalJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BoatTerminal.jsonld");
        assertNotNull(is, "BoatTerminal.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BoatTerminal acc = mapper.readValue(jsonld, BoatTerminal.class);
        assertNotNull(acc);

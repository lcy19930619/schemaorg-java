...
public class PodcastEpisodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PodcastEpisode.jsonld");
        assertNotNull(is, "PodcastEpisode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PodcastEpisode acc = mapper.readValue(jsonld, PodcastEpisode.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        PodcastEpisode obj = new PodcastEpisode();
        assertEquals("schema:PodcastEpisode", obj.getId());

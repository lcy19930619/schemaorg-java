...
public class GameServerStatusJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GameServerStatus.jsonld");
        assertNotNull(is, "GameServerStatus.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GameServerStatus acc = mapper.readValue(jsonld, GameServerStatus.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        GameServerStatus obj = new GameServerStatus();
        assertEquals("schema:GameServerStatus", obj.getId());

...
public class VesselJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Vessel.jsonld");
        assertNotNull(is, "Vessel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Vessel acc = mapper.readValue(jsonld, Vessel.class);
        assertNotNull(acc);

...
public class OnlineStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OnlineStore.jsonld");
        assertNotNull(is, "OnlineStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OnlineStore acc = mapper.readValue(jsonld, OnlineStore.class);
        assertNotNull(acc);

...
public class ShippingDeliveryTimeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ShippingDeliveryTime.jsonld");
        assertNotNull(is, "ShippingDeliveryTime.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ShippingDeliveryTime acc = mapper.readValue(jsonld, ShippingDeliveryTime.class);
        assertNotNull(acc);

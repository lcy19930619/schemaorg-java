...
public class MusicReleaseJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicRelease.jsonld");
        assertNotNull(is, "MusicRelease.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicRelease acc = mapper.readValue(jsonld, MusicRelease.class);
        assertNotNull(acc);

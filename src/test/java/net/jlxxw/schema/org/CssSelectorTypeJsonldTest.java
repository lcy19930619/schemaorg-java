...
public class CssSelectorTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CssSelectorType.jsonld");
        assertNotNull(is, "CssSelectorType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CssSelectorType acc = mapper.readValue(jsonld, CssSelectorType.class);
        assertNotNull(acc);

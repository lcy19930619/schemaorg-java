...
public class LakeBodyOfWaterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LakeBodyOfWater.jsonld");
        assertNotNull(is, "LakeBodyOfWater.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LakeBodyOfWater acc = mapper.readValue(jsonld, LakeBodyOfWater.class);
        assertNotNull(acc);

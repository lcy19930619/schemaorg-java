...
public class ShoppingCenterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ShoppingCenter.jsonld");
        assertNotNull(is, "ShoppingCenter.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ShoppingCenter acc = mapper.readValue(jsonld, ShoppingCenter.class);
        assertNotNull(acc);

...
public class MeetingRoomJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MeetingRoom.jsonld");
        assertNotNull(is, "MeetingRoom.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MeetingRoom acc = mapper.readValue(jsonld, MeetingRoom.class);
        assertNotNull(acc);

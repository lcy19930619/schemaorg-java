...
public class RestrictedDietJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RestrictedDiet.jsonld");
        assertNotNull(is, "RestrictedDiet.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RestrictedDiet acc = mapper.readValue(jsonld, RestrictedDiet.class);
        assertNotNull(acc);

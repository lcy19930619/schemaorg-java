...
public class PreOrderActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PreOrderAction.jsonld");
        assertNotNull(is, "PreOrderAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PreOrderAction acc = mapper.readValue(jsonld, PreOrderAction.class);
        assertNotNull(acc);

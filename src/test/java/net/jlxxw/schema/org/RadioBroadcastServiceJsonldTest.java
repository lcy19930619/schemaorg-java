...
public class RadioBroadcastServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RadioBroadcastService.jsonld");
        assertNotNull(is, "RadioBroadcastService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RadioBroadcastService acc = mapper.readValue(jsonld, RadioBroadcastService.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        RadioBroadcastService obj = new RadioBroadcastService();
        assertEquals("schema:RadioBroadcastService", obj.getId());

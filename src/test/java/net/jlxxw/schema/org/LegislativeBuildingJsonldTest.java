...
public class LegislativeBuildingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LegislativeBuilding.jsonld");
        assertNotNull(is, "LegislativeBuilding.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LegislativeBuilding acc = mapper.readValue(jsonld, LegislativeBuilding.class);
        assertNotNull(acc);

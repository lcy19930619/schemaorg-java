...
public class EducationalOrganizationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EducationalOrganization.jsonld");
        assertNotNull(is, "EducationalOrganization.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EducationalOrganization acc = mapper.readValue(jsonld, EducationalOrganization.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        EducationalOrganization obj = new EducationalOrganization();
        assertEquals("schema:EducationalOrganization", obj.getId());

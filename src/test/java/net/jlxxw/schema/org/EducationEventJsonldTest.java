...
public class EducationEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EducationEvent.jsonld");
        assertNotNull(is, "EducationEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EducationEvent acc = mapper.readValue(jsonld, EducationEvent.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        EducationEvent obj = new EducationEvent();
        assertEquals("schema:EducationEvent", obj.getId());

...
public class PhysicalExamJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PhysicalExam.jsonld");
        assertNotNull(is, "PhysicalExam.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PhysicalExam acc = mapper.readValue(jsonld, PhysicalExam.class);
        assertNotNull(acc);

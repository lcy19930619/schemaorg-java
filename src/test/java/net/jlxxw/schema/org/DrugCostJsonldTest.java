...
public class DrugCostJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DrugCost.jsonld");
        assertNotNull(is, "DrugCost.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DrugCost acc = mapper.readValue(jsonld, DrugCost.class);
        assertNotNull(acc);

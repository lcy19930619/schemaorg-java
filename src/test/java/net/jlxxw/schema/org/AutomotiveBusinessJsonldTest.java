...
public class AutomotiveBusinessJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AutomotiveBusiness.jsonld");
        assertNotNull(is, "AutomotiveBusiness.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AutomotiveBusiness acc = mapper.readValue(jsonld, AutomotiveBusiness.class);
        assertNotNull(acc);

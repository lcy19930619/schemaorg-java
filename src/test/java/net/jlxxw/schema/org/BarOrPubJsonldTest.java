...
public class BarOrPubJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BarOrPub.jsonld");
        assertNotNull(is, "BarOrPub.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BarOrPub acc = mapper.readValue(jsonld, BarOrPub.class);
        assertNotNull(acc);

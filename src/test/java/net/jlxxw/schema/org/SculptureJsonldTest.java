...
public class SculptureJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Sculpture.jsonld");
        assertNotNull(is, "Sculpture.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Sculpture acc = mapper.readValue(jsonld, Sculpture.class);
        assertNotNull(acc);

...
public class TVSeasonJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TVSeason.jsonld");
        assertNotNull(is, "TVSeason.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TVSeason acc = mapper.readValue(jsonld, TVSeason.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        TVSeason obj = new TVSeason();
        assertEquals("schema:TVSeason", obj.getId());

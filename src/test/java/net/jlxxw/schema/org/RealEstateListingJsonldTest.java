...
public class RealEstateListingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RealEstateListing.jsonld");
        assertNotNull(is, "RealEstateListing.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RealEstateListing acc = mapper.readValue(jsonld, RealEstateListing.class);
        assertNotNull(acc);

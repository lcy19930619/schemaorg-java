...
public class ReplyActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReplyAction.jsonld");
        assertNotNull(is, "ReplyAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReplyAction acc = mapper.readValue(jsonld, ReplyAction.class);
        assertNotNull(acc);

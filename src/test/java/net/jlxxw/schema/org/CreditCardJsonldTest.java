...
public class CreditCardJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CreditCard.jsonld");
        assertNotNull(is, "CreditCard.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CreditCard acc = mapper.readValue(jsonld, CreditCard.class);
        assertNotNull(acc);

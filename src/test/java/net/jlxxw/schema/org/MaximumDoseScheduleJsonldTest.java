...
public class MaximumDoseScheduleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MaximumDoseSchedule.jsonld");
        assertNotNull(is, "MaximumDoseSchedule.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MaximumDoseSchedule acc = mapper.readValue(jsonld, MaximumDoseSchedule.class);
        assertNotNull(acc);

...
public class PodcastSeasonJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PodcastSeason.jsonld");
        assertNotNull(is, "PodcastSeason.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PodcastSeason acc = mapper.readValue(jsonld, PodcastSeason.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        PodcastSeason obj = new PodcastSeason();
        assertEquals("schema:PodcastSeason", obj.getId());

...
public class CommunicateActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CommunicateAction.jsonld");
        assertNotNull(is, "CommunicateAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CommunicateAction acc = mapper.readValue(jsonld, CommunicateAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        CommunicateAction obj = new CommunicateAction();
        assertEquals("schema:CommunicateAction", obj.getId());

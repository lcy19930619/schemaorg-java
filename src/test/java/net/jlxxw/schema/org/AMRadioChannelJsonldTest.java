...
public class AMRadioChannelJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AMRadioChannel.jsonld");
        assertNotNull(is, "AMRadioChannel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AMRadioChannel acc = mapper.readValue(jsonld, AMRadioChannel.class);
        assertNotNull(acc);

...
public class TicketJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Ticket.jsonld");
        assertNotNull(is, "Ticket.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Ticket acc = mapper.readValue(jsonld, Ticket.class);
        assertNotNull(acc);

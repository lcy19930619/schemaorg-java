...
public class NGOJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("NGO.jsonld");
        assertNotNull(is, "NGO.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        NGO acc = mapper.readValue(jsonld, NGO.class);
        assertNotNull(acc);

...
public class ComicStoryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ComicStory.jsonld");
        assertNotNull(is, "ComicStory.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ComicStory acc = mapper.readValue(jsonld, ComicStory.class);
        assertNotNull(acc);

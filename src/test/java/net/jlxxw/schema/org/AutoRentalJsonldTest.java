...
public class AutoRentalJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AutoRental.jsonld");
        assertNotNull(is, "AutoRental.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AutoRental acc = mapper.readValue(jsonld, AutoRental.class);
        assertNotNull(acc);

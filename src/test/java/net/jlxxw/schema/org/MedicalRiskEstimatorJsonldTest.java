...
public class MedicalRiskEstimatorJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalRiskEstimator.jsonld");
        assertNotNull(is, "MedicalRiskEstimator.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalRiskEstimator acc = mapper.readValue(jsonld, MedicalRiskEstimator.class);
        assertNotNull(acc);

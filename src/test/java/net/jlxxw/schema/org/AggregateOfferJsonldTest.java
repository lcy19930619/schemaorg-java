...
public class AggregateOfferJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AggregateOffer.jsonld");
        assertNotNull(is, "AggregateOffer.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AggregateOffer acc = mapper.readValue(jsonld, AggregateOffer.class);
        assertNotNull(acc);

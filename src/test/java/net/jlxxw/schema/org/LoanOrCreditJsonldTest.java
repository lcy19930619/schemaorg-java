...
public class LoanOrCreditJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LoanOrCredit.jsonld");
        assertNotNull(is, "LoanOrCredit.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LoanOrCredit acc = mapper.readValue(jsonld, LoanOrCredit.class);
        assertNotNull(acc);

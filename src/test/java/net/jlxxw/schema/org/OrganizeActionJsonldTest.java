...
public class OrganizeActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OrganizeAction.jsonld");
        assertNotNull(is, "OrganizeAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OrganizeAction acc = mapper.readValue(jsonld, OrganizeAction.class);
        assertNotNull(acc);

...
public class PawnShopJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PawnShop.jsonld");
        assertNotNull(is, "PawnShop.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PawnShop acc = mapper.readValue(jsonld, PawnShop.class);
        assertNotNull(acc);

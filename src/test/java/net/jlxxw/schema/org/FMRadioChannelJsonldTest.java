...
public class FMRadioChannelJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FMRadioChannel.jsonld");
        assertNotNull(is, "FMRadioChannel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FMRadioChannel acc = mapper.readValue(jsonld, FMRadioChannel.class);
        assertNotNull(acc);

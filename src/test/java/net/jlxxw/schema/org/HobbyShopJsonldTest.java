...
public class HobbyShopJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HobbyShop.jsonld");
        assertNotNull(is, "HobbyShop.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HobbyShop acc = mapper.readValue(jsonld, HobbyShop.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        HobbyShop obj = new HobbyShop();
        assertEquals("schema:HobbyShop", obj.getId());

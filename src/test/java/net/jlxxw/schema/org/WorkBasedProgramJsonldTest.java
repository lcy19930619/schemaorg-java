...
public class WorkBasedProgramJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WorkBasedProgram.jsonld");
        assertNotNull(is, "WorkBasedProgram.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WorkBasedProgram acc = mapper.readValue(jsonld, WorkBasedProgram.class);
        assertNotNull(acc);

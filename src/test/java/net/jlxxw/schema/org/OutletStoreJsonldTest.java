...
public class OutletStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OutletStore.jsonld");
        assertNotNull(is, "OutletStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OutletStore acc = mapper.readValue(jsonld, OutletStore.class);
        assertNotNull(acc);

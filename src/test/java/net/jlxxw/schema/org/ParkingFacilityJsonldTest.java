...
public class ParkingFacilityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ParkingFacility.jsonld");
        assertNotNull(is, "ParkingFacility.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ParkingFacility acc = mapper.readValue(jsonld, ParkingFacility.class);
        assertNotNull(acc);

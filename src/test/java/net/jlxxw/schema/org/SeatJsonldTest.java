...
public class SeatJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Seat.jsonld");
        assertNotNull(is, "Seat.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Seat acc = mapper.readValue(jsonld, Seat.class);
        assertNotNull(acc);

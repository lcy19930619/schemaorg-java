...
public class AnatomicalStructureJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AnatomicalStructure.jsonld");
        assertNotNull(is, "AnatomicalStructure.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AnatomicalStructure acc = mapper.readValue(jsonld, AnatomicalStructure.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        AnatomicalStructure obj = new AnatomicalStructure();
        assertEquals("schema:AnatomicalStructure", obj.getId());

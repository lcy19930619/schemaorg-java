...
public class VirtualLocationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VirtualLocation.jsonld");
        assertNotNull(is, "VirtualLocation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VirtualLocation acc = mapper.readValue(jsonld, VirtualLocation.class);
        assertNotNull(acc);

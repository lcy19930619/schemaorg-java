...
public class AutoWashJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AutoWash.jsonld");
        assertNotNull(is, "AutoWash.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AutoWash acc = mapper.readValue(jsonld, AutoWash.class);
        assertNotNull(acc);

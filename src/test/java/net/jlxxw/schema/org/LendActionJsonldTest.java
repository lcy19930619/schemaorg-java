...
public class LendActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LendAction.jsonld");
        assertNotNull(is, "LendAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LendAction acc = mapper.readValue(jsonld, LendAction.class);
        assertNotNull(acc);

...
public class WearActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WearAction.jsonld");
        assertNotNull(is, "WearAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WearAction acc = mapper.readValue(jsonld, WearAction.class);
        assertNotNull(acc);

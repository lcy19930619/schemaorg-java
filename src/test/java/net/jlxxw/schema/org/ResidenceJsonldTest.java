...
public class ResidenceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Residence.jsonld");
        assertNotNull(is, "Residence.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Residence acc = mapper.readValue(jsonld, Residence.class);
        assertNotNull(acc);

...
public class NailSalonJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("NailSalon.jsonld");
        assertNotNull(is, "NailSalon.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        NailSalon acc = mapper.readValue(jsonld, NailSalon.class);
        assertNotNull(acc);

...
public class EmployerReviewJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EmployerReview.jsonld");
        assertNotNull(is, "EmployerReview.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EmployerReview acc = mapper.readValue(jsonld, EmployerReview.class);
        assertNotNull(acc);

...
public class TaxiReservationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TaxiReservation.jsonld");
        assertNotNull(is, "TaxiReservation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TaxiReservation acc = mapper.readValue(jsonld, TaxiReservation.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        TaxiReservation obj = new TaxiReservation();
        assertEquals("schema:TaxiReservation", obj.getId());

...
public class RecommendedDoseScheduleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RecommendedDoseSchedule.jsonld");
        assertNotNull(is, "RecommendedDoseSchedule.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RecommendedDoseSchedule acc = mapper.readValue(jsonld, RecommendedDoseSchedule.class);
        assertNotNull(acc);

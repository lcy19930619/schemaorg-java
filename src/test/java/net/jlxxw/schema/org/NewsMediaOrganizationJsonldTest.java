...
public class NewsMediaOrganizationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("NewsMediaOrganization.jsonld");
        assertNotNull(is, "NewsMediaOrganization.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        NewsMediaOrganization acc = mapper.readValue(jsonld, NewsMediaOrganization.class);
        assertNotNull(acc);

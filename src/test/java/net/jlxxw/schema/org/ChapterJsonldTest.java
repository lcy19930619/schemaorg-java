...
public class ChapterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Chapter.jsonld");
        assertNotNull(is, "Chapter.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Chapter acc = mapper.readValue(jsonld, Chapter.class);
        assertNotNull(acc);

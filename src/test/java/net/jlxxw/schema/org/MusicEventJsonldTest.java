...
public class MusicEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicEvent.jsonld");
        assertNotNull(is, "MusicEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicEvent acc = mapper.readValue(jsonld, MusicEvent.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MusicEvent obj = new MusicEvent();
        assertEquals("schema:MusicEvent", obj.getId());

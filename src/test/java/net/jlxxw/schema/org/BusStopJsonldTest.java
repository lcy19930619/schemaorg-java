...
public class BusStopJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BusStop.jsonld");
        assertNotNull(is, "BusStop.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BusStop acc = mapper.readValue(jsonld, BusStop.class);
        assertNotNull(acc);

...
public class MedicalStudyStatusJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalStudyStatus.jsonld");
        assertNotNull(is, "MedicalStudyStatus.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalStudyStatus acc = mapper.readValue(jsonld, MedicalStudyStatus.class);
        assertNotNull(acc);

...
public class LearningResourceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LearningResource.jsonld");
        assertNotNull(is, "LearningResource.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LearningResource acc = mapper.readValue(jsonld, LearningResource.class);
        assertNotNull(acc);

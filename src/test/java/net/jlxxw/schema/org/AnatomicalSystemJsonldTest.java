...
public class AnatomicalSystemJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AnatomicalSystem.jsonld");
        assertNotNull(is, "AnatomicalSystem.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AnatomicalSystem acc = mapper.readValue(jsonld, AnatomicalSystem.class);
        assertNotNull(acc);

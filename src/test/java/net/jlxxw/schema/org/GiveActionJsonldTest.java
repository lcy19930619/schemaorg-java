...
public class GiveActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GiveAction.jsonld");
        assertNotNull(is, "GiveAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GiveAction acc = mapper.readValue(jsonld, GiveAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        GiveAction obj = new GiveAction();
        assertEquals("schema:GiveAction", obj.getId());

...
public class OccupationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Occupation.jsonld");
        assertNotNull(is, "Occupation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Occupation acc = mapper.readValue(jsonld, Occupation.class);
        assertNotNull(acc);

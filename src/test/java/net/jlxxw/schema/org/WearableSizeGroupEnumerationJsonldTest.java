...
public class WearableSizeGroupEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WearableSizeGroupEnumeration.jsonld");
        assertNotNull(is, "WearableSizeGroupEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WearableSizeGroupEnumeration acc = mapper.readValue(jsonld, WearableSizeGroupEnumeration.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        WearableSizeGroupEnumeration obj = new WearableSizeGroupEnumeration();
        assertEquals("schema:WearableSizeGroupEnumeration", obj.getId());

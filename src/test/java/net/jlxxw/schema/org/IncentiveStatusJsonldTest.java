...
public class IncentiveStatusJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("IncentiveStatus.jsonld");
        assertNotNull(is, "IncentiveStatus.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        IncentiveStatus acc = mapper.readValue(jsonld, IncentiveStatus.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        IncentiveStatus obj = new IncentiveStatus();
        assertEquals("schema:IncentiveStatus", obj.getId());

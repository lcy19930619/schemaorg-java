...
public class ProductGroupJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ProductGroup.jsonld");
        assertNotNull(is, "ProductGroup.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ProductGroup acc = mapper.readValue(jsonld, ProductGroup.class);
        assertNotNull(acc);

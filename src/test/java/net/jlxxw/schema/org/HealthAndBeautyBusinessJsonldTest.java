...
public class HealthAndBeautyBusinessJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HealthAndBeautyBusiness.jsonld");
        assertNotNull(is, "HealthAndBeautyBusiness.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HealthAndBeautyBusiness acc = mapper.readValue(jsonld, HealthAndBeautyBusiness.class);
        assertNotNull(acc);

...
public class MediaObjectJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MediaObject.jsonld");
        assertNotNull(is, "MediaObject.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MediaObject acc = mapper.readValue(jsonld, MediaObject.class);
        assertNotNull(acc);

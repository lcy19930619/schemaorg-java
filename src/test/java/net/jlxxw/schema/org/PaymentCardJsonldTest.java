...
public class PaymentCardJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PaymentCard.jsonld");
        assertNotNull(is, "PaymentCard.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PaymentCard acc = mapper.readValue(jsonld, PaymentCard.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        PaymentCard obj = new PaymentCard();
        assertEquals("schema:PaymentCard", obj.getId());

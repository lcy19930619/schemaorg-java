...
public class RadioSeasonJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RadioSeason.jsonld");
        assertNotNull(is, "RadioSeason.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RadioSeason acc = mapper.readValue(jsonld, RadioSeason.class);
        assertNotNull(acc);

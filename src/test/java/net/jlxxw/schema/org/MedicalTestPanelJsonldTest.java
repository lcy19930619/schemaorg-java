...
public class MedicalTestPanelJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalTestPanel.jsonld");
        assertNotNull(is, "MedicalTestPanel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalTestPanel acc = mapper.readValue(jsonld, MedicalTestPanel.class);
        assertNotNull(acc);

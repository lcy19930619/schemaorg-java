...
public class BloodTestJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BloodTest.jsonld");
        assertNotNull(is, "BloodTest.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BloodTest acc = mapper.readValue(jsonld, BloodTest.class);
        assertNotNull(acc);

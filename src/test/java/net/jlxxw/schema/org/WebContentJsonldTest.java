...
public class WebContentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WebContent.jsonld");
        assertNotNull(is, "WebContent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WebContent acc = mapper.readValue(jsonld, WebContent.class);
        assertNotNull(acc);

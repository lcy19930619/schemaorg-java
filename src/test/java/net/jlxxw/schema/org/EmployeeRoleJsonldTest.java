...
public class EmployeeRoleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EmployeeRole.jsonld");
        assertNotNull(is, "EmployeeRole.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EmployeeRole acc = mapper.readValue(jsonld, EmployeeRole.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ...
        assertEquals("schema:EmployeeRole", obj.getId());

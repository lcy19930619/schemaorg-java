...
public class EnergyJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Energy.jsonld");
        assertNotNull(is, "Energy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Energy acc = mapper.readValue(jsonld, Energy.class);
        assertNotNull(acc);

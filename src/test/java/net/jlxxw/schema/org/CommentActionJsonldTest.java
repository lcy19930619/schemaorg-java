...
public class CommentActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CommentAction.jsonld");
        assertNotNull(is, "CommentAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CommentAction acc = mapper.readValue(jsonld, CommentAction.class);
        assertNotNull(acc);

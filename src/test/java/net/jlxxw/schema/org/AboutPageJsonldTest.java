...
public class AboutPageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AboutPage.jsonld");
        assertNotNull(is, "AboutPage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AboutPage acc = mapper.readValue(jsonld, AboutPage.class);
        assertNotNull(acc);

...
public class GeneralContractorJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GeneralContractor.jsonld");
        assertNotNull(is, "GeneralContractor.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GeneralContractor acc = mapper.readValue(jsonld, GeneralContractor.class);
        assertNotNull(acc);

...
public class DryCleaningOrLaundryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DryCleaningOrLaundry.jsonld");
        assertNotNull(is, "DryCleaningOrLaundry.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DryCleaningOrLaundry acc = mapper.readValue(jsonld, DryCleaningOrLaundry.class);
        assertNotNull(acc);

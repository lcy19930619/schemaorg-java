...
public class PeopleAudienceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PeopleAudience.jsonld");
        assertNotNull(is, "PeopleAudience.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PeopleAudience acc = mapper.readValue(jsonld, PeopleAudience.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        PeopleAudience obj = new PeopleAudience();
        assertEquals("schema:PeopleAudience", obj.getId());

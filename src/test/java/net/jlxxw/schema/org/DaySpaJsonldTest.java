...
public class DaySpaJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DaySpa.jsonld");
        assertNotNull(is, "DaySpa.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DaySpa acc = mapper.readValue(jsonld, DaySpa.class);
        assertNotNull(acc);

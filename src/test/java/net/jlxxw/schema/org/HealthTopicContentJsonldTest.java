...
public class HealthTopicContentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HealthTopicContent.jsonld");
        assertNotNull(is, "HealthTopicContent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HealthTopicContent acc = mapper.readValue(jsonld, HealthTopicContent.class);
        assertNotNull(acc);

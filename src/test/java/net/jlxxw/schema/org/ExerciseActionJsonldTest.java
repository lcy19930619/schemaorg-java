...
public class ExerciseActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ExerciseAction.jsonld");
        assertNotNull(is, "ExerciseAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ExerciseAction acc = mapper.readValue(jsonld, ExerciseAction.class);
        assertNotNull(acc);

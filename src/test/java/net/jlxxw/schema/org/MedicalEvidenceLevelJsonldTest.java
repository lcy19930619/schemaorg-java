...
public class MedicalEvidenceLevelJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalEvidenceLevel.jsonld");
        assertNotNull(is, "MedicalEvidenceLevel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalEvidenceLevel acc = mapper.readValue(jsonld, MedicalEvidenceLevel.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalEvidenceLevel obj = new MedicalEvidenceLevel();
        assertEquals("schema:MedicalEvidenceLevel", obj.getId());

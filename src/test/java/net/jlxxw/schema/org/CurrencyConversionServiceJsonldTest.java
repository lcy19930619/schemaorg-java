...
public class CurrencyConversionServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CurrencyConversionService.jsonld");
        assertNotNull(is, "CurrencyConversionService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CurrencyConversionService acc = mapper.readValue(jsonld, CurrencyConversionService.class);
        assertNotNull(acc);

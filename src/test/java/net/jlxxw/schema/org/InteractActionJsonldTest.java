...
public class InteractActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("InteractAction.jsonld");
        assertNotNull(is, "InteractAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        InteractAction acc = mapper.readValue(jsonld, InteractAction.class);
        assertNotNull(acc);

...
public class MedicalObservationalStudyDesignJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalObservationalStudyDesign.jsonld");
        assertNotNull(is, "MedicalObservationalStudyDesign.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalObservationalStudyDesign acc = mapper.readValue(jsonld, MedicalObservationalStudyDesign.class);
        assertNotNull(acc);

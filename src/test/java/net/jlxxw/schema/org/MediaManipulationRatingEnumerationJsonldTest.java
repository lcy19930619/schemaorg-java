...
public class MediaManipulationRatingEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MediaManipulationRatingEnumeration.jsonld");
        assertNotNull(is, "MediaManipulationRatingEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MediaManipulationRatingEnumeration acc = mapper.readValue(jsonld, MediaManipulationRatingEnumeration.class);
        assertNotNull(acc);

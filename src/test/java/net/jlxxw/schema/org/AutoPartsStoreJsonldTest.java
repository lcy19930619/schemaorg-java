...
public class AutoPartsStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AutoPartsStore.jsonld");
        assertNotNull(is, "AutoPartsStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AutoPartsStore acc = mapper.readValue(jsonld, AutoPartsStore.class);
        assertNotNull(acc);

...
public class MusicAlbumJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicAlbum.jsonld");
        assertNotNull(is, "MusicAlbum.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicAlbum acc = mapper.readValue(jsonld, MusicAlbum.class);
        assertNotNull(acc);

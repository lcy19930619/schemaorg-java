...
public class ProfilePageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ProfilePage.jsonld");
        assertNotNull(is, "ProfilePage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ProfilePage acc = mapper.readValue(jsonld, ProfilePage.class);
        assertNotNull(acc);

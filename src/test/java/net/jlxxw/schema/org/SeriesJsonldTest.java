...
public class SeriesJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Series.jsonld");
        assertNotNull(is, "Series.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Series acc = mapper.readValue(jsonld, Series.class);
        assertNotNull(acc);

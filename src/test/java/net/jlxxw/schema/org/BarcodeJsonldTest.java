...
public class BarcodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Barcode.jsonld");
        assertNotNull(is, "Barcode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Barcode acc = mapper.readValue(jsonld, Barcode.class);
        assertNotNull(acc);

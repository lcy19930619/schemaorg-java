...
public class BoneJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Bone.jsonld");
        assertNotNull(is, "Bone.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Bone acc = mapper.readValue(jsonld, Bone.class);
        assertNotNull(acc);

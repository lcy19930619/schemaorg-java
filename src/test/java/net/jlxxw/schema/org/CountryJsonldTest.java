...
public class CountryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Country.jsonld");
        assertNotNull(is, "Country.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Country acc = mapper.readValue(jsonld, Country.class);
        assertNotNull(acc);

...
public class WaterfallJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Waterfall.jsonld");
        assertNotNull(is, "Waterfall.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Waterfall acc = mapper.readValue(jsonld, Waterfall.class);
        assertNotNull(acc);

...
public class JoinActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("JoinAction.jsonld");
        assertNotNull(is, "JoinAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        JoinAction acc = mapper.readValue(jsonld, JoinAction.class);
        assertNotNull(acc);

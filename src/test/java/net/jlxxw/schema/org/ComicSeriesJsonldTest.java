...
public class ComicSeriesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ComicSeries.jsonld");
        assertNotNull(is, "ComicSeries.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ComicSeries acc = mapper.readValue(jsonld, ComicSeries.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ComicSeries obj = new ComicSeries();
        assertEquals("schema:ComicSeries", obj.getId());

...
public class EventAttendanceModeEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EventAttendanceModeEnumeration.jsonld");
        assertNotNull(is, "EventAttendanceModeEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EventAttendanceModeEnumeration acc = mapper.readValue(jsonld, EventAttendanceModeEnumeration.class);
        assertNotNull(acc);

...
public class ClaimReviewJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ClaimReview.jsonld");
        assertNotNull(is, "ClaimReview.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ClaimReview acc = mapper.readValue(jsonld, ClaimReview.class);
        assertNotNull(acc);

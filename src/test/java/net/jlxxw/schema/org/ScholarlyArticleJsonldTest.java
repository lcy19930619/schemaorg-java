...
public class ScholarlyArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ScholarlyArticle.jsonld");
        assertNotNull(is, "ScholarlyArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ScholarlyArticle acc = mapper.readValue(jsonld, ScholarlyArticle.class);
        assertNotNull(acc);

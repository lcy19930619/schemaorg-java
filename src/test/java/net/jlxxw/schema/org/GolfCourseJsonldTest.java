...
public class GolfCourseJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GolfCourse.jsonld");
        assertNotNull(is, "GolfCourse.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GolfCourse acc = mapper.readValue(jsonld, GolfCourse.class);
        assertNotNull(acc);

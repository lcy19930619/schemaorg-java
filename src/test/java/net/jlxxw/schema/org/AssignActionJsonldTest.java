...
public class AssignActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AssignAction.jsonld");
        assertNotNull(is, "AssignAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AssignAction acc = mapper.readValue(jsonld, AssignAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        AssignAction obj = new AssignAction();
        assertEquals("schema:AssignAction", obj.getId());

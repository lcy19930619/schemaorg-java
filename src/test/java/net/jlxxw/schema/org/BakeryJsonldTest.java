...
public class BakeryJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Bakery.jsonld");
        assertNotNull(is, "Bakery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Bakery acc = mapper.readValue(jsonld, Bakery.class);
        assertNotNull(acc);

...
public class UKNonprofitTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UKNonprofitType.jsonld");
        assertNotNull(is, "UKNonprofitType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UKNonprofitType acc = mapper.readValue(jsonld, UKNonprofitType.class);
        assertNotNull(acc);

...
public class RatingJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Rating.jsonld");
        assertNotNull(is, "Rating.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Rating acc = mapper.readValue(jsonld, Rating.class);
        assertNotNull(acc);

...
public class HealthClubJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HealthClub.jsonld");
        assertNotNull(is, "HealthClub.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HealthClub acc = mapper.readValue(jsonld, HealthClub.class);
        assertNotNull(acc);

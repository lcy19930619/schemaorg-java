...
public class DepartActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DepartAction.jsonld");
        assertNotNull(is, "DepartAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DepartAction acc = mapper.readValue(jsonld, DepartAction.class);
        assertNotNull(acc);

...
public class FilmActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FilmAction.jsonld");
        assertNotNull(is, "FilmAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FilmAction acc = mapper.readValue(jsonld, FilmAction.class);
        assertNotNull(acc);

...
public class CasinoJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Casino.jsonld");
        assertNotNull(is, "Casino.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Casino acc = mapper.readValue(jsonld, Casino.class);
        assertNotNull(acc);

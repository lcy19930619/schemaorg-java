...
public class BrainStructureJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BrainStructure.jsonld");
        assertNotNull(is, "BrainStructure.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BrainStructure acc = mapper.readValue(jsonld, BrainStructure.class);
        assertNotNull(acc);

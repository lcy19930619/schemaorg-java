...
public class PlanActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PlanAction.jsonld");
        assertNotNull(is, "PlanAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PlanAction acc = mapper.readValue(jsonld, PlanAction.class);
        assertNotNull(acc);

...
public class CityJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("City.jsonld");
        assertNotNull(is, "City.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        City acc = mapper.readValue(jsonld, City.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        City obj = new City();
        assertEquals("schema:City", obj.getId());

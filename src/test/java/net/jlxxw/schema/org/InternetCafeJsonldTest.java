...
public class InternetCafeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("InternetCafe.jsonld");
        assertNotNull(is, "InternetCafe.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        InternetCafe acc = mapper.readValue(jsonld, InternetCafe.class);
        assertNotNull(acc);

...
public class HotelJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Hotel.jsonld");
        assertNotNull(is, "Hotel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Hotel acc = mapper.readValue(jsonld, Hotel.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Hotel obj = new Hotel();
        assertEquals("schema:Hotel", obj.getId());

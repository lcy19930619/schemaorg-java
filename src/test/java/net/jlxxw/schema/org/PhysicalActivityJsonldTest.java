...
public class PhysicalActivityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PhysicalActivity.jsonld");
        assertNotNull(is, "PhysicalActivity.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PhysicalActivity acc = mapper.readValue(jsonld, PhysicalActivity.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ...
        assertEquals("schema:PhysicalActivity", obj.getId());

...
public class PostalCodeRangeSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PostalCodeRangeSpecification.jsonld");
        assertNotNull(is, "PostalCodeRangeSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PostalCodeRangeSpecification acc = mapper.readValue(jsonld, PostalCodeRangeSpecification.class);
        assertNotNull(acc);

...
public class GameServerJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GameServer.jsonld");
        assertNotNull(is, "GameServer.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GameServer acc = mapper.readValue(jsonld, GameServer.class);
        assertNotNull(acc);

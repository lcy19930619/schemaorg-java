...
public class MountainJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Mountain.jsonld");
        assertNotNull(is, "Mountain.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Mountain acc = mapper.readValue(jsonld, Mountain.class);
        assertNotNull(acc);

...
public class HostelJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Hostel.jsonld");
        assertNotNull(is, "Hostel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Hostel acc = mapper.readValue(jsonld, Hostel.class);
        assertNotNull(acc);

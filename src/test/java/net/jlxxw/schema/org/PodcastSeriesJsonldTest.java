...
public class PodcastSeriesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PodcastSeries.jsonld");
        assertNotNull(is, "PodcastSeries.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PodcastSeries acc = mapper.readValue(jsonld, PodcastSeries.class);
        assertNotNull(acc);

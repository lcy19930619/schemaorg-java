...
public class MobilePhoneStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MobilePhoneStore.jsonld");
        assertNotNull(is, "MobilePhoneStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MobilePhoneStore acc = mapper.readValue(jsonld, MobilePhoneStore.class);
        assertNotNull(acc);

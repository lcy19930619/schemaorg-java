...
public class MedicalGuidelineContraindicationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalGuidelineContraindication.jsonld");
        assertNotNull(is, "MedicalGuidelineContraindication.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalGuidelineContraindication acc = mapper.readValue(jsonld, MedicalGuidelineContraindication.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalGuidelineContraindication obj = new MedicalGuidelineContraindication();
        assertEquals("schema:MedicalGuidelineContraindication", obj.getId());

...
public class UnRegisterActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UnRegisterAction.jsonld");
        assertNotNull(is, "UnRegisterAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UnRegisterAction acc = mapper.readValue(jsonld, UnRegisterAction.class);
        assertNotNull(acc);

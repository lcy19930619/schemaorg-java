...
public class LifestyleModificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LifestyleModification.jsonld");
        assertNotNull(is, "LifestyleModification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LifestyleModification acc = mapper.readValue(jsonld, LifestyleModification.class);
        assertNotNull(acc);

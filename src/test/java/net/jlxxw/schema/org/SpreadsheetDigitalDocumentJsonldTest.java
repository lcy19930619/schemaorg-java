...
public class SpreadsheetDigitalDocumentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SpreadsheetDigitalDocument.jsonld");
        assertNotNull(is, "SpreadsheetDigitalDocument.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SpreadsheetDigitalDocument acc = mapper.readValue(jsonld, SpreadsheetDigitalDocument.class);
        assertNotNull(acc);

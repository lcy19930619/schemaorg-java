...
public class MeasurementTypeEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MeasurementTypeEnumeration.jsonld");
        assertNotNull(is, "MeasurementTypeEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MeasurementTypeEnumeration acc = mapper.readValue(jsonld, MeasurementTypeEnumeration.class);
        assertNotNull(acc);

...
public class ConstraintNodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ConstraintNode.jsonld");
        assertNotNull(is, "ConstraintNode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ConstraintNode acc = mapper.readValue(jsonld, ConstraintNode.class);
        assertNotNull(acc);

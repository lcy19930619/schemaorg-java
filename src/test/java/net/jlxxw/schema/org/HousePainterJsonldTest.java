...
public class HousePainterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HousePainter.jsonld");
        assertNotNull(is, "HousePainter.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HousePainter acc = mapper.readValue(jsonld, HousePainter.class);
        assertNotNull(acc);

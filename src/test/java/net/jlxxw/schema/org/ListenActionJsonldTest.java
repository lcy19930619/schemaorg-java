...
public class ListenActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ListenAction.jsonld");
        assertNotNull(is, "ListenAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ListenAction acc = mapper.readValue(jsonld, ListenAction.class);
        assertNotNull(acc);

...
public class MedicalTestJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalTest.jsonld");
        assertNotNull(is, "MedicalTest.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalTest acc = mapper.readValue(jsonld, MedicalTest.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalTest obj = new MedicalTest();
        assertEquals("schema:MedicalTest", obj.getId());

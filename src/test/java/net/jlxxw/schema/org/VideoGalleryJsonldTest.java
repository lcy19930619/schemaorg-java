...
public class VideoGalleryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VideoGallery.jsonld");
        assertNotNull(is, "VideoGallery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VideoGallery acc = mapper.readValue(jsonld, VideoGallery.class);
        assertNotNull(acc);

...
public class CreativeWorkSeasonJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CreativeWorkSeason.jsonld");
        assertNotNull(is, "CreativeWorkSeason.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CreativeWorkSeason acc = mapper.readValue(jsonld, CreativeWorkSeason.class);
        assertNotNull(acc);

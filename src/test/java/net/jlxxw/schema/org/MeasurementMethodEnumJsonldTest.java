...
public class MeasurementMethodEnumJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MeasurementMethodEnum.jsonld");
        assertNotNull(is, "MeasurementMethodEnum.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MeasurementMethodEnum acc = mapper.readValue(jsonld, MeasurementMethodEnum.class);
        assertNotNull(acc);

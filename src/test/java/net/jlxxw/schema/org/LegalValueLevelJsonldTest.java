...
public class LegalValueLevelJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LegalValueLevel.jsonld");
        assertNotNull(is, "LegalValueLevel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LegalValueLevel acc = mapper.readValue(jsonld, LegalValueLevel.class);
        assertNotNull(acc);

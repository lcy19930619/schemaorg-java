...
public class AdvertiserContentArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AdvertiserContentArticle.jsonld");
        assertNotNull(is, "AdvertiserContentArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AdvertiserContentArticle acc = mapper.readValue(jsonld, AdvertiserContentArticle.class);
        assertNotNull(acc);

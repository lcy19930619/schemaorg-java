...
public class PathologyTestJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PathologyTest.jsonld");
        assertNotNull(is, "PathologyTest.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PathologyTest acc = mapper.readValue(jsonld, PathologyTest.class);
        assertNotNull(acc);

...
public class HighSchoolJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HighSchool.jsonld");
        assertNotNull(is, "HighSchool.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HighSchool acc = mapper.readValue(jsonld, HighSchool.class);
        assertNotNull(acc);

...
public class CoverArtJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CoverArt.jsonld");
        assertNotNull(is, "CoverArt.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CoverArt acc = mapper.readValue(jsonld, CoverArt.class);
        assertNotNull(acc);

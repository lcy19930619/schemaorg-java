...
public class BuddhistTempleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BuddhistTemple.jsonld");
        assertNotNull(is, "BuddhistTemple.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BuddhistTemple acc = mapper.readValue(jsonld, BuddhistTemple.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        BuddhistTemple obj = new BuddhistTemple();
        assertEquals("schema:BuddhistTemple", obj.getId());

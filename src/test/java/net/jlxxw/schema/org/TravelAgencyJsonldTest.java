...
public class TravelAgencyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TravelAgency.jsonld");
        assertNotNull(is, "TravelAgency.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TravelAgency acc = mapper.readValue(jsonld, TravelAgency.class);
        assertNotNull(acc);

...
public class UnitPriceSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UnitPriceSpecification.jsonld");
        assertNotNull(is, "UnitPriceSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UnitPriceSpecification acc = mapper.readValue(jsonld, UnitPriceSpecification.class);
        assertNotNull(acc);

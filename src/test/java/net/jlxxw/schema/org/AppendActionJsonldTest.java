...
public class AppendActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AppendAction.jsonld");
        assertNotNull(is, "AppendAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AppendAction acc = mapper.readValue(jsonld, AppendAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        AppendAction obj = new AppendAction();
        assertEquals("schema:AppendAction", obj.getId());

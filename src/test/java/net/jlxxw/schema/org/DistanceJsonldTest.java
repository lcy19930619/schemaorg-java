...
public class DistanceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Distance.jsonld");
        assertNotNull(is, "Distance.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Distance acc = mapper.readValue(jsonld, Distance.class);
        assertNotNull(acc);

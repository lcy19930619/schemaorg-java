...
public class ReviewNewsArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReviewNewsArticle.jsonld");
        assertNotNull(is, "ReviewNewsArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReviewNewsArticle acc = mapper.readValue(jsonld, ReviewNewsArticle.class);
        assertNotNull(acc);

...
public class DrinkActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DrinkAction.jsonld");
        assertNotNull(is, "DrinkAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DrinkAction acc = mapper.readValue(jsonld, DrinkAction.class);
        assertNotNull(acc);

...
public class PhotographJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Photograph.jsonld");
        assertNotNull(is, "Photograph.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Photograph acc = mapper.readValue(jsonld, Photograph.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Photograph obj = new Photograph();
        assertEquals("schema:Photograph", obj.getId());

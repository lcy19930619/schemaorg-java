...
public class CafeOrCoffeeShopJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CafeOrCoffeeShop.jsonld");
        assertNotNull(is, "CafeOrCoffeeShop.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CafeOrCoffeeShop acc = mapper.readValue(jsonld, CafeOrCoffeeShop.class);
        assertNotNull(acc);

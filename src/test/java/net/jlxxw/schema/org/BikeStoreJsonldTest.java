...
public class BikeStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BikeStore.jsonld");
        assertNotNull(is, "BikeStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BikeStore acc = mapper.readValue(jsonld, BikeStore.class);
        assertNotNull(acc);

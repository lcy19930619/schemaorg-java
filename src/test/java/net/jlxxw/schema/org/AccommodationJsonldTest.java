...
public class AccommodationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Accommodation.jsonld");
        assertNotNull(is, "Accommodation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Accommodation acc = mapper.readValue(jsonld, Accommodation.class);
        assertNotNull(acc);

...
public class PeriodicalJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Periodical.jsonld");
        assertNotNull(is, "Periodical.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Periodical acc = mapper.readValue(jsonld, Periodical.class);
        assertNotNull(acc);

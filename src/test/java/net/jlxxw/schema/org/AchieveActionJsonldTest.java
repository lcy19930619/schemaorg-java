...
public class AchieveActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AchieveAction.jsonld");
        assertNotNull(is, "AchieveAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AchieveAction acc = mapper.readValue(jsonld, AchieveAction.class);
        assertNotNull(acc);

...
public class UserPageVisitsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserPageVisits.jsonld");
        assertNotNull(is, "UserPageVisits.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserPageVisits acc = mapper.readValue(jsonld, UserPageVisits.class);
        assertNotNull(acc);

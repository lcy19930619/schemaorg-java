...
public class MedicalCodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalCode.jsonld");
        assertNotNull(is, "MedicalCode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalCode acc = mapper.readValue(jsonld, MedicalCode.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalCode obj = new MedicalCode();
        assertEquals("schema:MedicalCode", obj.getId());

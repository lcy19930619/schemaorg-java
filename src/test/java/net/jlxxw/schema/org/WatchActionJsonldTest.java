...
public class WatchActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WatchAction.jsonld");
        assertNotNull(is, "WatchAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WatchAction acc = mapper.readValue(jsonld, WatchAction.class);
        assertNotNull(acc);

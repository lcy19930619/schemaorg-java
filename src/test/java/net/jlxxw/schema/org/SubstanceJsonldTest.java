...
public class SubstanceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Substance.jsonld");
        assertNotNull(is, "Substance.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Substance acc = mapper.readValue(jsonld, Substance.class);
        assertNotNull(acc);

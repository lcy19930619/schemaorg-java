...
public class LegislationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Legislation.jsonld");
        assertNotNull(is, "Legislation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Legislation acc = mapper.readValue(jsonld, Legislation.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Legislation obj = new Legislation();
        assertEquals("schema:Legislation", obj.getId());

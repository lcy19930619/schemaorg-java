...
public class WebAPIJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WebAPI.jsonld");
        assertNotNull(is, "WebAPI.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WebAPI acc = mapper.readValue(jsonld, WebAPI.class);
        assertNotNull(acc);

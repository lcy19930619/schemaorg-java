...
public class MemberProgramJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MemberProgram.jsonld");
        assertNotNull(is, "MemberProgram.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MemberProgram acc = mapper.readValue(jsonld, MemberProgram.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MemberProgram obj = new MemberProgram();
        assertEquals("schema:MemberProgram", obj.getId());

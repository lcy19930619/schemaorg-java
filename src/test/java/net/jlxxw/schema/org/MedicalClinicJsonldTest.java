...
public class MedicalClinicJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalClinic.jsonld");
        assertNotNull(is, "MedicalClinic.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalClinic acc = mapper.readValue(jsonld, MedicalClinic.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalClinic obj = new MedicalClinic();
        assertEquals("schema:MedicalClinic", obj.getId());

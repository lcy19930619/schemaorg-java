...
public class PublicToiletJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PublicToilet.jsonld");
        assertNotNull(is, "PublicToilet.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PublicToilet acc = mapper.readValue(jsonld, PublicToilet.class);
        assertNotNull(acc);

...
public class ScreeningEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ScreeningEvent.jsonld");
        assertNotNull(is, "ScreeningEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ScreeningEvent acc = mapper.readValue(jsonld, ScreeningEvent.class);
        assertNotNull(acc);

...
public class StateJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("State.jsonld");
        assertNotNull(is, "State.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        State acc = mapper.readValue(jsonld, State.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        State obj = new State();
        assertEquals("schema:State", obj.getId());

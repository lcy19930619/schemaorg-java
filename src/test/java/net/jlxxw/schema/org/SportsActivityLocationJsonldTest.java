...
public class SportsActivityLocationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SportsActivityLocation.jsonld");
        assertNotNull(is, "SportsActivityLocation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SportsActivityLocation acc = mapper.readValue(jsonld, SportsActivityLocation.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        SportsActivityLocation obj = new SportsActivityLocation();
        assertEquals("schema:SportsActivityLocation", obj.getId());

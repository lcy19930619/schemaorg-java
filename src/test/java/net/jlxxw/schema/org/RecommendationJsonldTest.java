...
public class RecommendationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Recommendation.jsonld");
        assertNotNull(is, "Recommendation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Recommendation acc = mapper.readValue(jsonld, Recommendation.class);
        assertNotNull(acc);

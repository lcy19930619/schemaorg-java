...
public class PlayGameActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PlayGameAction.jsonld");
        assertNotNull(is, "PlayGameAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PlayGameAction acc = mapper.readValue(jsonld, PlayGameAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        PlayGameAction obj = new PlayGameAction();
        assertEquals("schema:PlayGameAction", obj.getId());

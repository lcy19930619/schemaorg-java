...
public class UserDownloadsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserDownloads.jsonld");
        assertNotNull(is, "UserDownloads.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserDownloads acc = mapper.readValue(jsonld, UserDownloads.class);
        assertNotNull(acc);

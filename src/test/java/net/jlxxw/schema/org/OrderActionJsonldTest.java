...
public class OrderActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OrderAction.jsonld");
        assertNotNull(is, "OrderAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OrderAction acc = mapper.readValue(jsonld, OrderAction.class);
        assertNotNull(acc);

...
public class EventReservationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EventReservation.jsonld");
        assertNotNull(is, "EventReservation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EventReservation acc = mapper.readValue(jsonld, EventReservation.class);
        assertNotNull(acc);

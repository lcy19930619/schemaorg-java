...
public class BroadcastEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BroadcastEvent.jsonld");
        assertNotNull(is, "BroadcastEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BroadcastEvent acc = mapper.readValue(jsonld, BroadcastEvent.class);
        assertNotNull(acc);

...
public class IPTCDigitalSourceEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("IPTCDigitalSourceEnumeration.jsonld");
        assertNotNull(is, "IPTCDigitalSourceEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        IPTCDigitalSourceEnumeration acc = mapper.readValue(jsonld, IPTCDigitalSourceEnumeration.class);
        assertNotNull(acc);

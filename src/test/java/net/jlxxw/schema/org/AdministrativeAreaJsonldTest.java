...
public class AdministrativeAreaJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AdministrativeArea.jsonld");
        assertNotNull(is, "AdministrativeArea.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AdministrativeArea acc = mapper.readValue(jsonld, AdministrativeArea.class);
        assertNotNull(acc);

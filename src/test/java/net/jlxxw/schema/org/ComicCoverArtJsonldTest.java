...
public class ComicCoverArtJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ComicCoverArt.jsonld");
        assertNotNull(is, "ComicCoverArt.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ComicCoverArt acc = mapper.readValue(jsonld, ComicCoverArt.class);
        assertNotNull(acc);

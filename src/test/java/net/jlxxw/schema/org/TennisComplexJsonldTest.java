...
public class TennisComplexJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TennisComplex.jsonld");
        assertNotNull(is, "TennisComplex.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TennisComplex acc = mapper.readValue(jsonld, TennisComplex.class);
        assertNotNull(acc);

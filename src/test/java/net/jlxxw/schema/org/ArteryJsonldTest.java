...
public class ArteryJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Artery.jsonld");
        assertNotNull(is, "Artery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Artery acc = mapper.readValue(jsonld, Artery.class);
        assertNotNull(acc);

...
public class BoatReservationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BoatReservation.jsonld");
        assertNotNull(is, "BoatReservation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BoatReservation acc = mapper.readValue(jsonld, BoatReservation.class);
        assertNotNull(acc);

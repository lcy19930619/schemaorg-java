...
public class NoteDigitalDocumentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("NoteDigitalDocument.jsonld");
        assertNotNull(is, "NoteDigitalDocument.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        NoteDigitalDocument acc = mapper.readValue(jsonld, NoteDigitalDocument.class);
        assertNotNull(acc);

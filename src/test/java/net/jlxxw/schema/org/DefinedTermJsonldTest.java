...
public class DefinedTermJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DefinedTerm.jsonld");
        assertNotNull(is, "DefinedTerm.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DefinedTerm acc = mapper.readValue(jsonld, DefinedTerm.class);
        assertNotNull(acc);

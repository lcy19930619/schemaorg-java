...
public class UserReviewJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserReview.jsonld");
        assertNotNull(is, "UserReview.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserReview acc = mapper.readValue(jsonld, UserReview.class);
        assertNotNull(acc);

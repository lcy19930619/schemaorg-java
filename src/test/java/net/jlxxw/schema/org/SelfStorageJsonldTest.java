...
public class SelfStorageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SelfStorage.jsonld");
        assertNotNull(is, "SelfStorage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SelfStorage acc = mapper.readValue(jsonld, SelfStorage.class);
        assertNotNull(acc);

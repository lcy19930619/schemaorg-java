...
public class ChildCareJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ChildCare.jsonld");
        assertNotNull(is, "ChildCare.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ChildCare acc = mapper.readValue(jsonld, ChildCare.class);
        assertNotNull(acc);

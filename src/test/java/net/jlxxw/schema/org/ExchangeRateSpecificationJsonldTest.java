...
public class ExchangeRateSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ExchangeRateSpecification.jsonld");
        assertNotNull(is, "ExchangeRateSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ExchangeRateSpecification acc = mapper.readValue(jsonld, ExchangeRateSpecification.class);
        assertNotNull(acc);

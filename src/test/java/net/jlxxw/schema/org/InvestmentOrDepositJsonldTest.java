...
public class InvestmentOrDepositJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("InvestmentOrDeposit.jsonld");
        assertNotNull(is, "InvestmentOrDeposit.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        InvestmentOrDeposit acc = mapper.readValue(jsonld, InvestmentOrDeposit.class);
        assertNotNull(acc);

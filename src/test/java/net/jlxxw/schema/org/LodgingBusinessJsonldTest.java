...
public class LodgingBusinessJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LodgingBusiness.jsonld");
        assertNotNull(is, "LodgingBusiness.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LodgingBusiness acc = mapper.readValue(jsonld, LodgingBusiness.class);
        assertNotNull(acc);

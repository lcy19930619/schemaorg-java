...
public class MusicRecordingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicRecording.jsonld");
        assertNotNull(is, "MusicRecording.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicRecording acc = mapper.readValue(jsonld, MusicRecording.class);
        assertNotNull(acc);

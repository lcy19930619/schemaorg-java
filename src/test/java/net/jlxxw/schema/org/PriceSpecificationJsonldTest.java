...
public class PriceSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PriceSpecification.jsonld");
        assertNotNull(is, "PriceSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PriceSpecification acc = mapper.readValue(jsonld, PriceSpecification.class);
        assertNotNull(acc);

...
public class QualitativeValueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("QualitativeValue.jsonld");
        assertNotNull(is, "QualitativeValue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        QualitativeValue acc = mapper.readValue(jsonld, QualitativeValue.class);
        assertNotNull(acc);

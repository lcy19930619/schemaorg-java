...
public class BeautySalonJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BeautySalon.jsonld");
        assertNotNull(is, "BeautySalon.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BeautySalon acc = mapper.readValue(jsonld, BeautySalon.class);
        assertNotNull(acc);

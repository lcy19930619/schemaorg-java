...
public class ReservationStatusTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReservationStatusType.jsonld");
        assertNotNull(is, "ReservationStatusType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReservationStatusType acc = mapper.readValue(jsonld, ReservationStatusType.class);
        assertNotNull(acc);

...
public class GovernmentBenefitsTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GovernmentBenefitsType.jsonld");
        assertNotNull(is, "GovernmentBenefitsType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GovernmentBenefitsType acc = mapper.readValue(jsonld, GovernmentBenefitsType.class);
        assertNotNull(acc);

...
public class MedicalProcedureTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalProcedureType.jsonld");
        assertNotNull(is, "MedicalProcedureType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalProcedureType acc = mapper.readValue(jsonld, MedicalProcedureType.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalProcedureType obj = new MedicalProcedureType();
        assertEquals("schema:MedicalProcedureType", obj.getId());

...
public class LocationFeatureSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LocationFeatureSpecification.jsonld");
        assertNotNull(is, "LocationFeatureSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LocationFeatureSpecification acc = mapper.readValue(jsonld, LocationFeatureSpecification.class);
        assertNotNull(acc);

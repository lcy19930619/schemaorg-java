...
public class SendActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SendAction.jsonld");
        assertNotNull(is, "SendAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SendAction acc = mapper.readValue(jsonld, SendAction.class);
        assertNotNull(acc);

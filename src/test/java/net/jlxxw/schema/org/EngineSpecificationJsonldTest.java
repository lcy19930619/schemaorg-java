...
public class EngineSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EngineSpecification.jsonld");
        assertNotNull(is, "EngineSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EngineSpecification acc = mapper.readValue(jsonld, EngineSpecification.class);
        assertNotNull(acc);

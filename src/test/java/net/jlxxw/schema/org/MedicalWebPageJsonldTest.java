...
public class MedicalWebPageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalWebPage.jsonld");
        assertNotNull(is, "MedicalWebPage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalWebPage acc = mapper.readValue(jsonld, MedicalWebPage.class);
        assertNotNull(acc);

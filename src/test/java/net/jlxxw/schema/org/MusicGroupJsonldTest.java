...
public class MusicGroupJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicGroup.jsonld");
        assertNotNull(is, "MusicGroup.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicGroup acc = mapper.readValue(jsonld, MusicGroup.class);
        assertNotNull(acc);

...
public class ImageGalleryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ImageGallery.jsonld");
        assertNotNull(is, "ImageGallery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ImageGallery acc = mapper.readValue(jsonld, ImageGallery.class);
        assertNotNull(acc);

...
public class RestaurantJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Restaurant.jsonld");
        assertNotNull(is, "Restaurant.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Restaurant acc = mapper.readValue(jsonld, Restaurant.class);
        assertNotNull(acc);

...
public class CollectionPageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CollectionPage.jsonld");
        assertNotNull(is, "CollectionPage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CollectionPage acc = mapper.readValue(jsonld, CollectionPage.class);
        assertNotNull(acc);

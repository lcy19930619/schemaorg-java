...
public class ReservationPackageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReservationPackage.jsonld");
        assertNotNull(is, "ReservationPackage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReservationPackage acc = mapper.readValue(jsonld, ReservationPackage.class);
        assertNotNull(acc);

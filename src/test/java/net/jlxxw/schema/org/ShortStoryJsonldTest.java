...
public class ShortStoryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ShortStory.jsonld");
        assertNotNull(is, "ShortStory.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ShortStory acc = mapper.readValue(jsonld, ShortStory.class);
        assertNotNull(acc);

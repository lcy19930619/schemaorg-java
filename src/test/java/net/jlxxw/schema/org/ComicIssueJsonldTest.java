...
public class ComicIssueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ComicIssue.jsonld");
        assertNotNull(is, "ComicIssue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ComicIssue acc = mapper.readValue(jsonld, ComicIssue.class);
        assertNotNull(acc);

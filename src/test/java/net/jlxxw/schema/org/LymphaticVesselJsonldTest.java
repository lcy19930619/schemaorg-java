...
public class LymphaticVesselJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LymphaticVessel.jsonld");
        assertNotNull(is, "LymphaticVessel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LymphaticVessel acc = mapper.readValue(jsonld, LymphaticVessel.class);
        assertNotNull(acc);

...
public class SaleEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SaleEvent.jsonld");
        assertNotNull(is, "SaleEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SaleEvent acc = mapper.readValue(jsonld, SaleEvent.class);
        assertNotNull(acc);

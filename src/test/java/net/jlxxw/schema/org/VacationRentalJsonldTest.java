...
public class VacationRentalJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VacationRental.jsonld");
        assertNotNull(is, "VacationRental.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VacationRental acc = mapper.readValue(jsonld, VacationRental.class);
        assertNotNull(acc);

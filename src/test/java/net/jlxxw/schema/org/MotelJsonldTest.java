...
public class MotelJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Motel.jsonld");
        assertNotNull(is, "Motel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Motel acc = mapper.readValue(jsonld, Motel.class);
        assertNotNull(acc);

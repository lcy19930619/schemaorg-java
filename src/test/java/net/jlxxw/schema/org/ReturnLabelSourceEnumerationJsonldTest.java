...
public class ReturnLabelSourceEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReturnLabelSourceEnumeration.jsonld");
        assertNotNull(is, "ReturnLabelSourceEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReturnLabelSourceEnumeration acc = mapper.readValue(jsonld, ReturnLabelSourceEnumeration.class);
        assertNotNull(acc);

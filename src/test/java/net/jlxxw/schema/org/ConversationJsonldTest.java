...
public class ConversationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Conversation.jsonld");
        assertNotNull(is, "Conversation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Conversation acc = mapper.readValue(jsonld, Conversation.class);
        assertNotNull(acc);

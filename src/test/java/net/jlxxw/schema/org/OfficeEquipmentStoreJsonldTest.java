...
public class OfficeEquipmentStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OfficeEquipmentStore.jsonld");
        assertNotNull(is, "OfficeEquipmentStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OfficeEquipmentStore acc = mapper.readValue(jsonld, OfficeEquipmentStore.class);
        assertNotNull(acc);

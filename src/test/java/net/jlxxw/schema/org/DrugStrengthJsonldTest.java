...
public class DrugStrengthJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DrugStrength.jsonld");
        assertNotNull(is, "DrugStrength.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DrugStrength acc = mapper.readValue(jsonld, DrugStrength.class);
        assertNotNull(acc);

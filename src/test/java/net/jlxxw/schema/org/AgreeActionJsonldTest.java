...
public class AgreeActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AgreeAction.jsonld");
        assertNotNull(is, "AgreeAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AgreeAction acc = mapper.readValue(jsonld, AgreeAction.class);
        assertNotNull(acc);

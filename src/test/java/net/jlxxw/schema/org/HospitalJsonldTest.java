...
public class HospitalJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Hospital.jsonld");
        assertNotNull(is, "Hospital.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Hospital acc = mapper.readValue(jsonld, Hospital.class);
        assertNotNull(acc);

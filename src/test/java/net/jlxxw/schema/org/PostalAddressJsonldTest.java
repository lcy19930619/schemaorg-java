...
public class PostalAddressJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PostalAddress.jsonld");
        assertNotNull(is, "PostalAddress.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PostalAddress acc = mapper.readValue(jsonld, PostalAddress.class);
        assertNotNull(acc);

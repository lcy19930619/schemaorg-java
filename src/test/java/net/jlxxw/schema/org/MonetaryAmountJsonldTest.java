...
public class MonetaryAmountJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MonetaryAmount.jsonld");
        assertNotNull(is, "MonetaryAmount.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MonetaryAmount acc = mapper.readValue(jsonld, MonetaryAmount.class);
        assertNotNull(acc);

...
public class ExhibitionEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ExhibitionEvent.jsonld");
        assertNotNull(is, "ExhibitionEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ExhibitionEvent acc = mapper.readValue(jsonld, ExhibitionEvent.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ExhibitionEvent obj = new ExhibitionEvent();
        assertEquals("schema:ExhibitionEvent", obj.getId());

...
public class TradeActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TradeAction.jsonld");
        assertNotNull(is, "TradeAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TradeAction acc = mapper.readValue(jsonld, TradeAction.class);
        assertNotNull(acc);

...
public class AssessActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AssessAction.jsonld");
        assertNotNull(is, "AssessAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AssessAction acc = mapper.readValue(jsonld, AssessAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        AssessAction obj = new AssessAction();
        assertEquals("schema:AssessAction", obj.getId());

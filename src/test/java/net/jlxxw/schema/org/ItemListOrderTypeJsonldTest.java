...
public class ItemListOrderTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ItemListOrderType.jsonld");
        assertNotNull(is, "ItemListOrderType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ItemListOrderType acc = mapper.readValue(jsonld, ItemListOrderType.class);
        assertNotNull(acc);

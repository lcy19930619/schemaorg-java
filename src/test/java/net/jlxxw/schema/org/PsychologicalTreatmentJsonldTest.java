...
public class PsychologicalTreatmentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PsychologicalTreatment.jsonld");
        assertNotNull(is, "PsychologicalTreatment.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PsychologicalTreatment acc = mapper.readValue(jsonld, PsychologicalTreatment.class);
        assertNotNull(acc);

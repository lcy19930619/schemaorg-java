...
public class DefenceEstablishmentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DefenceEstablishment.jsonld");
        assertNotNull(is, "DefenceEstablishment.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DefenceEstablishment acc = mapper.readValue(jsonld, DefenceEstablishment.class);
        assertNotNull(acc);

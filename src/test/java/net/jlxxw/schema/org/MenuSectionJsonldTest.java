...
public class MenuSectionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MenuSection.jsonld");
        assertNotNull(is, "MenuSection.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MenuSection acc = mapper.readValue(jsonld, MenuSection.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MenuSection obj = new MenuSection();
        assertEquals("schema:MenuSection", obj.getId());

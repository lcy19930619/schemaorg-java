...
public class AudienceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Audience.jsonld");
        assertNotNull(is, "Audience.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Audience acc = mapper.readValue(jsonld, Audience.class);
        assertNotNull(acc);

...
public class JobPostingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("JobPosting.jsonld");
        assertNotNull(is, "JobPosting.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        JobPosting acc = mapper.readValue(jsonld, JobPosting.class);
        assertNotNull(acc);

...
public class ToyStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ToyStore.jsonld");
        assertNotNull(is, "ToyStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ToyStore acc = mapper.readValue(jsonld, ToyStore.class);
        assertNotNull(acc);

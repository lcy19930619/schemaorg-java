...
public class DataDownloadJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DataDownload.jsonld");
        assertNotNull(is, "DataDownload.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DataDownload acc = mapper.readValue(jsonld, DataDownload.class);
        assertNotNull(acc);

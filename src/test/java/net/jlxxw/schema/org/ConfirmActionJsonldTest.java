...
public class ConfirmActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ConfirmAction.jsonld");
        assertNotNull(is, "ConfirmAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ConfirmAction acc = mapper.readValue(jsonld, ConfirmAction.class);
        assertNotNull(acc);

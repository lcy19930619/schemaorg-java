...
public class InteractionCounterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("InteractionCounter.jsonld");
        assertNotNull(is, "InteractionCounter.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        InteractionCounter acc = mapper.readValue(jsonld, InteractionCounter.class);
        assertNotNull(acc);

...
public class BorrowActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BorrowAction.jsonld");
        assertNotNull(is, "BorrowAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BorrowAction acc = mapper.readValue(jsonld, BorrowAction.class);
        assertNotNull(acc);

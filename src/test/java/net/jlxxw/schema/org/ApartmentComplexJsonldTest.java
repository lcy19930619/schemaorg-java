...
public class ApartmentComplexJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ApartmentComplex.jsonld");
        assertNotNull(is, "ApartmentComplex.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ApartmentComplex acc = mapper.readValue(jsonld, ApartmentComplex.class);
        assertNotNull(acc);

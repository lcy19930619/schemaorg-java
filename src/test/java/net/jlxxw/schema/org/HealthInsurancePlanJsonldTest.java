...
public class HealthInsurancePlanJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HealthInsurancePlan.jsonld");
        assertNotNull(is, "HealthInsurancePlan.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HealthInsurancePlan acc = mapper.readValue(jsonld, HealthInsurancePlan.class);
        assertNotNull(acc);

...
public class LikeActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LikeAction.jsonld");
        assertNotNull(is, "LikeAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LikeAction acc = mapper.readValue(jsonld, LikeAction.class);
        assertNotNull(acc);

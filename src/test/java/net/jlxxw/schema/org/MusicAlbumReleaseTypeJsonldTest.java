...
public class MusicAlbumReleaseTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicAlbumReleaseType.jsonld");
        assertNotNull(is, "MusicAlbumReleaseType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicAlbumReleaseType acc = mapper.readValue(jsonld, MusicAlbumReleaseType.class);
        assertNotNull(acc);

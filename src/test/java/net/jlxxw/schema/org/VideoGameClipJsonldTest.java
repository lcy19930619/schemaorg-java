...
public class VideoGameClipJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VideoGameClip.jsonld");
        assertNotNull(is, "VideoGameClip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VideoGameClip acc = mapper.readValue(jsonld, VideoGameClip.class);
        assertNotNull(acc);

...
public class BankOrCreditUnionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BankOrCreditUnion.jsonld");
        assertNotNull(is, "BankOrCreditUnion.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BankOrCreditUnion acc = mapper.readValue(jsonld, BankOrCreditUnion.class);
        assertNotNull(acc);

...
public class PublicationEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PublicationEvent.jsonld");
        assertNotNull(is, "PublicationEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PublicationEvent acc = mapper.readValue(jsonld, PublicationEvent.class);
        assertNotNull(acc);

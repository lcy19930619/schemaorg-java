...
public class ClaimJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Claim.jsonld");
        assertNotNull(is, "Claim.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Claim acc = mapper.readValue(jsonld, Claim.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Claim obj = new Claim();
        assertEquals("schema:Claim", obj.getId());

...
public class GenderTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GenderType.jsonld");
        assertNotNull(is, "GenderType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GenderType acc = mapper.readValue(jsonld, GenderType.class);
        assertNotNull(acc);

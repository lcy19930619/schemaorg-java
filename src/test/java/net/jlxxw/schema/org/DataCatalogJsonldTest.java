...
public class DataCatalogJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DataCatalog.jsonld");
        assertNotNull(is, "DataCatalog.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DataCatalog acc = mapper.readValue(jsonld, DataCatalog.class);
        assertNotNull(acc);

...
public class CivicStructureJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CivicStructure.jsonld");
        assertNotNull(is, "CivicStructure.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CivicStructure acc = mapper.readValue(jsonld, CivicStructure.class);
        assertNotNull(acc);

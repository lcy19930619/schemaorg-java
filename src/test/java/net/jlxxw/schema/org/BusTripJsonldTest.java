...
public class BusTripJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BusTrip.jsonld");
        assertNotNull(is, "BusTrip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BusTrip acc = mapper.readValue(jsonld, BusTrip.class);
        assertNotNull(acc);

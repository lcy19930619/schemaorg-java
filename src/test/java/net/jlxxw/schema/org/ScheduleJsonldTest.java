...
public class ScheduleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Schedule.jsonld");
        assertNotNull(is, "Schedule.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Schedule acc = mapper.readValue(jsonld, Schedule.class);
        assertNotNull(acc);

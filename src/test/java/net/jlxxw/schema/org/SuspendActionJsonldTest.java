...
public class SuspendActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SuspendAction.jsonld");
        assertNotNull(is, "SuspendAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SuspendAction acc = mapper.readValue(jsonld, SuspendAction.class);
        assertNotNull(acc);

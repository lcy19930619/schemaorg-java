...
public class SolveMathActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SolveMathAction.jsonld");
        assertNotNull(is, "SolveMathAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SolveMathAction acc = mapper.readValue(jsonld, SolveMathAction.class);
        assertNotNull(acc);

...
public class ArchiveOrganizationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ArchiveOrganization.jsonld");
        assertNotNull(is, "ArchiveOrganization.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ArchiveOrganization acc = mapper.readValue(jsonld, ArchiveOrganization.class);
        assertNotNull(acc);

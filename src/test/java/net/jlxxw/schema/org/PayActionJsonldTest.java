...
public class PayActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PayAction.jsonld");
        assertNotNull(is, "PayAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PayAction acc = mapper.readValue(jsonld, PayAction.class);
        assertNotNull(acc);

...
public class MediaSubscriptionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MediaSubscription.jsonld");
        assertNotNull(is, "MediaSubscription.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MediaSubscription acc = mapper.readValue(jsonld, MediaSubscription.class);
        assertNotNull(acc);

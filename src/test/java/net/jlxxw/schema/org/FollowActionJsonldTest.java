...
public class FollowActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FollowAction.jsonld");
        assertNotNull(is, "FollowAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FollowAction acc = mapper.readValue(jsonld, FollowAction.class);
        assertNotNull(acc);

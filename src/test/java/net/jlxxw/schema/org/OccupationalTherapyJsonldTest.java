...
public class OccupationalTherapyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OccupationalTherapy.jsonld");
        assertNotNull(is, "OccupationalTherapy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OccupationalTherapy acc = mapper.readValue(jsonld, OccupationalTherapy.class);
        assertNotNull(acc);

...
public class CreateActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CreateAction.jsonld");
        assertNotNull(is, "CreateAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CreateAction acc = mapper.readValue(jsonld, CreateAction.class);
        assertNotNull(acc);

...
public class SomeProductsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SomeProducts.jsonld");
        assertNotNull(is, "SomeProducts.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SomeProducts acc = mapper.readValue(jsonld, SomeProducts.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        SomeProducts obj = new SomeProducts();
        assertEquals("schema:SomeProducts", obj.getId());

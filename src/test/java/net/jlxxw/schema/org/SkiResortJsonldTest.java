...
public class SkiResortJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SkiResort.jsonld");
        assertNotNull(is, "SkiResort.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SkiResort acc = mapper.readValue(jsonld, SkiResort.class);
        assertNotNull(acc);

...
public class SpecialAnnouncementJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SpecialAnnouncement.jsonld");
        assertNotNull(is, "SpecialAnnouncement.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SpecialAnnouncement acc = mapper.readValue(jsonld, SpecialAnnouncement.class);
        assertNotNull(acc);

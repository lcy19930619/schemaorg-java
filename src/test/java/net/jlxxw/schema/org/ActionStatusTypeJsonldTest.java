...
public class ActionStatusTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ActionStatusType.jsonld");
        assertNotNull(is, "ActionStatusType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ActionStatusType acc = mapper.readValue(jsonld, ActionStatusType.class);
        assertNotNull(acc);

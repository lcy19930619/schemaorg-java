...
public class AirlineJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Airline.jsonld");
        assertNotNull(is, "Airline.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Airline acc = mapper.readValue(jsonld, Airline.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ...
        assertEquals("schema:Airline", obj.getId());

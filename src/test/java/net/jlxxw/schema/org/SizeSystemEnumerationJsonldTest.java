...
public class SizeSystemEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SizeSystemEnumeration.jsonld");
        assertNotNull(is, "SizeSystemEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SizeSystemEnumeration acc = mapper.readValue(jsonld, SizeSystemEnumeration.class);
        assertNotNull(acc);

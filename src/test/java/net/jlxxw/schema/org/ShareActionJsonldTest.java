...
public class ShareActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ShareAction.jsonld");
        assertNotNull(is, "ShareAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ShareAction acc = mapper.readValue(jsonld, ShareAction.class);
        assertNotNull(acc);

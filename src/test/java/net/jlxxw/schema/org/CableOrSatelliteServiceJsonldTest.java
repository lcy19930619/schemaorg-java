...
public class CableOrSatelliteServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CableOrSatelliteService.jsonld");
        assertNotNull(is, "CableOrSatelliteService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CableOrSatelliteService acc = mapper.readValue(jsonld, CableOrSatelliteService.class);
        assertNotNull(acc);

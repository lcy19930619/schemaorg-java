...
public class APIReferenceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("APIReference.jsonld");
        assertNotNull(is, "APIReference.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        APIReference acc = mapper.readValue(jsonld, APIReference.class);
        assertNotNull(acc);

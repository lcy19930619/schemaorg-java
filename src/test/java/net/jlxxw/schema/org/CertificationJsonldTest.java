...
public class CertificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Certification.jsonld");
        assertNotNull(is, "Certification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Certification acc = mapper.readValue(jsonld, Certification.class);
        assertNotNull(acc);

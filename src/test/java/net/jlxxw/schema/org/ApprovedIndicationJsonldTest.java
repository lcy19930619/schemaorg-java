...
public class ApprovedIndicationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ApprovedIndication.jsonld");
        assertNotNull(is, "ApprovedIndication.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ApprovedIndication acc = mapper.readValue(jsonld, ApprovedIndication.class);
        assertNotNull(acc);

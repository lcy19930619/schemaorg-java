...
public class EatActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EatAction.jsonld");
        assertNotNull(is, "EatAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EatAction acc = mapper.readValue(jsonld, EatAction.class);
        assertNotNull(acc);

...
public class OfferForPurchaseJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OfferForPurchase.jsonld");
        assertNotNull(is, "OfferForPurchase.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OfferForPurchase acc = mapper.readValue(jsonld, OfferForPurchase.class);
        assertNotNull(acc);

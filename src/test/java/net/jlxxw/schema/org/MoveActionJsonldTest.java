...
public class MoveActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MoveAction.jsonld");
        assertNotNull(is, "MoveAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MoveAction acc = mapper.readValue(jsonld, MoveAction.class);
        assertNotNull(acc);

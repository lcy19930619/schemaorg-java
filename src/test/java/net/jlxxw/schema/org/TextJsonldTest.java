...
public class TextJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Text.jsonld");
        assertNotNull(is, "Text.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Text acc = mapper.readValue(jsonld, Text.class);
        assertNotNull(acc);

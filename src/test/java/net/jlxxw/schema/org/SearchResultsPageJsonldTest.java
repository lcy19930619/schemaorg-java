...
public class SearchResultsPageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SearchResultsPage.jsonld");
        assertNotNull(is, "SearchResultsPage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SearchResultsPage acc = mapper.readValue(jsonld, SearchResultsPage.class);
        assertNotNull(acc);

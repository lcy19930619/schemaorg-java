...
public class MusicStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicStore.jsonld");
        assertNotNull(is, "MusicStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicStore acc = mapper.readValue(jsonld, MusicStore.class);
        assertNotNull(acc);

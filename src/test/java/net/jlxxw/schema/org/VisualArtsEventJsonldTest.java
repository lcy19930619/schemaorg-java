...
public class VisualArtsEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VisualArtsEvent.jsonld");
        assertNotNull(is, "VisualArtsEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VisualArtsEvent acc = mapper.readValue(jsonld, VisualArtsEvent.class);
        assertNotNull(acc);

...
public class AdultEntertainmentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AdultEntertainment.jsonld");
        assertNotNull(is, "AdultEntertainment.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AdultEntertainment acc = mapper.readValue(jsonld, AdultEntertainment.class);
        assertNotNull(acc);

...
public class GovernmentOfficeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GovernmentOffice.jsonld");
        assertNotNull(is, "GovernmentOffice.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GovernmentOffice acc = mapper.readValue(jsonld, GovernmentOffice.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        GovernmentOffice obj = new GovernmentOffice();
        assertEquals("schema:GovernmentOffice", obj.getId());

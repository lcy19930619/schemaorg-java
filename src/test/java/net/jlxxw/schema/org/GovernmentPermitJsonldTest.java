...
public class GovernmentPermitJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GovernmentPermit.jsonld");
        assertNotNull(is, "GovernmentPermit.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GovernmentPermit acc = mapper.readValue(jsonld, GovernmentPermit.class);
        assertNotNull(acc);

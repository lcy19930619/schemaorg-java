...
public class PersonJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Person.jsonld");
        assertNotNull(is, "Person.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Person acc = mapper.readValue(jsonld, Person.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ...
        assertEquals("schema:Person", obj.getId());

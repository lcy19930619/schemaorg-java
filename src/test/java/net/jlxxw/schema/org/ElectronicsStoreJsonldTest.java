...
public class ElectronicsStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ElectronicsStore.jsonld");
        assertNotNull(is, "ElectronicsStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ElectronicsStore acc = mapper.readValue(jsonld, ElectronicsStore.class);
        assertNotNull(acc);

...
public class FestivalJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Festival.jsonld");
        assertNotNull(is, "Festival.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Festival acc = mapper.readValue(jsonld, Festival.class);
        assertNotNull(acc);

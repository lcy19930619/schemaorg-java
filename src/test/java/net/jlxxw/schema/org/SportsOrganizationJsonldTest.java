...
public class SportsOrganizationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SportsOrganization.jsonld");
        assertNotNull(is, "SportsOrganization.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SportsOrganization acc = mapper.readValue(jsonld, SportsOrganization.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        SportsOrganization obj = new SportsOrganization();
        assertEquals("schema:SportsOrganization", obj.getId());

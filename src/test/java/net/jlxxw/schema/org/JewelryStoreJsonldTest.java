...
public class JewelryStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("JewelryStore.jsonld");
        assertNotNull(is, "JewelryStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        JewelryStore acc = mapper.readValue(jsonld, JewelryStore.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        JewelryStore obj = new JewelryStore();
        assertEquals("schema:JewelryStore", obj.getId());

...
public class PharmacyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Pharmacy.jsonld");
        assertNotNull(is, "Pharmacy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Pharmacy acc = mapper.readValue(jsonld, Pharmacy.class);
        assertNotNull(acc);

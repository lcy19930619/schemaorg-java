...
public class DrugPrescriptionStatusJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DrugPrescriptionStatus.jsonld");
        assertNotNull(is, "DrugPrescriptionStatus.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DrugPrescriptionStatus acc = mapper.readValue(jsonld, DrugPrescriptionStatus.class);
        assertNotNull(acc);

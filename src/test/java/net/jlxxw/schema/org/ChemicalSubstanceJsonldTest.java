...
public class ChemicalSubstanceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ChemicalSubstance.jsonld");
        assertNotNull(is, "ChemicalSubstance.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ChemicalSubstance acc = mapper.readValue(jsonld, ChemicalSubstance.class);
        assertNotNull(acc);

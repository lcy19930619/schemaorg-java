...
public class HealthPlanFormularyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HealthPlanFormulary.jsonld");
        assertNotNull(is, "HealthPlanFormulary.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HealthPlanFormulary acc = mapper.readValue(jsonld, HealthPlanFormulary.class);
        assertNotNull(acc);

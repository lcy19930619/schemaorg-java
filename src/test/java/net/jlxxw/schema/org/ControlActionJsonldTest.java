...
public class ControlActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ControlAction.jsonld");
        assertNotNull(is, "ControlAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ControlAction acc = mapper.readValue(jsonld, ControlAction.class);
        assertNotNull(acc);

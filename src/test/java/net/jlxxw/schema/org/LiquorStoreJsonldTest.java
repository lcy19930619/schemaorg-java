...
public class LiquorStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LiquorStore.jsonld");
        assertNotNull(is, "LiquorStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LiquorStore acc = mapper.readValue(jsonld, LiquorStore.class);
        assertNotNull(acc);

...
public class LeaveActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LeaveAction.jsonld");
        assertNotNull(is, "LeaveAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LeaveAction acc = mapper.readValue(jsonld, LeaveAction.class);
        assertNotNull(acc);

...
public class PublicSwimmingPoolJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PublicSwimmingPool.jsonld");
        assertNotNull(is, "PublicSwimmingPool.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PublicSwimmingPool acc = mapper.readValue(jsonld, PublicSwimmingPool.class);
        assertNotNull(acc);

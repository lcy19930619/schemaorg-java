...
public class MoneyTransferJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MoneyTransfer.jsonld");
        assertNotNull(is, "MoneyTransfer.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MoneyTransfer acc = mapper.readValue(jsonld, MoneyTransfer.class);
        assertNotNull(acc);

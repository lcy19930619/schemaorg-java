...
public class AcceptActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AcceptAction.jsonld");
        assertNotNull(is, "AcceptAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AcceptAction acc = mapper.readValue(jsonld, AcceptAction.class);
        assertNotNull(acc);

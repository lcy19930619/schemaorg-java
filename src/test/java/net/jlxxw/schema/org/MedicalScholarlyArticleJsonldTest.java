...
public class MedicalScholarlyArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalScholarlyArticle.jsonld");
        assertNotNull(is, "MedicalScholarlyArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalScholarlyArticle acc = mapper.readValue(jsonld, MedicalScholarlyArticle.class);
        assertNotNull(acc);

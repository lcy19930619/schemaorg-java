...
public class PriceTypeEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PriceTypeEnumeration.jsonld");
        assertNotNull(is, "PriceTypeEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PriceTypeEnumeration acc = mapper.readValue(jsonld, PriceTypeEnumeration.class);
        assertNotNull(acc);

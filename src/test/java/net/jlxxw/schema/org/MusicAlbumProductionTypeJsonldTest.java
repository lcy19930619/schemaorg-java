...
public class MusicAlbumProductionTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicAlbumProductionType.jsonld");
        assertNotNull(is, "MusicAlbumProductionType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicAlbumProductionType acc = mapper.readValue(jsonld, MusicAlbumProductionType.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MusicAlbumProductionType obj = new MusicAlbumProductionType();
        assertEquals("schema:MusicAlbumProductionType", obj.getId());

...
public class PhysicianJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Physician.jsonld");
        assertNotNull(is, "Physician.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Physician acc = mapper.readValue(jsonld, Physician.class);
        assertNotNull(acc);

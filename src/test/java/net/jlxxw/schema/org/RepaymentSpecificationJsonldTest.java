...
public class RepaymentSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RepaymentSpecification.jsonld");
        assertNotNull(is, "RepaymentSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RepaymentSpecification acc = mapper.readValue(jsonld, RepaymentSpecification.class);
        assertNotNull(acc);

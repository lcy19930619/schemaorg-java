...
public class CheckoutPageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CheckoutPage.jsonld");
        assertNotNull(is, "CheckoutPage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CheckoutPage acc = mapper.readValue(jsonld, CheckoutPage.class);
        assertNotNull(acc);

...
public class VitalSignJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VitalSign.jsonld");
        assertNotNull(is, "VitalSign.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VitalSign acc = mapper.readValue(jsonld, VitalSign.class);
        assertNotNull(acc);

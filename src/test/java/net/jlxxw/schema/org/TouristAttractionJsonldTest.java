...
public class TouristAttractionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TouristAttraction.jsonld");
        assertNotNull(is, "TouristAttraction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TouristAttraction acc = mapper.readValue(jsonld, TouristAttraction.class);
        assertNotNull(acc);

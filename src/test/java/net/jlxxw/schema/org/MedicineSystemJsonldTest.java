...
public class MedicineSystemJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicineSystem.jsonld");
        assertNotNull(is, "MedicineSystem.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicineSystem acc = mapper.readValue(jsonld, MedicineSystem.class);
        assertNotNull(acc);

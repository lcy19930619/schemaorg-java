...
public class UseActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UseAction.jsonld");
        assertNotNull(is, "UseAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UseAction acc = mapper.readValue(jsonld, UseAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        UseAction obj = new UseAction();
        assertEquals("schema:UseAction", obj.getId());

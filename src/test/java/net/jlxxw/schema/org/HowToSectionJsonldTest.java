...
public class HowToSectionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HowToSection.jsonld");
        assertNotNull(is, "HowToSection.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HowToSection acc = mapper.readValue(jsonld, HowToSection.class);
        assertNotNull(acc);

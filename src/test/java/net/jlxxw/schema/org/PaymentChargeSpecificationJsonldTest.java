...
public class PaymentChargeSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PaymentChargeSpecification.jsonld");
        assertNotNull(is, "PaymentChargeSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PaymentChargeSpecification acc = mapper.readValue(jsonld, PaymentChargeSpecification.class);
        assertNotNull(acc);

...
public class ReactActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReactAction.jsonld");
        assertNotNull(is, "ReactAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReactAction acc = mapper.readValue(jsonld, ReactAction.class);
        assertNotNull(acc);

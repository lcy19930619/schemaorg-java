...
public class DigitalPlatformEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DigitalPlatformEnumeration.jsonld");
        assertNotNull(is, "DigitalPlatformEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DigitalPlatformEnumeration acc = mapper.readValue(jsonld, DigitalPlatformEnumeration.class);
        assertNotNull(acc);

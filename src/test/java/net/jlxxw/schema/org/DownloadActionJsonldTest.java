...
public class DownloadActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DownloadAction.jsonld");
        assertNotNull(is, "DownloadAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DownloadAction acc = mapper.readValue(jsonld, DownloadAction.class);
        assertNotNull(acc);

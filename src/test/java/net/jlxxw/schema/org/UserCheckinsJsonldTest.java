...
public class UserCheckinsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserCheckins.jsonld");
        assertNotNull(is, "UserCheckins.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserCheckins acc = mapper.readValue(jsonld, UserCheckins.class);
        assertNotNull(acc);

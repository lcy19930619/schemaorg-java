...
public class TouristTripJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TouristTrip.jsonld");
        assertNotNull(is, "TouristTrip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TouristTrip acc = mapper.readValue(jsonld, TouristTrip.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        TouristTrip obj = new TouristTrip();
        assertEquals("schema:TouristTrip", obj.getId());

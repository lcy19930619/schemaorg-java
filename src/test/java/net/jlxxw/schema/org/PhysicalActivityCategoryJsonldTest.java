...
public class PhysicalActivityCategoryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PhysicalActivityCategory.jsonld");
        assertNotNull(is, "PhysicalActivityCategory.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PhysicalActivityCategory acc = mapper.readValue(jsonld, PhysicalActivityCategory.class);
        assertNotNull(acc);

...
public class HotelRoomJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HotelRoom.jsonld");
        assertNotNull(is, "HotelRoom.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HotelRoom acc = mapper.readValue(jsonld, HotelRoom.class);
        assertNotNull(acc);

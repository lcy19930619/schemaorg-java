...
public class TheaterEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TheaterEvent.jsonld");
        assertNotNull(is, "TheaterEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TheaterEvent acc = mapper.readValue(jsonld, TheaterEvent.class);
        assertNotNull(acc);

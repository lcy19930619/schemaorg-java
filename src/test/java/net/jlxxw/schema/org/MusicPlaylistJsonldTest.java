...
public class MusicPlaylistJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicPlaylist.jsonld");
        assertNotNull(is, "MusicPlaylist.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicPlaylist acc = mapper.readValue(jsonld, MusicPlaylist.class);
        assertNotNull(acc);

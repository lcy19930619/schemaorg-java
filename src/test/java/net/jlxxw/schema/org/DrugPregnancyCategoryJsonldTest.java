...
public class DrugPregnancyCategoryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DrugPregnancyCategory.jsonld");
        assertNotNull(is, "DrugPregnancyCategory.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DrugPregnancyCategory acc = mapper.readValue(jsonld, DrugPregnancyCategory.class);
        assertNotNull(acc);

...
public class HowToJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HowTo.jsonld");
        assertNotNull(is, "HowTo.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HowTo acc = mapper.readValue(jsonld, HowTo.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        HowTo obj = new HowTo();
        assertEquals("schema:HowTo", obj.getId());

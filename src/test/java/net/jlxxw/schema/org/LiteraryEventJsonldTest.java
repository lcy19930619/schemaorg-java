...
public class LiteraryEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LiteraryEvent.jsonld");
        assertNotNull(is, "LiteraryEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LiteraryEvent acc = mapper.readValue(jsonld, LiteraryEvent.class);
        assertNotNull(acc);

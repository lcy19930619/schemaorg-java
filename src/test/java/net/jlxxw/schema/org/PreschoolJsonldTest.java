...
public class PreschoolJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Preschool.jsonld");
        assertNotNull(is, "Preschool.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Preschool acc = mapper.readValue(jsonld, Preschool.class);
        assertNotNull(acc);

...
public class CheckInActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CheckInAction.jsonld");
        assertNotNull(is, "CheckInAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CheckInAction acc = mapper.readValue(jsonld, CheckInAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        CheckInAction obj = new CheckInAction();
        assertEquals("schema:CheckInAction", obj.getId());

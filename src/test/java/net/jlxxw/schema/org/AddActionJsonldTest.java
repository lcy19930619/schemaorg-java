...
public class AddActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AddAction.jsonld");
        assertNotNull(is, "AddAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AddAction acc = mapper.readValue(jsonld, AddAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        AddAction obj = new AddAction();
        assertEquals("schema:AddAction", obj.getId());

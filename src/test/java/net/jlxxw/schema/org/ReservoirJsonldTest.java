...
public class ReservoirJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Reservoir.jsonld");
        assertNotNull(is, "Reservoir.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Reservoir acc = mapper.readValue(jsonld, Reservoir.class);
        assertNotNull(acc);

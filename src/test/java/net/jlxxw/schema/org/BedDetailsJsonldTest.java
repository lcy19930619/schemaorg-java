...
public class BedDetailsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BedDetails.jsonld");
        assertNotNull(is, "BedDetails.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BedDetails acc = mapper.readValue(jsonld, BedDetails.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        BedDetails obj = new BedDetails();
        assertEquals("schema:BedDetails", obj.getId());

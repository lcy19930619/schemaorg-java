...
public class SurgicalProcedureJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SurgicalProcedure.jsonld");
        assertNotNull(is, "SurgicalProcedure.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SurgicalProcedure acc = mapper.readValue(jsonld, SurgicalProcedure.class);
        assertNotNull(acc);

...
public class BookmarkActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BookmarkAction.jsonld");
        assertNotNull(is, "BookmarkAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BookmarkAction acc = mapper.readValue(jsonld, BookmarkAction.class);
        assertNotNull(acc);

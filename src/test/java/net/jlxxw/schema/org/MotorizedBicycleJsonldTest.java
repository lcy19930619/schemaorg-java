...
public class MotorizedBicycleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MotorizedBicycle.jsonld");
        assertNotNull(is, "MotorizedBicycle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MotorizedBicycle acc = mapper.readValue(jsonld, MotorizedBicycle.class);
        assertNotNull(acc);

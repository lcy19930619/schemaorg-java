...
public class AutoBodyShopJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AutoBodyShop.jsonld");
        assertNotNull(is, "AutoBodyShop.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AutoBodyShop acc = mapper.readValue(jsonld, AutoBodyShop.class);
        assertNotNull(acc);

...
public class ResearchProjectJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ResearchProject.jsonld");
        assertNotNull(is, "ResearchProject.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ResearchProject acc = mapper.readValue(jsonld, ResearchProject.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ResearchProject obj = new ResearchProject();
        assertEquals("schema:ResearchProject", obj.getId());

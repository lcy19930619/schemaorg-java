...
public class MenuItemJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MenuItem.jsonld");
        assertNotNull(is, "MenuItem.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MenuItem acc = mapper.readValue(jsonld, MenuItem.class);
        assertNotNull(acc);

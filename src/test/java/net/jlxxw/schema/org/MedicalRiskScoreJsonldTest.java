...
public class MedicalRiskScoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalRiskScore.jsonld");
        assertNotNull(is, "MedicalRiskScore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalRiskScore acc = mapper.readValue(jsonld, MedicalRiskScore.class);
        assertNotNull(acc);

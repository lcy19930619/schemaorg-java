...
public class TrackActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TrackAction.jsonld");
        assertNotNull(is, "TrackAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TrackAction acc = mapper.readValue(jsonld, TrackAction.class);
        assertNotNull(acc);

...
public class DefinedTermSetJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DefinedTermSet.jsonld");
        assertNotNull(is, "DefinedTermSet.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DefinedTermSet acc = mapper.readValue(jsonld, DefinedTermSet.class);
        assertNotNull(acc);

...
public class MathSolverJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MathSolver.jsonld");
        assertNotNull(is, "MathSolver.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MathSolver acc = mapper.readValue(jsonld, MathSolver.class);
        assertNotNull(acc);

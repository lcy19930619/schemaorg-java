...
public class WearableSizeSystemEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WearableSizeSystemEnumeration.jsonld");
        assertNotNull(is, "WearableSizeSystemEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WearableSizeSystemEnumeration acc = mapper.readValue(jsonld, WearableSizeSystemEnumeration.class);
        assertNotNull(acc);

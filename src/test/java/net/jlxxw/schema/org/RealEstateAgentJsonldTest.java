...
public class RealEstateAgentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RealEstateAgent.jsonld");
        assertNotNull(is, "RealEstateAgent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RealEstateAgent acc = mapper.readValue(jsonld, RealEstateAgent.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        RealEstateAgent obj = new RealEstateAgent();
        assertEquals("schema:RealEstateAgent", obj.getId());

...
public class PetStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PetStore.jsonld");
        assertNotNull(is, "PetStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PetStore acc = mapper.readValue(jsonld, PetStore.class);
        assertNotNull(acc);

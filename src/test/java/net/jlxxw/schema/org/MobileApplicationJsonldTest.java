...
public class MobileApplicationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MobileApplication.jsonld");
        assertNotNull(is, "MobileApplication.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MobileApplication acc = mapper.readValue(jsonld, MobileApplication.class);
        assertNotNull(acc);

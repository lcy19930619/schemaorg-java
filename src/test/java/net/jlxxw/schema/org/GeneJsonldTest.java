...
public class GeneJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Gene.jsonld");
        assertNotNull(is, "Gene.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Gene acc = mapper.readValue(jsonld, Gene.class);
        assertNotNull(acc);

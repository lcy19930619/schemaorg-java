...
public class ScheduleActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ScheduleAction.jsonld");
        assertNotNull(is, "ScheduleAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ScheduleAction acc = mapper.readValue(jsonld, ScheduleAction.class);
        assertNotNull(acc);

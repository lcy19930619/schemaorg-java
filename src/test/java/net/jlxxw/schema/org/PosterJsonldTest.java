...
public class PosterJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Poster.jsonld");
        assertNotNull(is, "Poster.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Poster acc = mapper.readValue(jsonld, Poster.class);
        assertNotNull(acc);

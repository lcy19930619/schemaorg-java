...
public class SingleFamilyResidenceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SingleFamilyResidence.jsonld");
        assertNotNull(is, "SingleFamilyResidence.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SingleFamilyResidence acc = mapper.readValue(jsonld, SingleFamilyResidence.class);
        assertNotNull(acc);

...
public class BrokerageAccountJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BrokerageAccount.jsonld");
        assertNotNull(is, "BrokerageAccount.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BrokerageAccount acc = mapper.readValue(jsonld, BrokerageAccount.class);
        assertNotNull(acc);

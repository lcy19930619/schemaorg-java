...
public class SeaBodyOfWaterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SeaBodyOfWater.jsonld");
        assertNotNull(is, "SeaBodyOfWater.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SeaBodyOfWater acc = mapper.readValue(jsonld, SeaBodyOfWater.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        SeaBodyOfWater obj = new SeaBodyOfWater();
        assertEquals("schema:SeaBodyOfWater", obj.getId());

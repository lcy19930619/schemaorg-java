...
public class MessageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Message.jsonld");
        assertNotNull(is, "Message.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Message acc = mapper.readValue(jsonld, Message.class);
        assertNotNull(acc);

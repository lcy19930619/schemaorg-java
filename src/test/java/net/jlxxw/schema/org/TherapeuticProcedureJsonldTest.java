...
public class TherapeuticProcedureJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TherapeuticProcedure.jsonld");
        assertNotNull(is, "TherapeuticProcedure.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TherapeuticProcedure acc = mapper.readValue(jsonld, TherapeuticProcedure.class);
        assertNotNull(acc);

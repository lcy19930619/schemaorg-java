...
public class BedTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BedType.jsonld");
        assertNotNull(is, "BedType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BedType acc = mapper.readValue(jsonld, BedType.class);
        assertNotNull(acc);

...
public class DislikeActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DislikeAction.jsonld");
        assertNotNull(is, "DislikeAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DislikeAction acc = mapper.readValue(jsonld, DislikeAction.class);
        assertNotNull(acc);

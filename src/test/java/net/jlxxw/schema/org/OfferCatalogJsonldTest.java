...
public class OfferCatalogJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OfferCatalog.jsonld");
        assertNotNull(is, "OfferCatalog.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OfferCatalog acc = mapper.readValue(jsonld, OfferCatalog.class);
        assertNotNull(acc);

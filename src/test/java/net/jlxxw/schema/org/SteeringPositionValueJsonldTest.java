...
public class SteeringPositionValueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SteeringPositionValue.jsonld");
        assertNotNull(is, "SteeringPositionValue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SteeringPositionValue acc = mapper.readValue(jsonld, SteeringPositionValue.class);
        assertNotNull(acc);

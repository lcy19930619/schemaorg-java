...
public class MortgageLoanJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MortgageLoan.jsonld");
        assertNotNull(is, "MortgageLoan.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MortgageLoan acc = mapper.readValue(jsonld, MortgageLoan.class);
        assertNotNull(acc);

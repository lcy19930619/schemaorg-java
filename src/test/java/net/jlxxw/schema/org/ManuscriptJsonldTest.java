...
public class ManuscriptJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Manuscript.jsonld");
        assertNotNull(is, "Manuscript.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Manuscript acc = mapper.readValue(jsonld, Manuscript.class);
        assertNotNull(acc);

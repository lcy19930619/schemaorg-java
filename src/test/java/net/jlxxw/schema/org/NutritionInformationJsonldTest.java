...
public class NutritionInformationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("NutritionInformation.jsonld");
        assertNotNull(is, "NutritionInformation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        NutritionInformation acc = mapper.readValue(jsonld, NutritionInformation.class);
        assertNotNull(acc);

...
public class ApplyActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ApplyAction.jsonld");
        assertNotNull(is, "ApplyAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ApplyAction acc = mapper.readValue(jsonld, ApplyAction.class);
        assertNotNull(acc);

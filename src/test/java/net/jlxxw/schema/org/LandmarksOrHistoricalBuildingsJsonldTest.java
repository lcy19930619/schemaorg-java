...
public class LandmarksOrHistoricalBuildingsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LandmarksOrHistoricalBuildings.jsonld");
        assertNotNull(is, "LandmarksOrHistoricalBuildings.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LandmarksOrHistoricalBuildings acc = mapper.readValue(jsonld, LandmarksOrHistoricalBuildings.class);
        assertNotNull(acc);

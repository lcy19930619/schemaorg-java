...
public class JointJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Joint.jsonld");
        assertNotNull(is, "Joint.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Joint acc = mapper.readValue(jsonld, Joint.class);
        assertNotNull(acc);

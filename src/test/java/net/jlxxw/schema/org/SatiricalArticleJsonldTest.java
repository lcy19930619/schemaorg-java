...
public class SatiricalArticleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SatiricalArticle.jsonld");
        assertNotNull(is, "SatiricalArticle.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SatiricalArticle acc = mapper.readValue(jsonld, SatiricalArticle.class);
        assertNotNull(acc);

...
public class TrainReservationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TrainReservation.jsonld");
        assertNotNull(is, "TrainReservation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TrainReservation acc = mapper.readValue(jsonld, TrainReservation.class);
        assertNotNull(acc);

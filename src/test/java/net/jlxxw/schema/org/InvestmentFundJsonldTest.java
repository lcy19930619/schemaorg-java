...
public class InvestmentFundJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("InvestmentFund.jsonld");
        assertNotNull(is, "InvestmentFund.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        InvestmentFund acc = mapper.readValue(jsonld, InvestmentFund.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        InvestmentFund obj = new InvestmentFund();
        assertEquals("schema:InvestmentFund", obj.getId());

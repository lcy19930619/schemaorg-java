...
public class PaintActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PaintAction.jsonld");
        assertNotNull(is, "PaintAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PaintAction acc = mapper.readValue(jsonld, PaintAction.class);
        assertNotNull(acc);

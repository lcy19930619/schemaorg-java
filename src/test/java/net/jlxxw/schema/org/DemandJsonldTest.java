...
public class DemandJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Demand.jsonld");
        assertNotNull(is, "Demand.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Demand acc = mapper.readValue(jsonld, Demand.class);
        assertNotNull(acc);

...
public class HowToDirectionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HowToDirection.jsonld");
        assertNotNull(is, "HowToDirection.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HowToDirection acc = mapper.readValue(jsonld, HowToDirection.class);
        assertNotNull(acc);

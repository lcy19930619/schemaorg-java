...
public class EducationalOccupationalCredentialJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EducationalOccupationalCredential.jsonld");
        assertNotNull(is, "EducationalOccupationalCredential.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EducationalOccupationalCredential acc = mapper.readValue(jsonld, EducationalOccupationalCredential.class);
        assertNotNull(acc);

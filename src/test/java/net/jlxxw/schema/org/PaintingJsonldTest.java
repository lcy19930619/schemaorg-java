...
public class PaintingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Painting.jsonld");
        assertNotNull(is, "Painting.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Painting acc = mapper.readValue(jsonld, Painting.class);
        assertNotNull(acc);

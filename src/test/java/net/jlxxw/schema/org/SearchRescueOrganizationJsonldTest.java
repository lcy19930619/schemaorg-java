...
public class SearchRescueOrganizationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SearchRescueOrganization.jsonld");
        assertNotNull(is, "SearchRescueOrganization.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SearchRescueOrganization acc = mapper.readValue(jsonld, SearchRescueOrganization.class);
        assertNotNull(acc);

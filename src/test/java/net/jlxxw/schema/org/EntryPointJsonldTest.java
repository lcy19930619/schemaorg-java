...
public class EntryPointJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EntryPoint.jsonld");
        assertNotNull(is, "EntryPoint.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EntryPoint acc = mapper.readValue(jsonld, EntryPoint.class);
        assertNotNull(acc);

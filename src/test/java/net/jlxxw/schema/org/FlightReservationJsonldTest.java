...
public class FlightReservationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FlightReservation.jsonld");
        assertNotNull(is, "FlightReservation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FlightReservation acc = mapper.readValue(jsonld, FlightReservation.class);
        assertNotNull(acc);

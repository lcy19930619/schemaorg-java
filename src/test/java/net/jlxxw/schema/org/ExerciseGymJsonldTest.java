...
public class ExerciseGymJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ExerciseGym.jsonld");
        assertNotNull(is, "ExerciseGym.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ExerciseGym acc = mapper.readValue(jsonld, ExerciseGym.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ExerciseGym obj = new ExerciseGym();
        assertEquals("schema:ExerciseGym", obj.getId());

...
public class CriticReviewJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CriticReview.jsonld");
        assertNotNull(is, "CriticReview.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CriticReview acc = mapper.readValue(jsonld, CriticReview.class);
        assertNotNull(acc);

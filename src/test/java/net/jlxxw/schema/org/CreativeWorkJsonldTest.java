...
public class CreativeWorkJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CreativeWork.jsonld");
        assertNotNull(is, "CreativeWork.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CreativeWork acc = mapper.readValue(jsonld, CreativeWork.class);
        assertNotNull(acc);

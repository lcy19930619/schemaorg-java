...
public class InformActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("InformAction.jsonld");
        assertNotNull(is, "InformAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        InformAction acc = mapper.readValue(jsonld, InformAction.class);
        assertNotNull(acc);

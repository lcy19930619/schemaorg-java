...
public class MusicReleaseFormatTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicReleaseFormatType.jsonld");
        assertNotNull(is, "MusicReleaseFormatType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicReleaseFormatType acc = mapper.readValue(jsonld, MusicReleaseFormatType.class);
        assertNotNull(acc);

...
public class NonprofitTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("NonprofitType.jsonld");
        assertNotNull(is, "NonprofitType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        NonprofitType acc = mapper.readValue(jsonld, NonprofitType.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        NonprofitType obj = new NonprofitType();
        assertEquals("schema:NonprofitType", obj.getId());

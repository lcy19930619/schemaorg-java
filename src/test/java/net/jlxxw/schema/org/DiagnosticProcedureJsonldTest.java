...
public class DiagnosticProcedureJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DiagnosticProcedure.jsonld");
        assertNotNull(is, "DiagnosticProcedure.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DiagnosticProcedure acc = mapper.readValue(jsonld, DiagnosticProcedure.class);
        assertNotNull(acc);

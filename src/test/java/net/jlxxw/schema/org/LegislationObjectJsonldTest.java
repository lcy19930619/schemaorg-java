...
public class LegislationObjectJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LegislationObject.jsonld");
        assertNotNull(is, "LegislationObject.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LegislationObject acc = mapper.readValue(jsonld, LegislationObject.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        LegislationObject obj = new LegislationObject();
        assertEquals("schema:LegislationObject", obj.getId());

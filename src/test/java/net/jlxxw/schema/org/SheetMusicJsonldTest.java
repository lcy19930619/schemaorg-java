...
public class SheetMusicJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SheetMusic.jsonld");
        assertNotNull(is, "SheetMusic.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SheetMusic acc = mapper.readValue(jsonld, SheetMusic.class);
        assertNotNull(acc);

...
public class MovieJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Movie.jsonld");
        assertNotNull(is, "Movie.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Movie acc = mapper.readValue(jsonld, Movie.class);
        assertNotNull(acc);

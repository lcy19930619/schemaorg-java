...
public class HowToTipJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HowToTip.jsonld");
        assertNotNull(is, "HowToTip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HowToTip acc = mapper.readValue(jsonld, HowToTip.class);
        assertNotNull(acc);

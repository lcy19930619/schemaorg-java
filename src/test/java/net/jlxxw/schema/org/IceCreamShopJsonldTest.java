...
public class IceCreamShopJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("IceCreamShop.jsonld");
        assertNotNull(is, "IceCreamShop.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        IceCreamShop acc = mapper.readValue(jsonld, IceCreamShop.class);
        assertNotNull(acc);

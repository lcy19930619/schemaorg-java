...
public class ResumeActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ResumeAction.jsonld");
        assertNotNull(is, "ResumeAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ResumeAction acc = mapper.readValue(jsonld, ResumeAction.class);
        assertNotNull(acc);

...
public class ParkJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Park.jsonld");
        assertNotNull(is, "Park.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Park acc = mapper.readValue(jsonld, Park.class);
        assertNotNull(acc);

...
public class ProteinJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Protein.jsonld");
        assertNotNull(is, "Protein.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Protein acc = mapper.readValue(jsonld, Protein.class);
        assertNotNull(acc);

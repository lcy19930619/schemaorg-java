...
public class DiscoverActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DiscoverAction.jsonld");
        assertNotNull(is, "DiscoverAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DiscoverAction acc = mapper.readValue(jsonld, DiscoverAction.class);
        assertNotNull(acc);

...
public class TipActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TipAction.jsonld");
        assertNotNull(is, "TipAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TipAction acc = mapper.readValue(jsonld, TipAction.class);
        assertNotNull(acc);

...
public class PoliceStationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PoliceStation.jsonld");
        assertNotNull(is, "PoliceStation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PoliceStation acc = mapper.readValue(jsonld, PoliceStation.class);
        assertNotNull(acc);

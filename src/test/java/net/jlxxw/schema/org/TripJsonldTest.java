...
public class TripJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Trip.jsonld");
        assertNotNull(is, "Trip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Trip acc = mapper.readValue(jsonld, Trip.class);
        assertNotNull(acc);

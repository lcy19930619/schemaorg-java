...
public class OnDemandEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OnDemandEvent.jsonld");
        assertNotNull(is, "OnDemandEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OnDemandEvent acc = mapper.readValue(jsonld, OnDemandEvent.class);
        assertNotNull(acc);

...
public class CampgroundJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Campground.jsonld");
        assertNotNull(is, "Campground.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Campground acc = mapper.readValue(jsonld, Campground.class);
        assertNotNull(acc);

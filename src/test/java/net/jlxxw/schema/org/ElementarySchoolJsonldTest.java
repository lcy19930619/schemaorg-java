...
public class ElementarySchoolJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ElementarySchool.jsonld");
        assertNotNull(is, "ElementarySchool.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ElementarySchool acc = mapper.readValue(jsonld, ElementarySchool.class);
        assertNotNull(acc);

...
public class StructuredValueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("StructuredValue.jsonld");
        assertNotNull(is, "StructuredValue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        StructuredValue acc = mapper.readValue(jsonld, StructuredValue.class);
        assertNotNull(acc);

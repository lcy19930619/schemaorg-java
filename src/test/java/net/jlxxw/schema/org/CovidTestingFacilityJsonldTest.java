...
public class CovidTestingFacilityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CovidTestingFacility.jsonld");
        assertNotNull(is, "CovidTestingFacility.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CovidTestingFacility acc = mapper.readValue(jsonld, CovidTestingFacility.class);
        assertNotNull(acc);

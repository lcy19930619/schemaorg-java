...
public class WebPageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WebPage.jsonld");
        assertNotNull(is, "WebPage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WebPage acc = mapper.readValue(jsonld, WebPage.class);
        assertNotNull(acc);

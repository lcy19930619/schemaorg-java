...
public class FundingAgencyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FundingAgency.jsonld");
        assertNotNull(is, "FundingAgency.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FundingAgency acc = mapper.readValue(jsonld, FundingAgency.class);
        assertNotNull(acc);

...
public class MedicalGuidelineJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalGuideline.jsonld");
        assertNotNull(is, "MedicalGuideline.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalGuideline acc = mapper.readValue(jsonld, MedicalGuideline.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalGuideline obj = new MedicalGuideline();
        assertEquals("schema:MedicalGuideline", obj.getId());

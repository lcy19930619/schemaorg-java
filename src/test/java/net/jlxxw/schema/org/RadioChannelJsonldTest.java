...
public class RadioChannelJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RadioChannel.jsonld");
        assertNotNull(is, "RadioChannel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RadioChannel acc = mapper.readValue(jsonld, RadioChannel.class);
        assertNotNull(acc);

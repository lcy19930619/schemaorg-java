...
public class FindActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FindAction.jsonld");
        assertNotNull(is, "FindAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FindAction acc = mapper.readValue(jsonld, FindAction.class);
        assertNotNull(acc);

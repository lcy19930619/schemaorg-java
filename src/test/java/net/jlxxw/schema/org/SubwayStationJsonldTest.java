...
public class SubwayStationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SubwayStation.jsonld");
        assertNotNull(is, "SubwayStation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SubwayStation acc = mapper.readValue(jsonld, SubwayStation.class);
        assertNotNull(acc);

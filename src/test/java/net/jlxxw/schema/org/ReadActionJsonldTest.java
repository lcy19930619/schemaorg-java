...
public class ReadActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReadAction.jsonld");
        assertNotNull(is, "ReadAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReadAction acc = mapper.readValue(jsonld, ReadAction.class);
        assertNotNull(acc);

...
public class DataFeedItemJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DataFeedItem.jsonld");
        assertNotNull(is, "DataFeedItem.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DataFeedItem acc = mapper.readValue(jsonld, DataFeedItem.class);
        assertNotNull(acc);

...
public class WebPageElementJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WebPageElement.jsonld");
        assertNotNull(is, "WebPageElement.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WebPageElement acc = mapper.readValue(jsonld, WebPageElement.class);
        assertNotNull(acc);

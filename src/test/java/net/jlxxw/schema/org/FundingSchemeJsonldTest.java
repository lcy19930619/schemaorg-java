...
public class FundingSchemeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FundingScheme.jsonld");
        assertNotNull(is, "FundingScheme.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FundingScheme acc = mapper.readValue(jsonld, FundingScheme.class);
        assertNotNull(acc);

...
public class BusinessEntityTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BusinessEntityType.jsonld");
        assertNotNull(is, "BusinessEntityType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BusinessEntityType acc = mapper.readValue(jsonld, BusinessEntityType.class);
        assertNotNull(acc);

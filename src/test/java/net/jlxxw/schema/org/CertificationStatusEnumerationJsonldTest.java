...
public class CertificationStatusEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CertificationStatusEnumeration.jsonld");
        assertNotNull(is, "CertificationStatusEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CertificationStatusEnumeration acc = mapper.readValue(jsonld, CertificationStatusEnumeration.class);
        assertNotNull(acc);

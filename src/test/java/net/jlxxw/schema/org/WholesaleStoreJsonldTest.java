...
public class WholesaleStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WholesaleStore.jsonld");
        assertNotNull(is, "WholesaleStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WholesaleStore acc = mapper.readValue(jsonld, WholesaleStore.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        WholesaleStore obj = new WholesaleStore();
        assertEquals("schema:WholesaleStore", obj.getId());

...
public class TireShopJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TireShop.jsonld");
        assertNotNull(is, "TireShop.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TireShop acc = mapper.readValue(jsonld, TireShop.class);
        assertNotNull(acc);

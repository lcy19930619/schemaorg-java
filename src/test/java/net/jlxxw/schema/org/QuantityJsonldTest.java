...
public class QuantityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Quantity.jsonld");
        assertNotNull(is, "Quantity.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Quantity acc = mapper.readValue(jsonld, Quantity.class);
        assertNotNull(acc);

...
public class MedicalSymptomJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalSymptom.jsonld");
        assertNotNull(is, "MedicalSymptom.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalSymptom acc = mapper.readValue(jsonld, MedicalSymptom.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalSymptom obj = new MedicalSymptom();
        assertEquals("schema:MedicalSymptom", obj.getId());

...
public class PlaygroundJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Playground.jsonld");
        assertNotNull(is, "Playground.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Playground acc = mapper.readValue(jsonld, Playground.class);
        assertNotNull(acc);

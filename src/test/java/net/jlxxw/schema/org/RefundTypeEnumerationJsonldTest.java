...
public class RefundTypeEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RefundTypeEnumeration.jsonld");
        assertNotNull(is, "RefundTypeEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RefundTypeEnumeration acc = mapper.readValue(jsonld, RefundTypeEnumeration.class);
        assertNotNull(acc);

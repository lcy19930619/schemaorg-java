...
public class TableJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Table.jsonld");
        assertNotNull(is, "Table.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Table acc = mapper.readValue(jsonld, Table.class);
        assertNotNull(acc);

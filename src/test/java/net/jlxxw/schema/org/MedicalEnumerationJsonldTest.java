...
public class MedicalEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalEnumeration.jsonld");
        assertNotNull(is, "MedicalEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalEnumeration acc = mapper.readValue(jsonld, MedicalEnumeration.class);
        assertNotNull(acc);

...
public class DigitalDocumentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DigitalDocument.jsonld");
        assertNotNull(is, "DigitalDocument.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DigitalDocument acc = mapper.readValue(jsonld, DigitalDocument.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        DigitalDocument obj = new DigitalDocument();
        assertEquals("schema:DigitalDocument", obj.getId());

...
public class EventJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Event.jsonld");
        assertNotNull(is, "Event.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Event acc = mapper.readValue(jsonld, Event.class);
        assertNotNull(acc);

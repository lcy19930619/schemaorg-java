...
public class HinduTempleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HinduTemple.jsonld");
        assertNotNull(is, "HinduTemple.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HinduTemple acc = mapper.readValue(jsonld, HinduTemple.class);
        assertNotNull(acc);

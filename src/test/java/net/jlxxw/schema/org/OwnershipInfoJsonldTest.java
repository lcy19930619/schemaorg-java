...
public class OwnershipInfoJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OwnershipInfo.jsonld");
        assertNotNull(is, "OwnershipInfo.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OwnershipInfo acc = mapper.readValue(jsonld, OwnershipInfo.class);
        assertNotNull(acc);

...
public class GameJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Game.jsonld");
        assertNotNull(is, "Game.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Game acc = mapper.readValue(jsonld, Game.class);
        assertNotNull(acc);

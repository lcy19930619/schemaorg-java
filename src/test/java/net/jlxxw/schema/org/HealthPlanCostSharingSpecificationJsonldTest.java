...
public class HealthPlanCostSharingSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HealthPlanCostSharingSpecification.jsonld");
        assertNotNull(is, "HealthPlanCostSharingSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HealthPlanCostSharingSpecification acc = mapper.readValue(jsonld, HealthPlanCostSharingSpecification.class);
        assertNotNull(acc);

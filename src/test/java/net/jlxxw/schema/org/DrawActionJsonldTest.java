...
public class DrawActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DrawAction.jsonld");
        assertNotNull(is, "DrawAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DrawAction acc = mapper.readValue(jsonld, DrawAction.class);
        assertNotNull(acc);

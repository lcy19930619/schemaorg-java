...
public class GeoShapeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GeoShape.jsonld");
        assertNotNull(is, "GeoShape.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GeoShape acc = mapper.readValue(jsonld, GeoShape.class);
        assertNotNull(acc);

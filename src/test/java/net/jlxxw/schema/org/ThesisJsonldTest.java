...
public class ThesisJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Thesis.jsonld");
        assertNotNull(is, "Thesis.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Thesis acc = mapper.readValue(jsonld, Thesis.class);
        assertNotNull(acc);

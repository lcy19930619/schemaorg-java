...
public class EventStatusTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EventStatusType.jsonld");
        assertNotNull(is, "EventStatusType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EventStatusType acc = mapper.readValue(jsonld, EventStatusType.class);
        assertNotNull(acc);

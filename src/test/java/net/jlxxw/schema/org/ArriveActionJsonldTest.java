...
public class ArriveActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ArriveAction.jsonld");
        assertNotNull(is, "ArriveAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ArriveAction acc = mapper.readValue(jsonld, ArriveAction.class);
        assertNotNull(acc);

...
public class HealthAspectEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HealthAspectEnumeration.jsonld");
        assertNotNull(is, "HealthAspectEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HealthAspectEnumeration acc = mapper.readValue(jsonld, HealthAspectEnumeration.class);
        assertNotNull(acc);

...
public class ProgramMembershipJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ProgramMembership.jsonld");
        assertNotNull(is, "ProgramMembership.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ProgramMembership acc = mapper.readValue(jsonld, ProgramMembership.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ProgramMembership obj = new ProgramMembership();
        assertEquals("schema:ProgramMembership", obj.getId());

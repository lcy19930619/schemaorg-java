...
public class MosqueJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Mosque.jsonld");
        assertNotNull(is, "Mosque.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Mosque acc = mapper.readValue(jsonld, Mosque.class);
        assertNotNull(acc);

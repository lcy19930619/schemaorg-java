...
public class DoseScheduleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DoseSchedule.jsonld");
        assertNotNull(is, "DoseSchedule.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DoseSchedule acc = mapper.readValue(jsonld, DoseSchedule.class);
        assertNotNull(acc);

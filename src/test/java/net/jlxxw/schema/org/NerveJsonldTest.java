...
public class NerveJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Nerve.jsonld");
        assertNotNull(is, "Nerve.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Nerve acc = mapper.readValue(jsonld, Nerve.class);
        assertNotNull(acc);

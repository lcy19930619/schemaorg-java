...
public class ComedyClubJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ComedyClub.jsonld");
        assertNotNull(is, "ComedyClub.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ComedyClub acc = mapper.readValue(jsonld, ComedyClub.class);
        assertNotNull(acc);

...
public class StadiumOrArenaJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("StadiumOrArena.jsonld");
        assertNotNull(is, "StadiumOrArena.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        StadiumOrArena acc = mapper.readValue(jsonld, StadiumOrArena.class);
        assertNotNull(acc);

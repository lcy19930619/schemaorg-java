...
public class PurchaseTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PurchaseType.jsonld");
        assertNotNull(is, "PurchaseType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PurchaseType acc = mapper.readValue(jsonld, PurchaseType.class);
        assertNotNull(acc);

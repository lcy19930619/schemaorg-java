...
public class WinActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WinAction.jsonld");
        assertNotNull(is, "WinAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WinAction acc = mapper.readValue(jsonld, WinAction.class);
        assertNotNull(acc);

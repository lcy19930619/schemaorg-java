...
public class ArtGalleryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ArtGallery.jsonld");
        assertNotNull(is, "ArtGallery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ArtGallery acc = mapper.readValue(jsonld, ArtGallery.class);
        assertNotNull(acc);

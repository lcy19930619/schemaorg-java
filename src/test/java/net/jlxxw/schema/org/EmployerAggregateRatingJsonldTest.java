...
public class EmployerAggregateRatingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EmployerAggregateRating.jsonld");
        assertNotNull(is, "EmployerAggregateRating.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EmployerAggregateRating acc = mapper.readValue(jsonld, EmployerAggregateRating.class);
        assertNotNull(acc);

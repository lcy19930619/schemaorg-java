...
public class ComputerStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ComputerStore.jsonld");
        assertNotNull(is, "ComputerStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ComputerStore acc = mapper.readValue(jsonld, ComputerStore.class);
        assertNotNull(acc);

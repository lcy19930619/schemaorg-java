...
public class LiveBlogPostingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LiveBlogPosting.jsonld");
        assertNotNull(is, "LiveBlogPosting.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LiveBlogPosting acc = mapper.readValue(jsonld, LiveBlogPosting.class);
        assertNotNull(acc);

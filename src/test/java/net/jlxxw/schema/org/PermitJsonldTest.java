...
public class PermitJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Permit.jsonld");
        assertNotNull(is, "Permit.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Permit acc = mapper.readValue(jsonld, Permit.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ...
        assertEquals("schema:Permit", obj.getId());

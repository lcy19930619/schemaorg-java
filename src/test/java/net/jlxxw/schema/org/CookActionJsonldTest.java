...
public class CookActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CookAction.jsonld");
        assertNotNull(is, "CookAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CookAction acc = mapper.readValue(jsonld, CookAction.class);
        assertNotNull(acc);

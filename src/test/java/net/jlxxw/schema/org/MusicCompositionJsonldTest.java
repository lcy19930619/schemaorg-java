...
public class MusicCompositionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicComposition.jsonld");
        assertNotNull(is, "MusicComposition.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicComposition acc = mapper.readValue(jsonld, MusicComposition.class);
        assertNotNull(acc);

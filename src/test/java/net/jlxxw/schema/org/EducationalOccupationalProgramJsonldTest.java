...
public class EducationalOccupationalProgramJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("EducationalOccupationalProgram.jsonld");
        assertNotNull(is, "EducationalOccupationalProgram.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        EducationalOccupationalProgram acc = mapper.readValue(jsonld, EducationalOccupationalProgram.class);
        assertNotNull(acc);

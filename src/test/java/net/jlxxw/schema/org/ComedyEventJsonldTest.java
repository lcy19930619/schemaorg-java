...
public class ComedyEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ComedyEvent.jsonld");
        assertNotNull(is, "ComedyEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ComedyEvent acc = mapper.readValue(jsonld, ComedyEvent.class);
        assertNotNull(acc);

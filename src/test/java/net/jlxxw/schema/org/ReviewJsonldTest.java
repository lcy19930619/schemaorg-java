...
public class ReviewJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Review.jsonld");
        assertNotNull(is, "Review.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Review acc = mapper.readValue(jsonld, Review.class);
        assertNotNull(acc);

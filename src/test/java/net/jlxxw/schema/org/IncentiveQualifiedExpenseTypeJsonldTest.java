...
public class IncentiveQualifiedExpenseTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("IncentiveQualifiedExpenseType.jsonld");
        assertNotNull(is, "IncentiveQualifiedExpenseType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        IncentiveQualifiedExpenseType acc = mapper.readValue(jsonld, IncentiveQualifiedExpenseType.class);
        assertNotNull(acc);

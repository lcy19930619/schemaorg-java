...
public class MedicalIntangibleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalIntangible.jsonld");
        assertNotNull(is, "MedicalIntangible.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalIntangible acc = mapper.readValue(jsonld, MedicalIntangible.class);
        assertNotNull(acc);

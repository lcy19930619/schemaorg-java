...
public class MassJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Mass.jsonld");
        assertNotNull(is, "Mass.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Mass acc = mapper.readValue(jsonld, Mass.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Mass obj = new Mass();
        assertEquals("schema:Mass", obj.getId());

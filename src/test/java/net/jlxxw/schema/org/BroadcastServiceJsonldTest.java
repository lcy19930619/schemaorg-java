...
public class BroadcastServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BroadcastService.jsonld");
        assertNotNull(is, "BroadcastService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BroadcastService acc = mapper.readValue(jsonld, BroadcastService.class);
        assertNotNull(acc);

...
public class PlayActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PlayAction.jsonld");
        assertNotNull(is, "PlayAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PlayAction acc = mapper.readValue(jsonld, PlayAction.class);
        assertNotNull(acc);

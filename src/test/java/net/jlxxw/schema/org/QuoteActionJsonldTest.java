...
public class QuoteActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("QuoteAction.jsonld");
        assertNotNull(is, "QuoteAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        QuoteAction acc = mapper.readValue(jsonld, QuoteAction.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        QuoteAction obj = new QuoteAction();
        assertEquals("schema:QuoteAction", obj.getId());

...
public class BusinessAudienceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BusinessAudience.jsonld");
        assertNotNull(is, "BusinessAudience.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BusinessAudience acc = mapper.readValue(jsonld, BusinessAudience.class);
        assertNotNull(acc);

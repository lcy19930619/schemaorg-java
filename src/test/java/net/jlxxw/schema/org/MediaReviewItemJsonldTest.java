...
public class MediaReviewItemJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MediaReviewItem.jsonld");
        assertNotNull(is, "MediaReviewItem.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MediaReviewItem acc = mapper.readValue(jsonld, MediaReviewItem.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MediaReviewItem obj = new MediaReviewItem();
        assertEquals("schema:MediaReviewItem", obj.getId());

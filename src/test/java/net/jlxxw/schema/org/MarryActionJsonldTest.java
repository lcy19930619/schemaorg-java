...
public class MarryActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MarryAction.jsonld");
        assertNotNull(is, "MarryAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MarryAction acc = mapper.readValue(jsonld, MarryAction.class);
        assertNotNull(acc);

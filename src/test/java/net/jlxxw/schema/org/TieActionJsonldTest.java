...
public class TieActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TieAction.jsonld");
        assertNotNull(is, "TieAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TieAction acc = mapper.readValue(jsonld, TieAction.class);
        assertNotNull(acc);

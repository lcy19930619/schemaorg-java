...
public class LanguageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Language.jsonld");
        assertNotNull(is, "Language.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Language acc = mapper.readValue(jsonld, Language.class);
        assertNotNull(acc);

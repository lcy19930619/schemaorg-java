...
public class DefinedRegionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DefinedRegion.jsonld");
        assertNotNull(is, "DefinedRegion.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DefinedRegion acc = mapper.readValue(jsonld, DefinedRegion.class);
        assertNotNull(acc);

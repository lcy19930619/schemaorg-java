...
public class WineryJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Winery.jsonld");
        assertNotNull(is, "Winery.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Winery acc = mapper.readValue(jsonld, Winery.class);
        assertNotNull(acc);

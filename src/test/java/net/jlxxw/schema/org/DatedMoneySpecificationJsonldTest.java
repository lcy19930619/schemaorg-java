...
public class DatedMoneySpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DatedMoneySpecification.jsonld");
        assertNotNull(is, "DatedMoneySpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DatedMoneySpecification acc = mapper.readValue(jsonld, DatedMoneySpecification.class);
        assertNotNull(acc);

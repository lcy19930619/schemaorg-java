...
public class ListItemJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ListItem.jsonld");
        assertNotNull(is, "ListItem.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ListItem acc = mapper.readValue(jsonld, ListItem.class);
        assertNotNull(acc);

...
public class ContactPageJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ContactPage.jsonld");
        assertNotNull(is, "ContactPage.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ContactPage acc = mapper.readValue(jsonld, ContactPage.class);
        assertNotNull(acc);

...
public class MotorcycleRepairJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MotorcycleRepair.jsonld");
        assertNotNull(is, "MotorcycleRepair.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MotorcycleRepair acc = mapper.readValue(jsonld, MotorcycleRepair.class);
        assertNotNull(acc);

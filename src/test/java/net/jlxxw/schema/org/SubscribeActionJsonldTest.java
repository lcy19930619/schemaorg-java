...
public class SubscribeActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SubscribeAction.jsonld");
        assertNotNull(is, "SubscribeAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SubscribeAction acc = mapper.readValue(jsonld, SubscribeAction.class);
        assertNotNull(acc);

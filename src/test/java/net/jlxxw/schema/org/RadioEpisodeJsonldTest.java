...
public class RadioEpisodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("RadioEpisode.jsonld");
        assertNotNull(is, "RadioEpisode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        RadioEpisode acc = mapper.readValue(jsonld, RadioEpisode.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        RadioEpisode obj = new RadioEpisode();
        assertEquals("schema:RadioEpisode", obj.getId());

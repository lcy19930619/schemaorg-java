...
public class GovernmentOrganizationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GovernmentOrganization.jsonld");
        assertNotNull(is, "GovernmentOrganization.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GovernmentOrganization acc = mapper.readValue(jsonld, GovernmentOrganization.class);
        assertNotNull(acc);

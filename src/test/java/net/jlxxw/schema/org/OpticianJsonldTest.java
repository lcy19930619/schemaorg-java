...
public class OpticianJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Optician.jsonld");
        assertNotNull(is, "Optician.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Optician acc = mapper.readValue(jsonld, Optician.class);
        assertNotNull(acc);

...
public class OfferShippingDetailsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OfferShippingDetails.jsonld");
        assertNotNull(is, "OfferShippingDetails.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OfferShippingDetails acc = mapper.readValue(jsonld, OfferShippingDetails.class);
        assertNotNull(acc);

...
public class QuantitativeValueDistributionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("QuantitativeValueDistribution.jsonld");
        assertNotNull(is, "QuantitativeValueDistribution.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        QuantitativeValueDistribution acc = mapper.readValue(jsonld, QuantitativeValueDistribution.class);
        assertNotNull(acc);

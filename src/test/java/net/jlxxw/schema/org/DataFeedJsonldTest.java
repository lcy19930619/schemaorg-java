...
public class DataFeedJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DataFeed.jsonld");
        assertNotNull(is, "DataFeed.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DataFeed acc = mapper.readValue(jsonld, DataFeed.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        DataFeed obj = new DataFeed();
        assertEquals("schema:DataFeed", obj.getId());

...
public class ActivateActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ActivateAction.jsonld");
        assertNotNull(is, "ActivateAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ActivateAction acc = mapper.readValue(jsonld, ActivateAction.class);
        assertNotNull(acc);

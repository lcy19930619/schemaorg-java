...
public class BookFormatTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BookFormatType.jsonld");
        assertNotNull(is, "BookFormatType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BookFormatType acc = mapper.readValue(jsonld, BookFormatType.class);
        assertNotNull(acc);

...
public class GeoCoordinatesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GeoCoordinates.jsonld");
        assertNotNull(is, "GeoCoordinates.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GeoCoordinates acc = mapper.readValue(jsonld, GeoCoordinates.class);
        assertNotNull(acc);

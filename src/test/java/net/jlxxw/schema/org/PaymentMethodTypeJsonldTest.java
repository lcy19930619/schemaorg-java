...
public class PaymentMethodTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PaymentMethodType.jsonld");
        assertNotNull(is, "PaymentMethodType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PaymentMethodType acc = mapper.readValue(jsonld, PaymentMethodType.class);
        assertNotNull(acc);

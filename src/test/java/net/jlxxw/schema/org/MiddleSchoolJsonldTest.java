...
public class MiddleSchoolJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MiddleSchool.jsonld");
        assertNotNull(is, "MiddleSchool.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MiddleSchool acc = mapper.readValue(jsonld, MiddleSchool.class);
        assertNotNull(acc);

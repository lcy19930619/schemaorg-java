...
public class ContinentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Continent.jsonld");
        assertNotNull(is, "Continent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Continent acc = mapper.readValue(jsonld, Continent.class);
        assertNotNull(acc);

...
public class LigamentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Ligament.jsonld");
        assertNotNull(is, "Ligament.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Ligament acc = mapper.readValue(jsonld, Ligament.class);
        assertNotNull(acc);

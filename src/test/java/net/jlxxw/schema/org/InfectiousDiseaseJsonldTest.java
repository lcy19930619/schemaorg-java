...
public class InfectiousDiseaseJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("InfectiousDisease.jsonld");
        assertNotNull(is, "InfectiousDisease.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        InfectiousDisease acc = mapper.readValue(jsonld, InfectiousDisease.class);
        assertNotNull(acc);

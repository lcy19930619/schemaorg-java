...
public class TelevisionChannelJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TelevisionChannel.jsonld");
        assertNotNull(is, "TelevisionChannel.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TelevisionChannel acc = mapper.readValue(jsonld, TelevisionChannel.class);
        assertNotNull(acc);

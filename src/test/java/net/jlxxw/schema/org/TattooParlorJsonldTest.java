...
public class TattooParlorJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TattooParlor.jsonld");
        assertNotNull(is, "TattooParlor.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TattooParlor acc = mapper.readValue(jsonld, TattooParlor.class);
        assertNotNull(acc);

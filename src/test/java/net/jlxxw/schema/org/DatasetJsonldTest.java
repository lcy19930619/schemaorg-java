...
public class DatasetJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Dataset.jsonld");
        assertNotNull(is, "Dataset.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Dataset acc = mapper.readValue(jsonld, Dataset.class);
        assertNotNull(acc);

...
public class MedicalObservationalStudyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalObservationalStudy.jsonld");
        assertNotNull(is, "MedicalObservationalStudy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalObservationalStudy acc = mapper.readValue(jsonld, MedicalObservationalStudy.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalObservationalStudy obj = new MedicalObservationalStudy();
        assertEquals("schema:MedicalObservationalStudy", obj.getId());

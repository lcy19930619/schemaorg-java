...
public class DDxElementJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DDxElement.jsonld");
        assertNotNull(is, "DDxElement.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DDxElement acc = mapper.readValue(jsonld, DDxElement.class);
        assertNotNull(acc);

...
public class SoftwareSourceCodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SoftwareSourceCode.jsonld");
        assertNotNull(is, "SoftwareSourceCode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SoftwareSourceCode acc = mapper.readValue(jsonld, SoftwareSourceCode.class);
        assertNotNull(acc);

...
public class PriceComponentTypeEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PriceComponentTypeEnumeration.jsonld");
        assertNotNull(is, "PriceComponentTypeEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PriceComponentTypeEnumeration acc = mapper.readValue(jsonld, PriceComponentTypeEnumeration.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        PriceComponentTypeEnumeration obj = new PriceComponentTypeEnumeration();
        assertEquals("schema:PriceComponentTypeEnumeration", obj.getId());

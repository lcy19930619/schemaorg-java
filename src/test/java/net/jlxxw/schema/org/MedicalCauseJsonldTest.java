...
public class MedicalCauseJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalCause.jsonld");
        assertNotNull(is, "MedicalCause.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalCause acc = mapper.readValue(jsonld, MedicalCause.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalCause obj = new MedicalCause();
        assertEquals("schema:MedicalCause", obj.getId());

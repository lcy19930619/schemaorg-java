...
public class PaymentStatusTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PaymentStatusType.jsonld");
        assertNotNull(is, "PaymentStatusType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PaymentStatusType acc = mapper.readValue(jsonld, PaymentStatusType.class);
        assertNotNull(acc);

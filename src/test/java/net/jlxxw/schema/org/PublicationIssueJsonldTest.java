...
public class PublicationIssueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PublicationIssue.jsonld");
        assertNotNull(is, "PublicationIssue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PublicationIssue acc = mapper.readValue(jsonld, PublicationIssue.class);
        assertNotNull(acc);

...
public class AmusementParkJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AmusementPark.jsonld");
        assertNotNull(is, "AmusementPark.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AmusementPark acc = mapper.readValue(jsonld, AmusementPark.class);
        assertNotNull(acc);

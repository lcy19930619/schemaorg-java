...
public class QuizJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Quiz.jsonld");
        assertNotNull(is, "Quiz.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Quiz acc = mapper.readValue(jsonld, Quiz.class);
        assertNotNull(acc);

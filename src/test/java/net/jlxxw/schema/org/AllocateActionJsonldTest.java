...
public class AllocateActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AllocateAction.jsonld");
        assertNotNull(is, "AllocateAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AllocateAction acc = mapper.readValue(jsonld, AllocateAction.class);
        assertNotNull(acc);

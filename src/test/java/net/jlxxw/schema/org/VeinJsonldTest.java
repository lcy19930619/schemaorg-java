...
public class VeinJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Vein.jsonld");
        assertNotNull(is, "Vein.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Vein acc = mapper.readValue(jsonld, Vein.class);
        assertNotNull(acc);

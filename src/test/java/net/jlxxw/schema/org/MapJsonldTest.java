...
public class MapJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Map.jsonld");
        assertNotNull(is, "Map.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Map acc = mapper.readValue(jsonld, Map.class);
        assertNotNull(acc);

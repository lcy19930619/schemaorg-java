...
public class SpeakableSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SpeakableSpecification.jsonld");
        assertNotNull(is, "SpeakableSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SpeakableSpecification acc = mapper.readValue(jsonld, SpeakableSpecification.class);
        assertNotNull(acc);

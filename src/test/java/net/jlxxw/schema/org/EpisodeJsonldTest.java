...
public class EpisodeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Episode.jsonld");
        assertNotNull(is, "Episode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Episode acc = mapper.readValue(jsonld, Episode.class);
        assertNotNull(acc);

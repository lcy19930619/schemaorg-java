...
public class MedicalEntityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalEntity.jsonld");
        assertNotNull(is, "MedicalEntity.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalEntity acc = mapper.readValue(jsonld, MedicalEntity.class);
        assertNotNull(acc);

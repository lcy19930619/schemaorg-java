...
public class DriveWheelConfigurationValueJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DriveWheelConfigurationValue.jsonld");
        assertNotNull(is, "DriveWheelConfigurationValue.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DriveWheelConfigurationValue acc = mapper.readValue(jsonld, DriveWheelConfigurationValue.class);
        assertNotNull(acc);

...
public class SportsClubJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SportsClub.jsonld");
        assertNotNull(is, "SportsClub.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SportsClub acc = mapper.readValue(jsonld, SportsClub.class);
        assertNotNull(acc);

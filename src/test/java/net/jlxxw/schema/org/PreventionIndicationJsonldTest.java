...
public class PreventionIndicationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PreventionIndication.jsonld");
        assertNotNull(is, "PreventionIndication.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PreventionIndication acc = mapper.readValue(jsonld, PreventionIndication.class);
        assertNotNull(acc);

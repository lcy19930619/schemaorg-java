...
public class OnlineBusinessJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OnlineBusiness.jsonld");
        assertNotNull(is, "OnlineBusiness.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OnlineBusiness acc = mapper.readValue(jsonld, OnlineBusiness.class);
        assertNotNull(acc);

...
public class TheaterGroupJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TheaterGroup.jsonld");
        assertNotNull(is, "TheaterGroup.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TheaterGroup acc = mapper.readValue(jsonld, TheaterGroup.class);
        assertNotNull(acc);

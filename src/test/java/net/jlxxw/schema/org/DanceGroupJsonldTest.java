...
public class DanceGroupJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DanceGroup.jsonld");
        assertNotNull(is, "DanceGroup.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DanceGroup acc = mapper.readValue(jsonld, DanceGroup.class);
        assertNotNull(acc);

...
public class ReturnMethodEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReturnMethodEnumeration.jsonld");
        assertNotNull(is, "ReturnMethodEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReturnMethodEnumeration acc = mapper.readValue(jsonld, ReturnMethodEnumeration.class);
        assertNotNull(acc);

...
public class MusicVideoObjectJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MusicVideoObject.jsonld");
        assertNotNull(is, "MusicVideoObject.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MusicVideoObject acc = mapper.readValue(jsonld, MusicVideoObject.class);
        assertNotNull(acc);

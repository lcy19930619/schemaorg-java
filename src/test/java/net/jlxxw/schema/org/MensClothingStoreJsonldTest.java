...
public class MensClothingStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MensClothingStore.jsonld");
        assertNotNull(is, "MensClothingStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MensClothingStore acc = mapper.readValue(jsonld, MensClothingStore.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MensClothingStore obj = new MensClothingStore();
        assertEquals("schema:MensClothingStore", obj.getId());

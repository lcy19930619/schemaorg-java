...
public class FastFoodRestaurantJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FastFoodRestaurant.jsonld");
        assertNotNull(is, "FastFoodRestaurant.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FastFoodRestaurant acc = mapper.readValue(jsonld, FastFoodRestaurant.class);
        assertNotNull(acc);

...
public class PhysicalTherapyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PhysicalTherapy.jsonld");
        assertNotNull(is, "PhysicalTherapy.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PhysicalTherapy acc = mapper.readValue(jsonld, PhysicalTherapy.class);
        assertNotNull(acc);

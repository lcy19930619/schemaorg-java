...
public class AutoRepairJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AutoRepair.jsonld");
        assertNotNull(is, "AutoRepair.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AutoRepair acc = mapper.readValue(jsonld, AutoRepair.class);
        assertNotNull(acc);

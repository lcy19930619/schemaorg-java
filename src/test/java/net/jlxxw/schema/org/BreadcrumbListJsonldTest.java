...
public class BreadcrumbListJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BreadcrumbList.jsonld");
        assertNotNull(is, "BreadcrumbList.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BreadcrumbList acc = mapper.readValue(jsonld, BreadcrumbList.class);
        assertNotNull(acc);

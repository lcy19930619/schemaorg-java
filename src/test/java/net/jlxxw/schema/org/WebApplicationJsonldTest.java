...
public class WebApplicationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WebApplication.jsonld");
        assertNotNull(is, "WebApplication.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WebApplication acc = mapper.readValue(jsonld, WebApplication.class);
        assertNotNull(acc);

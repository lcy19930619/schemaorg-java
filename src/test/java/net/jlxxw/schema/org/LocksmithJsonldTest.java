...
public class LocksmithJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Locksmith.jsonld");
        assertNotNull(is, "Locksmith.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Locksmith acc = mapper.readValue(jsonld, Locksmith.class);
        assertNotNull(acc);

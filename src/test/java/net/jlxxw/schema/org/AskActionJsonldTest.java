...
public class AskActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("AskAction.jsonld");
        assertNotNull(is, "AskAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        AskAction acc = mapper.readValue(jsonld, AskAction.class);
        assertNotNull(acc);

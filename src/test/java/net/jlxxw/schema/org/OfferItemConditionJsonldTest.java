...
public class OfferItemConditionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OfferItemCondition.jsonld");
        assertNotNull(is, "OfferItemCondition.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OfferItemCondition acc = mapper.readValue(jsonld, OfferItemCondition.class);
        assertNotNull(acc);

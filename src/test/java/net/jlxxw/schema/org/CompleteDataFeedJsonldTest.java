...
public class CompleteDataFeedJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CompleteDataFeed.jsonld");
        assertNotNull(is, "CompleteDataFeed.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CompleteDataFeed acc = mapper.readValue(jsonld, CompleteDataFeed.class);
        assertNotNull(acc);

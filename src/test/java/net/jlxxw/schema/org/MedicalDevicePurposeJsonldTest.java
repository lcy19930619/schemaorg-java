...
public class MedicalDevicePurposeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalDevicePurpose.jsonld");
        assertNotNull(is, "MedicalDevicePurpose.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalDevicePurpose acc = mapper.readValue(jsonld, MedicalDevicePurpose.class);
        assertNotNull(acc);

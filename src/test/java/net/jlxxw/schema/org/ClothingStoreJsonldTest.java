...
public class ClothingStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ClothingStore.jsonld");
        assertNotNull(is, "ClothingStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ClothingStore acc = mapper.readValue(jsonld, ClothingStore.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ClothingStore obj = new ClothingStore();
        assertEquals("schema:ClothingStore", obj.getId());

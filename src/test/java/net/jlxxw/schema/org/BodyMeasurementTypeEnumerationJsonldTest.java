...
public class BodyMeasurementTypeEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BodyMeasurementTypeEnumeration.jsonld");
        assertNotNull(is, "BodyMeasurementTypeEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BodyMeasurementTypeEnumeration acc = mapper.readValue(jsonld, BodyMeasurementTypeEnumeration.class);
        assertNotNull(acc);

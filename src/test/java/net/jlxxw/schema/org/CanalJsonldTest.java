...
public class CanalJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Canal.jsonld");
        assertNotNull(is, "Canal.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Canal acc = mapper.readValue(jsonld, Canal.class);
        assertNotNull(acc);

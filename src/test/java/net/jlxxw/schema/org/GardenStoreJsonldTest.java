...
public class GardenStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GardenStore.jsonld");
        assertNotNull(is, "GardenStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GardenStore acc = mapper.readValue(jsonld, GardenStore.class);
        assertNotNull(acc);

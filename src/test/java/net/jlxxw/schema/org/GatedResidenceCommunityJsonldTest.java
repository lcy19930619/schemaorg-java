...
public class GatedResidenceCommunityJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GatedResidenceCommunity.jsonld");
        assertNotNull(is, "GatedResidenceCommunity.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GatedResidenceCommunity acc = mapper.readValue(jsonld, GatedResidenceCommunity.class);
        assertNotNull(acc);

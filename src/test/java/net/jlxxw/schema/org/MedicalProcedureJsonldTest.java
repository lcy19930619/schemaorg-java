...
public class MedicalProcedureJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalProcedure.jsonld");
        assertNotNull(is, "MedicalProcedure.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalProcedure acc = mapper.readValue(jsonld, MedicalProcedure.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalProcedure obj = new MedicalProcedure();
        assertEquals("schema:MedicalProcedure", obj.getId());

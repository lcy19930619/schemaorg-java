...
public class VideoGameSeriesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VideoGameSeries.jsonld");
        assertNotNull(is, "VideoGameSeries.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VideoGameSeries acc = mapper.readValue(jsonld, VideoGameSeries.class);
        assertNotNull(acc);

...
public class ResearchOrganizationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ResearchOrganization.jsonld");
        assertNotNull(is, "ResearchOrganization.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ResearchOrganization acc = mapper.readValue(jsonld, ResearchOrganization.class);
        assertNotNull(acc);

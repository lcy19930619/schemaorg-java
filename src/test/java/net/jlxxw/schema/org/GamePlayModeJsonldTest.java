...
public class GamePlayModeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GamePlayMode.jsonld");
        assertNotNull(is, "GamePlayMode.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GamePlayMode acc = mapper.readValue(jsonld, GamePlayMode.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        GamePlayMode obj = new GamePlayMode();
        assertEquals("schema:GamePlayMode", obj.getId());

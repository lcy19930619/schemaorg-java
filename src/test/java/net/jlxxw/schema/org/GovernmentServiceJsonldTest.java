...
public class GovernmentServiceJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GovernmentService.jsonld");
        assertNotNull(is, "GovernmentService.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GovernmentService acc = mapper.readValue(jsonld, GovernmentService.class);
        assertNotNull(acc);

...
public class OceanBodyOfWaterJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OceanBodyOfWater.jsonld");
        assertNotNull(is, "OceanBodyOfWater.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OceanBodyOfWater acc = mapper.readValue(jsonld, OceanBodyOfWater.class);
        assertNotNull(acc);

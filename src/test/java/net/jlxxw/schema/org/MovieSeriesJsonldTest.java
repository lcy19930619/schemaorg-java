...
public class MovieSeriesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MovieSeries.jsonld");
        assertNotNull(is, "MovieSeries.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MovieSeries acc = mapper.readValue(jsonld, MovieSeries.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MovieSeries obj = new MovieSeries();
        assertEquals("schema:MovieSeries", obj.getId());

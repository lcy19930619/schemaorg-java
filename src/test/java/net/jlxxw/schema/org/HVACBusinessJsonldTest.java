...
public class HVACBusinessJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HVACBusiness.jsonld");
        assertNotNull(is, "HVACBusiness.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HVACBusiness acc = mapper.readValue(jsonld, HVACBusiness.class);
        assertNotNull(acc);

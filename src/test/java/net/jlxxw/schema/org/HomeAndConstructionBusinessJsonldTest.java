...
public class HomeAndConstructionBusinessJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HomeAndConstructionBusiness.jsonld");
        assertNotNull(is, "HomeAndConstructionBusiness.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HomeAndConstructionBusiness acc = mapper.readValue(jsonld, HomeAndConstructionBusiness.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        HomeAndConstructionBusiness obj = new HomeAndConstructionBusiness();
        assertEquals("schema:HomeAndConstructionBusiness", obj.getId());

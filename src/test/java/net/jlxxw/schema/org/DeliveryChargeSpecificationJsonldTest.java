...
public class DeliveryChargeSpecificationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DeliveryChargeSpecification.jsonld");
        assertNotNull(is, "DeliveryChargeSpecification.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DeliveryChargeSpecification acc = mapper.readValue(jsonld, DeliveryChargeSpecification.class);
        assertNotNull(acc);

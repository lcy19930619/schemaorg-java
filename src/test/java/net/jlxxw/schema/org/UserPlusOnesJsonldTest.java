...
public class UserPlusOnesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserPlusOnes.jsonld");
        assertNotNull(is, "UserPlusOnes.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserPlusOnes acc = mapper.readValue(jsonld, UserPlusOnes.class);
        assertNotNull(acc);

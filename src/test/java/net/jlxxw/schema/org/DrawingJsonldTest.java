...
public class DrawingJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Drawing.jsonld");
        assertNotNull(is, "Drawing.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Drawing acc = mapper.readValue(jsonld, Drawing.class);
        assertNotNull(acc);

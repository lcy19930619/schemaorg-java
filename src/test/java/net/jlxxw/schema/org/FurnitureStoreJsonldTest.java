...
public class FurnitureStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FurnitureStore.jsonld");
        assertNotNull(is, "FurnitureStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FurnitureStore acc = mapper.readValue(jsonld, FurnitureStore.class);
        assertNotNull(acc);

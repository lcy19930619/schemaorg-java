...
public class MedicalSignJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalSign.jsonld");
        assertNotNull(is, "MedicalSign.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalSign acc = mapper.readValue(jsonld, MedicalSign.class);
        assertNotNull(acc);

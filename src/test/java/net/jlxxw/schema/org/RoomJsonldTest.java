...
public class RoomJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Room.jsonld");
        assertNotNull(is, "Room.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Room acc = mapper.readValue(jsonld, Room.class);
        assertNotNull(acc);

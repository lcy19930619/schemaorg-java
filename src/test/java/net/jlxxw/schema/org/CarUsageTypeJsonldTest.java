...
public class CarUsageTypeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("CarUsageType.jsonld");
        assertNotNull(is, "CarUsageType.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        CarUsageType acc = mapper.readValue(jsonld, CarUsageType.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        CarUsageType obj = new CarUsageType();
        assertEquals("schema:CarUsageType", obj.getId());

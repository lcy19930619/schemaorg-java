...
public class BoatTripJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BoatTrip.jsonld");
        assertNotNull(is, "BoatTrip.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BoatTrip acc = mapper.readValue(jsonld, BoatTrip.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        BoatTrip obj = new BoatTrip();
        assertEquals("schema:BoatTrip", obj.getId());

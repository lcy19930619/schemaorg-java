...
public class TelevisionStationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("TelevisionStation.jsonld");
        assertNotNull(is, "TelevisionStation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        TelevisionStation acc = mapper.readValue(jsonld, TelevisionStation.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        TelevisionStation obj = new TelevisionStation();
        assertEquals("schema:TelevisionStation", obj.getId());

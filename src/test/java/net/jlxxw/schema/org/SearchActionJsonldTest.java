...
public class SearchActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("SearchAction.jsonld");
        assertNotNull(is, "SearchAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        SearchAction acc = mapper.readValue(jsonld, SearchAction.class);
        assertNotNull(acc);

...
public class OccupationalExperienceRequirementsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("OccupationalExperienceRequirements.jsonld");
        assertNotNull(is, "OccupationalExperienceRequirements.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        OccupationalExperienceRequirements acc = mapper.readValue(jsonld, OccupationalExperienceRequirements.class);
        assertNotNull(acc);

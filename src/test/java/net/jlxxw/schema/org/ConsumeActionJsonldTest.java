...
public class ConsumeActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ConsumeAction.jsonld");
        assertNotNull(is, "ConsumeAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ConsumeAction acc = mapper.readValue(jsonld, ConsumeAction.class);
        assertNotNull(acc);

...
public class PresentationDigitalDocumentJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PresentationDigitalDocument.jsonld");
        assertNotNull(is, "PresentationDigitalDocument.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PresentationDigitalDocument acc = mapper.readValue(jsonld, PresentationDigitalDocument.class);
        assertNotNull(acc);

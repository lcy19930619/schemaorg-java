...
public class LegalForceStatusJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("LegalForceStatus.jsonld");
        assertNotNull(is, "LegalForceStatus.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        LegalForceStatus acc = mapper.readValue(jsonld, LegalForceStatus.class);
        assertNotNull(acc);

...
public class UpdateActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UpdateAction.jsonld");
        assertNotNull(is, "UpdateAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UpdateAction acc = mapper.readValue(jsonld, UpdateAction.class);
        assertNotNull(acc);

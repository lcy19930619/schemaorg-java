...
public class VoteActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("VoteAction.jsonld");
        assertNotNull(is, "VoteAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        VoteAction acc = mapper.readValue(jsonld, VoteAction.class);
        assertNotNull(acc);

...
public class BridgeJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Bridge.jsonld");
        assertNotNull(is, "Bridge.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Bridge acc = mapper.readValue(jsonld, Bridge.class);
        assertNotNull(acc);

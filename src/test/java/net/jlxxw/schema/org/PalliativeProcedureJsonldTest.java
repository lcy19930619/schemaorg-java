...
public class PalliativeProcedureJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PalliativeProcedure.jsonld");
        assertNotNull(is, "PalliativeProcedure.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PalliativeProcedure acc = mapper.readValue(jsonld, PalliativeProcedure.class);
        assertNotNull(acc);

...
public class UserTweetsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("UserTweets.jsonld");
        assertNotNull(is, "UserTweets.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        UserTweets acc = mapper.readValue(jsonld, UserTweets.class);
        assertNotNull(acc);

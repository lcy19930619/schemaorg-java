...
public class HowToItemJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HowToItem.jsonld");
        assertNotNull(is, "HowToItem.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HowToItem acc = mapper.readValue(jsonld, HowToItem.class);
        assertNotNull(acc);

...
public class DonateActionJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DonateAction.jsonld");
        assertNotNull(is, "DonateAction.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DonateAction acc = mapper.readValue(jsonld, DonateAction.class);
        assertNotNull(acc);

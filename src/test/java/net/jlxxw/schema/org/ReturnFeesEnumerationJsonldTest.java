...
public class ReturnFeesEnumerationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReturnFeesEnumeration.jsonld");
        assertNotNull(is, "ReturnFeesEnumeration.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReturnFeesEnumeration acc = mapper.readValue(jsonld, ReturnFeesEnumeration.class);
        assertNotNull(acc);

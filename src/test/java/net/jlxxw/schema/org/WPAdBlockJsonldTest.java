...
public class WPAdBlockJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("WPAdBlock.jsonld");
        assertNotNull(is, "WPAdBlock.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        WPAdBlock acc = mapper.readValue(jsonld, WPAdBlock.class);
        assertNotNull(acc);

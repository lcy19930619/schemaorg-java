...
public class CooperativeJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Cooperative.jsonld");
        assertNotNull(is, "Cooperative.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Cooperative acc = mapper.readValue(jsonld, Cooperative.class);
        assertNotNull(acc);

...
public class ReportedDoseScheduleJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ReportedDoseSchedule.jsonld");
        assertNotNull(is, "ReportedDoseSchedule.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ReportedDoseSchedule acc = mapper.readValue(jsonld, ReportedDoseSchedule.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        ReportedDoseSchedule obj = new ReportedDoseSchedule();
        assertEquals("schema:ReportedDoseSchedule", obj.getId());

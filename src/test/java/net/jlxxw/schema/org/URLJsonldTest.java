...
public class URLJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("URL.jsonld");
        assertNotNull(is, "URL.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        URL acc = mapper.readValue(jsonld, URL.class);
        assertNotNull(acc);

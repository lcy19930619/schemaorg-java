...
public class BookSeriesJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BookSeries.jsonld");
        assertNotNull(is, "BookSeries.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BookSeries acc = mapper.readValue(jsonld, BookSeries.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        BookSeries obj = new BookSeries();
        assertEquals("schema:BookSeries", obj.getId());

...
public class AudiobookJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Audiobook.jsonld");
        assertNotNull(is, "Audiobook.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Audiobook acc = mapper.readValue(jsonld, Audiobook.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Audiobook obj = new Audiobook();
        assertEquals("schema:Audiobook", obj.getId());

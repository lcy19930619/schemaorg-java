...
public class HouseJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("House.jsonld");
        assertNotNull(is, "House.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        House acc = mapper.readValue(jsonld, House.class);
        assertNotNull(acc);

...
public class DrugLegalStatusJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DrugLegalStatus.jsonld");
        assertNotNull(is, "DrugLegalStatus.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DrugLegalStatus acc = mapper.readValue(jsonld, DrugLegalStatus.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        DrugLegalStatus obj = new DrugLegalStatus();
        assertEquals("schema:DrugLegalStatus", obj.getId());

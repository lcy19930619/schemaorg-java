...
public class FloorPlanJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("FloorPlan.jsonld");
        assertNotNull(is, "FloorPlan.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        FloorPlan acc = mapper.readValue(jsonld, FloorPlan.class);
        assertNotNull(acc);

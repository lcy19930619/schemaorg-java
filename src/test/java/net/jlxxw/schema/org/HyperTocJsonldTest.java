...
public class HyperTocJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HyperToc.jsonld");
        assertNotNull(is, "HyperToc.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HyperToc acc = mapper.readValue(jsonld, HyperToc.class);
        assertNotNull(acc);

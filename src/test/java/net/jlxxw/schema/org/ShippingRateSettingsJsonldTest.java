...
public class ShippingRateSettingsJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("ShippingRateSettings.jsonld");
        assertNotNull(is, "ShippingRateSettings.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        ShippingRateSettings acc = mapper.readValue(jsonld, ShippingRateSettings.class);
        assertNotNull(acc);

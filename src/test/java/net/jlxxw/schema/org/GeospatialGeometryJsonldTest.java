...
public class GeospatialGeometryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("GeospatialGeometry.jsonld");
        assertNotNull(is, "GeospatialGeometry.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        GeospatialGeometry acc = mapper.readValue(jsonld, GeospatialGeometry.class);
        assertNotNull(acc);

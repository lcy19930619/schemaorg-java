...
public class PoliticalPartyJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("PoliticalParty.jsonld");
        assertNotNull(is, "PoliticalParty.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        PoliticalParty acc = mapper.readValue(jsonld, PoliticalParty.class);
        assertNotNull(acc);

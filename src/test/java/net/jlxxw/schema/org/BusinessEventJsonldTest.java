...
public class BusinessEventJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("BusinessEvent.jsonld");
        assertNotNull(is, "BusinessEvent.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        BusinessEvent acc = mapper.readValue(jsonld, BusinessEvent.class);
        assertNotNull(acc);

...
public class DepartmentStoreJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("DepartmentStore.jsonld");
        assertNotNull(is, "DepartmentStore.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        DepartmentStore acc = mapper.readValue(jsonld, DepartmentStore.class);
        assertNotNull(acc);

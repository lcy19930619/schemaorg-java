...
public class NightClubJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("NightClub.jsonld");
        assertNotNull(is, "NightClub.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        NightClub acc = mapper.readValue(jsonld, NightClub.class);
        assertNotNull(acc);

...
public class MotorcycleDealerJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MotorcycleDealer.jsonld");
        assertNotNull(is, "MotorcycleDealer.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MotorcycleDealer acc = mapper.readValue(jsonld, MotorcycleDealer.class);
        assertNotNull(acc);

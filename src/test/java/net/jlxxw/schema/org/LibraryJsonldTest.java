...
public class LibraryJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Library.jsonld");
        assertNotNull(is, "Library.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Library acc = mapper.readValue(jsonld, Library.class);
        assertNotNull(acc);

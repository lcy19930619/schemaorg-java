...
public class BlogJsonldTest {
    ...
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Blog.jsonld");
        assertNotNull(is, "Blog.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Blog acc = mapper.readValue(jsonld, Blog.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        Blog obj = new Blog();
        assertEquals("schema:Blog", obj.getId());

...
public class MedicalRiskCalculatorJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("MedicalRiskCalculator.jsonld");
        assertNotNull(is, "MedicalRiskCalculator.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        MedicalRiskCalculator acc = mapper.readValue(jsonld, MedicalRiskCalculator.class);
        assertNotNull(acc);
    ...
    public void testDefaultIdAndType() {
        MedicalRiskCalculator obj = new MedicalRiskCalculator();
        assertEquals("schema:MedicalRiskCalculator", obj.getId());

...
public class CorporationJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("Corporation.jsonld");
        assertNotNull(is, "Corporation.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        Corporation acc = mapper.readValue(jsonld, Corporation.class);
        assertNotNull(acc);

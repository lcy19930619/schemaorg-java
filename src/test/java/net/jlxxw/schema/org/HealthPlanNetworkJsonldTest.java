...
public class HealthPlanNetworkJsonldTest {
    @Test
    public void testJsonldRoundTrip() throws Exception {
        ...
        InputStream is = getClass().getClassLoader().getResourceAsStream("HealthPlanNetwork.jsonld");
        assertNotNull(is, "HealthPlanNetwork.jsonld resource not found");
        String jsonld = new Scanner(is, StandardCharsets.UTF_8.name()).useDelimiter("\\A").next();

        // Deserialize to Java object
        HealthPlanNetwork acc = mapper.readValue(jsonld, HealthPlanNetwork.class);
        assertNotNull(acc);
